  ...
  >
    <div class="device-detail-container">
      <ElDescriptions :column="1" border>
        ...
        <ElDescriptionsItem label="管网路段">{{ detailObj.pipeRoad }}</ElDescriptionsItem>
        <ElDescriptionsItem label="所属区域">{{ detailObj.areaName }}</ElDescriptionsItem>
        <ElDescriptionsItem label="管网压力">{{ detailObj.pipePressure }}MPa</ElDescriptionsItem>
        <ElDescriptionsItem label="燃气浓度">{{ detailObj.gasConcentration }}%</ElDescriptionsItem>
        <ElDescriptionsItem label="负责运维员">{{ detailObj.staffName }}</ElDescriptionsItem>
