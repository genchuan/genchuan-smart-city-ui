...
const abnormalTypeMap = {
  'overtime_occupy': '超时长占用',
  'non_motor_occupy': '非机动占用',
  'device_fault': '设备故障',
  '': '无'
};
      ...
      if (item && typeof coordX === 'number' && typeof coordY === 'number') {
        ...
        geometriesData.push({
          ...
          properties: {
            ...
            occupy_space: item.occupy_space,
            warn_occupy_count: item.warn_occupy_count,
...
const getTooltipContent = (properties) => {
  ...
  const abnormalTypeText = abnormalTypeMap[properties.abnormal_type] || '未知';
  const occupyRate = properties.total_space
    ? ((properties.occupy_space / properties.total_space) * 100).toFixed(1) + '%'
    : '未知';
  ...
  return `
    <div style="padding: 10px 12px; font-size: 14px; color: #333; background: white; border: 1px solid #ccc; min-width: 280px; border-radius: 4px;">
      <div style="margin-bottom: 8px; font-weight: bold; color: #1E90FF; border-bottom: 1px solid #eee; padding-bottom: 4px; text-align: center;">停车场资源信息</div>
      <div style="${rowStyle}">
      ...
      <div style="${rowStyle}">
        <span style="${labelStyle}">占用车位数：</span>
        <span style="${valueStyle}">${properties.occupy_space || '未知'}</span>
      </div>
      <div style="${rowStyle}">
        <span style="${labelStyle}">车位占用率：</span>
        <span style="${valueStyle}">${occupyRate}</span>
      </div>
      <div style="${rowStyle}">
        <span style="${labelStyle}">预警占用数：</span>
        <span style="${valueStyle}">${properties.warn_occupy_count || 0}</span>
      </div>
      <div style="${rowStyle}">
        <span style="${labelStyle}">异常类型：</span>
        <span style="${valueStyle}">${abnormalTypeText}</span>
      </div>
      <div style="${rowStyle}">
...
const mapCallback = () => {
  ...
  startOrbitAnimation();
  // ✅ 核心修复：移除不存在的 getOptions() 方法，直接访问 enableRotate 属性
