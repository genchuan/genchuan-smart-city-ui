...
export const markFaultHandled = async (mngCompId) => {
  try {
    const response = await requestClient.post(`${BASE_URL}/markHandled`, {
      mng_comp_id: mngCompId,
    });
    if (response.data && response.data.code === 200) {
      return response.data;
    }
...
export const fetchFaultTypeDistribution = async () => {
  try {
    const response = await requestClient.get(
      `${BASE_URL}/faultTypeDistribution`,
    );
...
export const createEnforcementWorkOrder = async (orderData) => {
  try {
    const response = await requestClient.post(
      `${BASE_URL}/createEnforcementOrder`,
      orderData,
    );
    if (response.data && response.data.code === 200) {
      return response.data;
...
export const markOrderCompleted = async (mngMatterId) => {
  try {
    const response = await requestClient.post(`${BASE_URL}/markCompleted`, {
      mng_matter_id: mngMatterId,
    });
    if (response.data && response.data.code === 200) {
      return response.data;
    }
...
export const fetchHighIncidenceAnalysis = async (areaName) => {
  try {
    const response = await requestClient.get(
      `${BASE_URL}/highIncidenceAnalysis`,
    ...
    const analysisMap = {
      台江步行街: {
        reason:
          '1. 步行街人流量大，占道经营收益高，违规者屡教不改；2. 执法巡逻频次不足（仅每2小时1次）；3. 缺乏长效管控机制，违规成本低。',
        suggestion:
          '1. 增加执法巡逻频次至每1小时1次；2. 联合市场监管部门取缔无证摊贩；3. 划定规范经营区域，引导合规经营；4. 安装高清监控，远程取证执法。',
      },
      仓山万达商圈: {
        reason:
          '1. 商圈商户广告投放需求大，违规设置成本低；2. 广告审批流程繁琐，部分商户铤而走险；3. 外立面巡查覆盖不全，违规广告发现不及时。',
        suggestion:
          '1. 简化广告审批流程，压缩办理时限至1个工作日；2. 建立商圈广告台账，定期核查；3. 联合物业开展日常巡查，及时拆除违规广告；4. 对多次违规商户进行公示警告。',
      },
      鼓楼东街口: {
        reason:
          '1. 天桥下等公共空间管理权责不清；2. 临时棚屋搭建速度快，拆除后易反弹；3. 周边小商品需求大，违规搭建有市场。',
        suggestion:
          '1. 明确公共空间管理责任主体，落实网格化管理；2. 加装防违监控，实时预警违规搭建；3. 联合城管、街道开展专项整治行动；4. 引导违规者至正规经营场所。',
      },
      默认: {
        reason:
          '高发区域核心原因：1. 违规收益高于违规成本；2. 执法覆盖存在盲区；3. 长效管控机制缺失。',
        suggestion:
          '通用整改建议：1. 增加重点区域执法频次；2. 建立违规台账，实施分级管控；3. 联合多部门开展专项整治；4. 加强宣传教育，提升商户合规意识。',
      },
    };
    ...
    return [
      {
        ...
        // 多边形坐标（[lng, lat] 数组）
        polygon_coords: [
          [baseLng + 0.0, baseLat + 0.0],
          [baseLng + 0.003, baseLat + 0.0],
          [baseLng + 0.003, baseLat + 0.003],
          [baseLng + 0.0, baseLat + 0.003],
          [baseLng + 0.0, baseLat + 0.0],
        ],
      },
      {
      ...
      },
      {
        ...
        polygon_coords: [
          ...
          [baseLng + 0.004, baseLat + 0.0],
          [baseLng + 0.001, baseLat + 0.0],
          [baseLng + 0.001, baseLat - 0.003],
        ],
      },
      {
...
export const markClearPointCompleted = async (clearPointId) => {
  try {
    const response = await requestClient.post(
      `${BASE_URL}/markClearCompleted`,
      { clear_point_id: clearPointId },
    );
    if (response.data && response.data.code === 200) {
      return response.data;
    }
...
export const markCleanAreaCompleted = async (cleanAreaId) => {
  try {
    const response = await requestClient.post(
      `${BASE_URL}/markCleanCompleted`,
      { clean_area_id: cleanAreaId },
    );
    if (response.data && response.data.code === 200) {
      return response.data;
    }
...
export const fetchCleanCoverageDistribution = async () => {
  try {
    const response = await requestClient.get(
      `${BASE_URL}/cleanCoverageDistribution`,
    );
    if (response.data && response.data.legend && response.data.series) {
      return response.data;
    }
