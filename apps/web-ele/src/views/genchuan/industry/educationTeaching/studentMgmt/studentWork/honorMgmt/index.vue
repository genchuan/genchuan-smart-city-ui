...
import {
  ...
  getColumnsByStatus,
  useCreateFormSchema,
} from '#/api/genchuan/industry/educationTeaching/studentMgmt/studentWork/honorMgmt/form.js';

const getStatusType = (status) => {
  const map = { '待审核': 'warning', '已通过': 'success', '已推送': 'info' };
...
async function handleBatchAudit() {
  if (checkedIds.value.length === 0) return ElMessage.warning('请至少选择一条荣誉记录');
  const selectedRows = checkedRows.value.filter(row => row.status === '待审核');
  if (selectedRows.length === 0) return ElMessage.warning('请选择状态为【待审核】的荣誉记录');
  try {
    await ElMessageBox.confirm(`确认审核选中的 ${selectedRows.length} 条荣誉记录？审核后状态将变为"已通过"。`, '批量审核确认', { confirmButtonText: '确认', cancelButtonText: '取消', type: 'warning' });
...
async function handleAudit(row) {
  if (row.status !== '待审核') return ElMessage.warning('只有待审核状态的荣誉可以审核');
  try {
    await ElMessageBox.confirm(`确认审核荣誉"${row.honorName}"？审核后状态将变为"已通过"。`, '审核确认', { confirmButtonText: '确认', cancelButtonText: '取消', type: 'warning' });
    const loading = ElLoading.service({ text: '审核中...' });
    try {
...
async function handlePush(row) {
  if (row.status !== '已通过') return ElMessage.warning('只有已通过状态的荣誉可以推送');
  try {
    await ElMessageBox.confirm(`确认推送荣誉"${row.honorName}"给学生及家长？推送后状态将变为"已推送"。`, '推送确认', { confirmButtonText: '确认', cancelButtonText: '取消', type: 'warning' });
...
const [CreateForm, createFormApi] = useVbenForm({
  ...
  handleSubmit: async (values) => {
    ...
    try {
      ...
      if (isEditMode.value) res = await updateHonorMgmt({ ...submitData, id: currentEditId.value });
      else res = await createHonorMgmt(submitData);
      if (res && res !== false) {
...
const handleChartFilter = (event) => {
  ...
  if (type === 'status') {
    ...
    handleFilterTagClick('honorType', value);
...
onMounted(() => {
  window.addEventListener('honor-chart-filter', handleChartFilter);
});
onUnmounted(() => {
  window.removeEventListener('honor-chart-filter', handleChartFilter);
});
</script>

<template>
  <div class="tools-table-new">
    <HonorDetailDrawer ref="honorDetailDrawerRef" :detail-obj="dataObj.detailObj" @refresh="handleRefresh"/>
    <Drawer title="搜索"><QueryForm/></Drawer>
    <CreateDrawer :title="isEditMode ? '编辑荣誉信息' : '新增荣誉信息'"><CreateForm/></CreateDrawer>
    <Grid>
