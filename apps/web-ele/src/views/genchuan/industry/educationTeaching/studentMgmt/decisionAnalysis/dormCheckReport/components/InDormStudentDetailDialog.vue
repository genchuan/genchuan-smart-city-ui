...
import { getPresentStudentDetailList } from '#/api/genchuan/industry/educationTeaching/studentMgmt/decisionAnalysis/dormCheckReport/data.js';
...
const columns = [
  ...
  { field: 'inTime', title: '在寝时间', minWidth: 160 },
];
...
const fetchData = async (params) => {
  ...
  const res = await getPresentStudentDetailList({
  ...
  if (res.code === 200) {
    ...
    console.error('获取在寝学生明细失败', res.msg);
    return { list: [], total: 0 };
  }
};
...
<template>
  <DetailDrawer title="在寝学生明细" class="genchuan-detail-drawer">
