...
const getStatusType = (status) => {
  const map = { '待打卡': 'warning', '待调班审批': 'danger', '待出车审批': 'danger', '已完成': 'success' };
...
function clearFilters() { tagFilters.value = {}; gridApi.reload(); }
function removeFilterTag(field) { delete tagFilters.value[field]; gridApi.reload(); }
...
const [ScheduleDrawer, scheduleDrawerApi] = useVbenDrawer({ modal: false, footer: false, onCancel: () => scheduleDrawerApi.close() });
const [ShiftApplyDrawer, shiftApplyDrawerApi] = useVbenDrawer({ modal: false, footer: false, onCancel: () => shiftApplyDrawerApi.close() });
const [VehicleApplyDrawer, vehicleApplyDrawerApi] = useVbenDrawer({ modal: false, footer: false, onCancel: () => vehicleApplyDrawerApi.close() });
const [UploadRecordDrawer, uploadRecordDrawerApi] = useVbenDrawer({ modal: false, footer: false, onCancel: () => uploadRecordDrawerApi.close() });
const [EditDrawer, editDrawerApi] = useVbenDrawer({ modal: false, footer: false, onCancel: () => editDrawerApi.close() });
...
function handleRowCheckboxChange({ records }) { checkedIds.value = records.map(item => item.id); checkedRows.value = records; }
const searchParams = ref({});
const userOptions = ref([]);
const loadUserOptions = async () => { const res = await getUserOptions(); userOptions.value = res; };
...
function handleSchedule() { scheduleFormApi.resetForm(); scheduleDrawerApi.open(); }
...
function handleBatchShiftApply() {
  if (checkedIds.value.length === 0) { ElMessage.warning('请至少选择一条值班记录'); return; }
  const selectedRows = checkedRows.value.filter(row => row.status === '待打卡');
  if (selectedRows.length === 0) { ElMessage.warning('请选择状态为【待打卡】的值班记录'); return; }
...
const [ShiftApplyForm, shiftApplyFormApi] = useVbenForm({
  ...
  handleSubmit: async (values) => {
    ...
    try {
      ...
      const res = await shiftApplyDutyMgmt({ ids, transferReason: values.transferReason, transferUser: values.transferUser });
      if (res && res !== false) {
...
const [VehicleApplyForm, vehicleApplyFormApi] = useVbenForm({
  ...
  handleSubmit: async (values) => {
    ...
    try {
      ...
      const res = await vehicleApplyDutyMgmt({ ids, carReason: values.carReason, carDestination: values.carDestination });
      if (res && res !== false) {
...
async function handleCheckin(row) {
  if (row.status !== '待打卡') { ElMessage.warning('只有待打卡状态的记录可以打卡'); return; }
  try {
    await ElMessageBox.confirm(`确认打卡（值班人：${row.dutyUser}，日期：${row.dutyDate}）？`, '打卡确认', { confirmButtonText: '确认', cancelButtonText: '取消', type: 'warning' });
    const loading = ElLoading.service({ text: '打卡中...' });
    try {
      const res = await checkinDutyMgmt({ ids: [row.id] });
      if (res && res !== false) { ElMessage.success('打卡成功'); handleRefresh(); } else { ElMessage.error('打卡失败'); }
    } finally { loading.close(); }
  } catch {}
}
async function handleBatchCheckin() {
  ...
  if (selectedRows.length === 0) { ElMessage.warning('请选择状态为【待打卡】的值班记录'); return; }
  try {
    await ElMessageBox.confirm(`确认打卡选中的 ${selectedRows.length} 条值班记录？`, '批量打卡确认', { confirmButtonText: '确认', cancelButtonText: '取消', type: 'warning' });
    const loading = ElLoading.service({ text: '打卡中...' });
    try {
      const ids = selectedRows.map(row => row.id);
      const res = await checkinDutyMgmt({ ids });
      if (res && res !== false) { ElMessage.success('批量打卡成功'); handleRefresh(); } else { ElMessage.error('批量打卡失败'); }
    } finally { loading.close(); }
  } catch {}
}
...
function handleShiftAudit(row) {
  if (row.status !== '待调班审批') { ElMessage.warning('只有待调班审批状态的记录可以审批'); return; }
...
async function confirmShiftAudit() {
  if (!shiftAuditResult.value) { ElMessage.warning('请选择审批结果'); return; }
  const loading = ElLoading.service({ text: '审批中...' });
  try {
    const res = await shiftAuditDutyMgmt({ id: currentShiftAuditRow.value.id, auditResult: shiftAuditResult.value, remark: shiftAuditRemark.value || '' });
    if (res && res !== false) {
...
function handleVehicleAudit(row) {
  if (row.status !== '待出车审批') { ElMessage.warning('只有待出车审批状态的记录可以审批'); return; }
...
async function confirmVehicleAudit() {
  if (!vehicleAuditResult.value) { ElMessage.warning('请选择审批结果'); return; }
  const loading = ElLoading.service({ text: '审批中...' });
  try {
    const res = await vehicleAuditDutyMgmt({ id: currentVehicleAuditRow.value.id, auditResult: vehicleAuditResult.value, remark: vehicleAuditRemark.value || '' });
    if (res && res !== false) {
...
const [UploadRecordForm, uploadRecordFormApi] = useVbenForm({
  ...
  handleSubmit: async (values) => {
    ...
    try {
      const res = await uploadRecordDutyMgmt({ id: currentUploadRow.value.id, recordContent: values.recordContent });
      if (res && res !== false) {
...
function handleOpenDetail(row) { dataObj.detailObj = row; dutyDetailDrawerRef.value.open(); }

// 查询表单
const [QueryForm] = useVbenForm({
  collapsed: false,
  commonConfig: { componentProps: { class: 'w-full' }, formItemClass: 'col-span-2', labelWidth: 100 },
  handleSubmit: (values) => { searchParams.value = { ...values }; drawerApi.close(); gridApi.reload(); },
  layout: 'horizontal',
...
<template>
  <div class="park-lot-table-new">
    <DutyDetailDrawer ref="dutyDetailDrawerRef" :detail-obj="dataObj.detailObj" @refresh="handleRefresh" />
    <Drawer title="搜索"><QueryForm /></Drawer>
    <ScheduleDrawer title="排班"><ScheduleForm /></ScheduleDrawer>
    <ShiftApplyDrawer title="调班申请"><ShiftApplyForm /></ShiftApplyDrawer>
    <VehicleApplyDrawer title="出车申请"><VehicleApplyForm /></VehicleApplyDrawer>
    <UploadRecordDrawer title="上传值班记录"><UploadRecordForm /></UploadRecordDrawer>
    <EditDrawer title="编辑值班记录"><EditForm /></EditDrawer>

    <Grid>
      <template #table-title>
        <ElTag v-for="(value, field) in tagFilters" :key="field" type="success" closable @close="removeFilterTag(field)" style="height: 32px; margin: 4px 8px 4px 0; line-height: 32px">
      ...
      <template #dutyUser="{ row }">
        <el-text @click="handleFilterTagClick('dutyUser', row.dutyUser)" type="primary" style="cursor: pointer;">{{ row.dutyUser }}</el-text>
      </template>
      ...
      <template #actions="{ row }">
        <div class="table-toolbar-tools">
          ...
          <IconButton v-if="row.status === '待打卡'" content="打卡" icon-name="Check" @click="handleCheckin(row)" />
          <IconButton v-if="row.status === '待调班审批'" content="调班审批" icon-name="Checked" @click="handleShiftAudit(row)" />
          <IconButton v-if="row.status === '待出车审批'" content="出车审批" icon-name="Checked" @click="handleVehicleAudit(row)" />
          <IconButton v-if="row.status === '已完成'" content="上传记录" icon-name="Upload" @click="handleUploadRecord(row)" />
        </div>
      </template>
    </Grid>
      ...
      <el-form label-width="100px">
        <el-form-item label="审批结果" required>
          <el-select v-model="shiftAuditResult" placeholder="请选择审批结果" style="width: 100%;">
            <el-option label="通过" value="通过" /><el-option label="驳回" value="驳回" />
          </el-select>
        </el-form-item>
        <el-form-item label="审批备注"><el-input v-model="shiftAuditRemark" type="textarea" :rows="3" placeholder="请输入备注（可选）" /></el-form-item>
      </el-form>
      <template #footer><el-button @click="shiftAuditDialogVisible = false">取消</el-button><el-button type="primary" @click="confirmShiftAudit">确认</el-button></template>
    </el-dialog>
    ...
    <el-dialog title="出车审批" v-model="vehicleAuditDialogVisible" width="400px">
      <el-form label-width="100px">
        ...
        <el-form-item label="审批备注"><el-input v-model="vehicleAuditRemark" type="textarea" :rows="3" placeholder="请输入备注（可选）" /></el-form-item>
      </el-form>
      <template #footer><el-button @click="vehicleAuditDialogVisible = false">取消</el-button><el-button type="primary" @click="confirmVehicleAudit">确认</el-button></template>
    </el-dialog>
  </div>
</template>
