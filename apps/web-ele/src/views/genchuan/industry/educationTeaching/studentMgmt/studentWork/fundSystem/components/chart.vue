...
const handleCardClick = (cardInfo) => {
  ...
  window.dispatchEvent(new CustomEvent('fund-chart-filter', {
...
const handleBarClick = (params, chartType) => {
  ...
  if (gradeName) {
    window.dispatchEvent(new CustomEvent('fund-chart-filter', {
      detail: {type: 'grade', value: gradeName}
    }));
  } else {
      ...
      <div class="date-range-wrapper">
        <el-date-picker
          ...
          :shortcuts="[
            ...
            { text: '本学期', value: () => { const now = new Date(); const start = new Date(now.getFullYear(), 8, 1); const end = new Date(now.getFullYear(), 11, 31); return [start, end]; } }
          ]"
