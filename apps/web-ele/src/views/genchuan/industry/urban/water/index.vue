...
const activeName = ref('供水管网监测');
const secondShow = ref(true);
const activeSubTab = ref('实时监测');
const activeReportTab = ref('日报');

// 初始化时设置供水管网监测标签页的 secondShow 为 true
const initTabState = () => {
  const monitorTab = tabArray.value.find((v) => v.label === '供水管网监测');
  if (monitorTab) {
...
const handleSubTabChange = (subTab) => {
  if (activeName.value === '供水管网监测') {
    activeSubTab.value = subTab;
  } else if (activeName.value === '供水管网监测报表') {
