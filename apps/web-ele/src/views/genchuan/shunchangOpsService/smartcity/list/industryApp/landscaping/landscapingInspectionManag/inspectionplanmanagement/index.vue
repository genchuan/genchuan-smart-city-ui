...
const getList = async () => {
  ...
  try {
    const data =
      await InspectionPlanManagementApi.getInspectionPlanManagementPage(
        queryParams,
      );
...
const handleDelete = async (id: number) => {
  try {
    // 删除的二次确认
    await confirm('是否确认删除该巡查计划管理数据？', '系统提示');
    // 发起删除
...
const handleExport = async () => {
  try {
    // 导出的二次确认
    await confirm('是否确认导出所有巡查计划管理数据？', '系统提示');
    // 发起导出
    exportLoading.value = true;
    const data =
      await InspectionPlanManagementApi.exportInspectionPlanManagement(
        queryParams,
      );
