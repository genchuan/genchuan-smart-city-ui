...
async function handleExport() {
  const data = await exportPayTransferExcel();
  downloadFileFromBlobPart({ fileName: '支付转账报表.xls', source: data });
}
...
const getTableData = async (pageObj) => {
  ...
  try {
    ...
    const res = await getPayTransferPage(params);
    ...
    console.error('获取支付转账数据失败:', error);
    ElMessage.error('获取支付转账数据失败');
    return dataObj;
  } finally {
...
const handleExecuteSubmit = async () => {
  try {
    await executePayTransfer(executeForm);
    ElMessage.success('执行转账成功');
    executeDialogVisible.value = false;
    handleRefresh();
  } catch {
    ElMessage.error('执行转账失败');
  }
};
...
const handleCancelSubmit = async () => {
  try {
    await cancelPayTransfer(cancelForm);
    ElMessage.success('取消转账成功');
    cancelDialogVisible.value = false;
    handleRefresh();
  } catch {
    ElMessage.error('取消转账失败');
  }
};
