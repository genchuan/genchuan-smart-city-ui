...
import { getLateStudentDetailList } from '#/api/genchuan/industry/educationTeaching/studentMgmt/decisionAnalysis/dormCheckReport/data.js';
...
const columns = [
  ...
  { field: 'lateTime', title: '迟到时间', minWidth: 160 },
  { field: 'lateMinutes', title: '迟到时长(分钟)', minWidth: 120 },
];
...
const fetchData = async (params) => {
  ...
  const res = await getLateStudentDetailList({
  ...
  if (res.code === 200) {
    ...
    console.error('获取迟到学生明细失败', res.msg);
    return { list: [], total: 0 };
  }
};
...
<template>
  <DetailDrawer title="迟到学生明细" class="genchuan-detail-drawer">
