...
import { getShareChargeOrderChart, getShareChargeOrderPage } from '#/api/genchuan/industry/chargePark/orderTrade/orderMgmt/index.js';
...
const fetchOrderChartData = async () => {
  try {
    ...
    if (res.cardData) {
      ...
      state.cardList[2].value = res.cardData.todayLendCount || 0;
    } else {
      state.cardList[0].value = res.todayOrderCount || 0;
      state.cardList[1].value = res.todayRevenue || 0;
      state.cardList[2].value = res.todayLendCount || 0;
    }
    // 如果trendData为空，使用假数据
    state.trendData =
    ...
    state.stationData =
      res.stationData && Array.isArray(res.stationData) && res.stationData.length > 0
        ...
        : [
            ...
            { station: '泉州万达旗舰充电站', count: 18 },
            { station: '仓山万达地下停车场', count: 15 },
          ];
    // 更新折线图
    updateLineChart();
  } catch (error) {
