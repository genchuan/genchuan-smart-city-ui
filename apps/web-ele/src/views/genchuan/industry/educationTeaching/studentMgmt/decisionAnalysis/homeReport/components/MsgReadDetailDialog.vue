...
const mockReadMessages = [
  { id: 10, title: '成绩单发布通知', content: '期末成绩已发布', sendUser: '李老师', sendTime: Date.now() - 172800000, replyTime: Date.now() - 86400000, status: 'read', createTime: Date.now() - 172800000 },
];
...
const columns = [
  ...
  { field: 'replyTime', title: '阅读时间', minWidth: 180, formatter: ({ cellValue }) => cellValue ? new Date(cellValue).toLocaleString() : '-' },
...
<template>
  <DetailDrawer title="已读消息明细" class="genchuan-detail-drawer">
