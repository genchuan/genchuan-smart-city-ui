...
export const BIKE_CHARGE_MONITOR_STATUS_DICT =
  DICT_TYPE.BIKE_CHARGE_MONITOR_STATUS;
export const BIKE_CHARGE_MONITOR_ALARM_STATUS_DICT =
  DICT_TYPE.BIKE_CHARGE_MONITOR_ALARM_STATUS;
export const BIKE_CHARGE_MONITOR_PROCESS_STATUS_DICT =
  DICT_TYPE.BIKE_CHARGE_MONITOR_PROCESS_STATUS;
...
export function getMonitorStatusLabel(value) {
  return getDictLabel(BIKE_CHARGE_MONITOR_STATUS_DICT, value);
}

export function isMonitorStatusLabel(value, label) {
  return isDictLabel(BIKE_CHARGE_MONITOR_STATUS_DICT, value, label);
}

export function getMonitorStatusValueByLabel(label) {
  return getDictValueByLabel(BIKE_CHARGE_MONITOR_STATUS_DICT, label);
}

export function getAlarmStatusLabel(value) {
  return getDictLabel(BIKE_CHARGE_MONITOR_ALARM_STATUS_DICT, value);
}

export function isAlarmStatusLabel(value, label) {
  return isDictLabel(BIKE_CHARGE_MONITOR_ALARM_STATUS_DICT, value, label);
}

export function getProcessStatusLabel(value) {
  return getDictLabel(BIKE_CHARGE_MONITOR_PROCESS_STATUS_DICT, value);
}

export function isProcessStatusLabel(value, label) {
  return isDictLabel(BIKE_CHARGE_MONITOR_PROCESS_STATUS_DICT, value, label);
}
...
export const monitorStatusOptions = getDictOptions(BIKE_CHARGE_MONITOR_STATUS_DICT, 'string');

export const alarmStatusOptions = getDictOptions(BIKE_CHARGE_MONITOR_ALARM_STATUS_DICT, 'string');

export const processStatusOptions = getDictOptions(BIKE_CHARGE_MONITOR_PROCESS_STATUS_DICT, 'string');
...
export function getMonitorStatusTagType(status) {
  ...
  return getDictTagTypeFromDict(
    getDictObj(BIKE_CHARGE_MONITOR_STATUS_DICT, String(status)),
...
export function getAlarmStatusTagType(status) {
  ...
  return getDictTagTypeFromDict(
    getDictObj(BIKE_CHARGE_MONITOR_ALARM_STATUS_DICT, String(status)),
...
export function getProcessStatusTagType(status) {
  ...
  return getDictTagTypeFromDict(
    getDictObj(BIKE_CHARGE_MONITOR_PROCESS_STATUS_DICT, String(status)),
...
export function dataList() {
  return deviceCodes.map((deviceCode, index) => {
    ...
    return {
      ...
      alarmTime,
      alarmRemark: isAbnormal ? '两轮充电设备过载，触发异常告警' : '',
      processStatus,
...
export function normalizeBikeChargeMonitorRow(row) {
...
export function filterMockList(params = {}) {
  const list = dataList().map((item) => normalizeBikeChargeMonitorRow(item));
  ...
  return list.filter((item) => {
    ...
    const matchMonitorStatus = isSameDictValue(
      BIKE_CHARGE_MONITOR_STATUS_DICT,
      item.monitorStatus,
      params.monitorStatus,
    );
    const matchAlarmStatus = isSameDictValue(
      BIKE_CHARGE_MONITOR_ALARM_STATUS_DICT,
      item.alarmStatus,
      params.alarmStatus,
    );
    const matchProcessStatus = isSameDictValue(
      BIKE_CHARGE_MONITOR_PROCESS_STATUS_DICT,
      item.processStatus,
...
export const textObj = {
  excelAllName: '两轮充电监测数据.xlsx',
  total: '两轮充电监测支持状态采集、状态更新、异常标记、状态同步闭环管理',
};
