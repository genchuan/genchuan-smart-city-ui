...
const getTableData = async ({ page }) => {
  ...
  try {
    ...
    const res = await getMoralReportPage(params);
...
async function handleBatchExport() {
  ...
  try {
    ...
    try {
      const data = await exportMoralReport({ ids: checkedIds.value });
...
async function handleExport(row) {
  try {
    const loading = ElLoading.service({ text: '正在导出...' });
    try {
      const data = await exportMoralReport({ ids: [row.id] });
      downloadFileFromBlobPart({ fileName: `${row.timeScale}_德育报表_${formatTimestamp(row.statStartTime)}.xls`, source: data });
      ElMessage.success('导出成功');
    } finally {
      loading.close();
    }
...
const [GenerateForm, generateFormApi] = useVbenForm({
  ...
  handleSubmit: async (values) => {
    ...
    try {
      const res = await createMoralReport(values);
      if (res === true) {
