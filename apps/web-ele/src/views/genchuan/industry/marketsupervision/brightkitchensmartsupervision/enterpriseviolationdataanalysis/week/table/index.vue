...
function generateAlarmData(row) {
  const count = row.weekWarnCount || 0;
  ...
  types.forEach((type) => {
    for (let i = 0; i < Math.min(type.num, 5); i++) {
      list.push({
        ...
        alarmType: type.name,
        alarmTime: `${row.statCycle.split('至')[0].trim()} ${String(Math.trunc(Math.random() * 24)).padStart(2, '0')}:${String(Math.trunc(Math.random() * 60)).padStart(2, '0')}`,
