...
import { getPayTransferChart } from '#/api/genchuan/industry/chargePark/orderTrade/payMgmt/index.js';
import Card from '#/components/stats/card.vue';

const state = reactive({
  cardList: [
    { title: '今日转账', value: 0, color: '#FF6B6B' },
    { title: '累计转账', value: 0, color: '#4ECDC4' },
    { title: '待处理', value: 0, color: '#13ce66' },
  ],
  trendData: [],
});
...
const fetchOrderChartData = async () => {
  try {
    const res = await getPayTransferChart();
    state.cardList[0].value = res.todayTransferCount || 0;
    // 累计转账通过趋势数据计算
    state.cardList[1].value = res.trendData?.reduce((sum, item) => sum + item.count, 0) || 0;
    ...
    console.error('获取支付转账图表数据失败:', error);
    // 接口调用失败时使用假数据
