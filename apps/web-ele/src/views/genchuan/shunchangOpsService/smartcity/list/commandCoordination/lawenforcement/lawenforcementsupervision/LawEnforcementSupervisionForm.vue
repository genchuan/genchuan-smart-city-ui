...
const submitForm = async () => {
  ...
  try {
    ...
    if (formType.value === 'create') {
      await LawEnforcementSupervisionApi.createLawEnforcementSupervision(
        data,
      );
      ElMessage.success('新增成功');
    } else {
      await LawEnforcementSupervisionApi.updateLawEnforcementSupervision(
        data,
      );
  ...
  <ElDialog :title="dialogTitle" v-model="dialogVisible" width="600px" append-to-body>
    ...
    >
      <ElFormItem label="监督编号" prop="supervisionId">
        <ElInput v-model="formData.supervisionId" placeholder="请输入监督编号" />
      </ElFormItem>
      <ElFormItem label="执法事件编号" prop="eventNumber">
        <ElInput v-model="formData.eventNumber" placeholder="请输入执法事件编号" />
      </ElFormItem>
      ...
      <ElFormItem label="群众满意度" prop="satisfactionInvolved">
        <ElInput v-model="formData.satisfactionInvolved" placeholder="请输入当事人满意度" />
      </ElFormItem>
      ...
      <ElFormItem label="监督问题描述" prop="violationDescription">
        <ElInput v-model="formData.violationDescription" placeholder="请输入违规行为描述" />
      </ElFormItem>
    </ElForm>
    <template #footer>
