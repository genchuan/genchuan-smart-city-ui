...
export const ConsumableManagementApi = {
  ...
  getConsumableManagementPage: async (params: any) => {
    return await requestClient.get(`/waterdetection/consumable-management/page`, { params });
  },

  // 查询耗材库存与更换管理详情
  getConsumableManagement: async (id: number) => {
    return await requestClient.get(`/waterdetection/consumable-management/get`, { params: { id } });
  },

  // 新增耗材库存与更换管理
  createConsumableManagement: async (data: ConsumableManagementVO) => {
    return await requestClient.post(`/waterdetection/consumable-management/create`, data);
  },

  // 修改耗材库存与更换管理
  updateConsumableManagement: async (data: ConsumableManagementVO) => {
    return await requestClient.put(`/waterdetection/consumable-management/update`, data);
  },

  // 删除耗材库存与更换管理
  deleteConsumableManagement: async (id: number) => {
    return await requestClient.delete(`/waterdetection/consumable-management/delete`, { params: { id } });
  },

  // 导出耗材库存与更换管理 Excel
  exportConsumableManagement: async (params) => {
    return await requestClient.download(`/waterdetection/consumable-management/export-excel`, params);
  },
};
