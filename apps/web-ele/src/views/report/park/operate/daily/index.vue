...
<template>
  <div class="park-lot-table-new">
    ...
    <ReportDetailDrawer
      ...
      :title="`运营详情 - ${reportObj.detailObj.areaName} ${reportObj.detailObj.parkType}`"
    />
    <Drawer title="搜索">
    ...
    <Grid>
      ...
      <template #alarmTotal="{ row }">
        <el-tag
          :type="row.alarmTotal > 10 ? 'danger' : row.alarmTotal > 5 ? 'warning' : 'primary'"
          size="small"
        >
          {{ row.alarmTotal }}
        </el-tag>
      </template>
      <template #faultDeviceCount="{ row }">
        <el-tag
          :type="row.faultDeviceCount > 3 ? 'danger' : row.faultDeviceCount > 1 ? 'warning' : 'success'"
          size="small"
        >
          {{ row.faultDeviceCount }}
        </el-tag>
      </template>
      ...
      <template #bottom>
        <div class="common-total" @click="changeTotalShow">
          ...
          <span> 本页统计：数据量{{ reportObj.list.length }};
            总入场车次: {{ reportObj.list.reduce((sum, v) => sum + v.totalEntry, 0) }};
