...
// 引入封装后的详情抽屉组件
import BerthDetailDrawer from '#/views/report/park/berth/occupancy/detail.vue';
...
const handleOpenDetail = (row) => {
  ...
  // 通过ref调用组件的open方法
  berthDetailDrawerRef.value.open();
  console.log(row);
};
...
const handleViewMap = (row) => {
  // 这里可以跳转到地图页面，传递区域参数
  window.open(`/map/heatmap?area=${row.areaName}&type=berth`, '_blank');
};
...
// 定义组件ref，用于调用组件方法
const berthDetailDrawerRef = ref(null);
</script>
...
<template>
  <div class="park-lot-table-new">
    ...
    <!-- 使用封装后的详情抽屉组件 -->
    <BerthDetailDrawer
      ref="berthDetailDrawerRef"
      :detail-obj="reportObj.detailObj"
      :title="`泊位占用详情 - ${reportObj.detailObj.areaName} ${reportObj.detailObj.parkType}`"
    />
    <Drawer title="搜索">
    ...
    <Grid>
      ...
      <template #freeBerth="{ row }">
        <el-tag type="success" size="small">
          {{ row.freeBerth }}
        </el-tag>
      </template>
      <template #avgOccupationRate="{ row }">
        <el-tag
          :type="parseFloat(row.avgOccupationRate) > 80 ? 'danger' :
                 parseFloat(row.avgOccupationRate) > 60 ? 'warning' : 'success'"
          size="small"
        >
          {{ row.avgOccupationRate }}
        </el-tag>
      </template>
      <template #overtimeOccupationCount="{ row }">
        <el-tag type="warning" size="small">
          {{ row.overtimeOccupationCount }}
        </el-tag>
      </template>
      ...
      <template #bottom>
        <div class="common-total" @click="changeTotalShow">
          ...
          <span> 本页统计：数据量{{ reportObj.list.length }};
            总泊位数: {{ reportObj.list.reduce((sum, v) => sum + v.totalBerth, 0) }};
            占用泊位数: {{ reportObj.list.reduce((sum, v) => sum + v.occupiedBerth, 0) }};
            超时长占用: {{ reportObj.list.reduce((sum, v) => sum + v.overtimeOccupationCount, 0) }};
            </span>
        </div>
