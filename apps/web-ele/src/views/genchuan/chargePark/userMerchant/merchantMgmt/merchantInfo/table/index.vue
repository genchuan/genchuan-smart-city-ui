...
import type { MerchantInfoDetailVO } from '#/api/genchuan/chargePark/userMerchant/merchantMgmt/merchantInfo';
...
import { MerchantInfoApi } from '#/api/genchuan/chargePark/userMerchant/merchantMgmt/merchantInfo';
import DetailDrawer from '#/genchuan-components/DetailDrawer.vue';

import {
  buildMerchantInfoQueryParams,
  buildMerchantRowFromApi,
  formatAccountLogs,
  formatAuditLogs,
  formatCouponRecords,
  formatLinkRecords,
  formatRechargeRecords,
  getOperatorDetail,
  maskPhone,
  detailFields as merchantInfoDetailFields,
  textObj,
  useCreateSchema,
  useEditSchema,
  useGridColumns,
...
const detailCache = new Map<number, MerchantInfoDetailVO>();
...
const formSource = ref<MerchantInfoDetailVO>();
...
async function fetchMerchantDetail(
  ...
  if (cachedDetail) {
    return {
      row: buildMerchantRowFromApi(cachedDetail, row),
      source: cachedDetail,
    };
  }

  const loadingInstance = ElLoading.service({
    target: '.merchant-info-table',
    text: '加载中...',
  });

  try {
    const data = await MerchantInfoApi.getMerchantInfo(row.id);
    detailCache.set(row.id, data);

    return {
      row: buildMerchantRowFromApi(data, row),
      source: data,
    };
  } catch (error) {
    ElMessage.error(errorMessage);
    console.error('[merchantInfo] load detail failed:', error);
    return null;
  } finally {
...
async function queryMerchantInfoPage(
  ...
  const result = await MerchantInfoApi.getMerchantInfoPage({
    ...
    ...buildMerchantInfoQueryParams(formValues, {
  ...
  return {
    list: list.map((item) => buildMerchantRowFromApi(item)),
  ...
  async onConfirm() {
    ...
    try {
      if (formMode.value === 'create') {
        await MerchantInfoApi.createMerchantInfo({
        ...
        await MerchantInfoApi.updateMerchantInfo({
          address: values.address || '',
          contact: values.contact || '',
          id: formData.value.id,
          merchantType:
            formSource.value?.merchantType || formData.value.merchantType,
      ...
      console.error('[merchantInfo] save failed:', error);
    } finally {
      loadingInstance.close();
    }
  },
...
async function handleExport() {
  ...
  try {
    ...
    console.error('[merchantInfo] export failed:', error);
  }
}
...
/** 打开编辑抽屉 */
async function handleEdit(row: MerchantInfoRow) {
...
/** 审核通过商户 */
async function handleApprove(row: MerchantInfoRow) {
  const loadingInstance = ElLoading.service({
    target: '.merchant-info-table',
    text: '审核中...',
  });
  ...
  try {
    ...
    console.error('[merchantInfo] approve failed:', error);
  } finally {
    loadingInstance.close();
  }
}
...
async function handleConfirmReject() {
  ...
  try {
    ...
    console.error('[merchantInfo] reject failed:', error);
  } finally {
    loadingInstance.close();
  }
}
...
async function handleToggleStatus(
  ...
  try {
    await (status === '正常'
      ? MerchantInfoApi.enableMerchantInfo({
          ids: [row.id],
        })
      : MerchantInfoApi.disableMerchantInfo({
          ids: [row.id],
        }));
    ...
    console.error('[merchantInfo] toggle status failed:', error);
  } finally {
    loadingInstance.close();
  }
}
...
/** 打开详情抽屉 */
async function handleDetail(row: MerchantInfoRow) {
  const detail = await fetchMerchantDetail(row);
...
async function handleOpenOperator(
  ...
  const detail = await fetchMerchantDetail(row, '加载操作人员详情失败');
...
/** 打开账户明细弹窗 */
async function handleOpenAccount(row: MerchantInfoRow) {
  const detail = await fetchMerchantDetail(row, '加载账户明细失败');
...
/** 导入商户数据 */
async function handleImportMerchants() {
  ...
  try {
    await MerchantInfoApi.importMerchantInfo(file);
    ...
    console.error('[merchantInfo] import failed:', error);
  } finally {
    loadingInstance.close();
  }
}
...
<template>
  <div class="merchant-info-table">
    ...
    <DetailDrawer
      ref="detailDrawerRef"
      :data="detailData"
      :fields="merchantInfoDetailFields"
