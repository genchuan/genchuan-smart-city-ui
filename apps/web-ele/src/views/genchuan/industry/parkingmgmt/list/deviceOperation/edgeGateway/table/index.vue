...
const getTitle = computed(() => {
  return formData.value?.gatewayId ? textObj.editText : textObj.addText;
});
...
async function handleDeleteBatch() {
  ...
  try {
    dataObj.apilist = dataObj.apilist.filter(
      (v) => !checkedIds.value.includes(v.gatewayId),
    );
    checkedIds.value = [];
...
function handleRowCheckboxChange({ records }) {
  checkedIds.value = records.map((item) => item.gatewayId);
}
...
<!--   详情抽屉-->
    ...
    <Grid>
      ...
      <template #gatewayStatusName="{ row }">
        <el-tag :type="getStatusType(row.gatewayStatusName)">
          {{ row.gatewayStatusName }}
        </el-tag>
      </template>
      ...
      <template #bottom>
        <div class="common-total" @click="changeTotalShow">
          ...
          <span> 本页统计：边缘网关数量: {{ dataObj.total }} </span>
        </div>
