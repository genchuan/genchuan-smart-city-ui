...
import {
  ...
  fetchWarningData,
  handleEvent
} from '#/api/genchuan/industry/urbanmanagement/overview/GlobalSituationOverview.js';
...
const getIndicatorTrend = async (indexId) => {
  try {
    indicatorTrendData.value = await fetchIndicatorTrend(indexId);
  } catch (error) {
...
const simulateNewWarning = () => {
  const warningItems = mapItems.value.filter(item => {
    ...
    return item.data_type === '2' && item.status === '0';
