  ...
  <Page auto-content-height>
    ...
    <template v-if="viewMode === 'card'">
      <el-row v-loading="loading" :gutter="16">
        <el-col
          ...
          class="card-col"
        >
          <el-card class="property-card" shadow="hover" :body-style="{ padding: '0' }">
            <!-- 添加渐变背景层 -->
            ...
            <div class="card-content">
              ...
              <div class="card-header">
                <div class="header-icon">
                  <IconifyIcon class="icon" icon="ep:cpu" />
                </div>
                <div class="header-title">{{ item.name }}</div>
                <el-tag type="primary" size="small" class="identifier-tag">
                  {{ item.identifier }}
                </el-tag>
                <el-tag size="small" class="data-type-tag">
                  {{ item.dataType }}
                </el-tag>
                <!-- 数据图标 - 可点击 -->
                <div
                ...
                >
                  <IconifyIcon class="icon-primary" icon="ep:data-line" />
                </div>
              </div>
    ...
    <Grid v-show="viewMode === 'list'">
      ...
      <template #actions="{ row }">
        <el-button link type="primary" @click="openHistory(props.deviceId, row.identifier, row.dataType)">
          查看数据
        </el-button>
      </template>
    </Grid>
