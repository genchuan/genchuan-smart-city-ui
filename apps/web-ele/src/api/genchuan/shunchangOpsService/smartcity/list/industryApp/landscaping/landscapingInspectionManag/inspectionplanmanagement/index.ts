import { requestClient } from '#/api/request';

// 巡查计划管理 VO
export type InspectionPlanManagementVO = {
  arrangementOfPatrolPersonnel: string; // 巡查人员安排
  emergencyResponsePlan: string; // 应急处置预案
  id: number; // 主键
  inspectionContent: string; // 巡查内容
  inspectionCycle: string; // 巡查周期
  inspectionStandards: string; // 巡查标准
  notes: string; // 备注
  patrolArea: string; // 巡查区域
  planName: string; // 计划名称
  plannedEndTime: string; // 计划结束时间
  scheduledStartTime: string; // 计划开始时间
};

// 巡查计划管理 API
export const InspectionPlanManagementApi = {
  // 查询巡查计划管理分页
  getInspectionPlanManagementPage: async (params: any) => {
    return await requestClient.get(
      `/smartcity/inspection-plan-management/page`,
      { params },
    );
  },

  // 查询巡查计划管理详情
  getInspectionPlanManagement: async (id: number) => {
    return await requestClient.get(
      `/smartcity/inspection-plan-management/get`,
      { params: { id } },
    );
  },

  // 新增巡查计划管理
  createInspectionPlanManagement: async (data: any) => {
    return await requestClient.post(
      `/smartcity/inspection-plan-management/create`,
      data,
    );
  },

  // 修改巡查计划管理
  updateInspectionPlanManagement: async (data: any) => {
    return await requestClient.put(
      `/smartcity/inspection-plan-management/update`,
      data,
    );
  },

  // 删除巡查计划管理
  deleteInspectionPlanManagement: async (id: number) => {
    return await requestClient.delete(
      `/smartcity/inspection-plan-management/delete`,
      { params: { id } },
    );
  },

  // 导出巡查计划管理 Excel
  exportInspectionPlanManagement: async (params) => {
    return await requestClient.download(
      `/smartcity/inspection-plan-management/export-excel`,
      params,
    );
  },
};
