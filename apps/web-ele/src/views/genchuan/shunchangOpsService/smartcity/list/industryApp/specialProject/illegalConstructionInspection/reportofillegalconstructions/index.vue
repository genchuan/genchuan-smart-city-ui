...
const getList = async () => {
  ...
  try {
    const data =
      await ReportOfIllegalConstructionsApi.getReportOfIllegalConstructionsPage(
        queryParams,
      );
...
const handleExport = async () => {
  try {
    // 导出的二次确认
    await confirm('是否确认导出所有违建上报数据？', '系统提示');
    // 发起导出
    exportLoading.value = true;
    const data =
      await ReportOfIllegalConstructionsApi.exportReportOfIllegalConstructions(
        queryParams,
      );
