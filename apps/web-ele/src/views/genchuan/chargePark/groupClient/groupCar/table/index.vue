...
import type { GroupCarRow } from '../data';
...
const emit = defineEmits<{
  'update:cars': [value: GroupCarRow[]];
}>();

const cars = computed({
  get: () => props.cars,
  set: (value: GroupCarRow[]) => {
...
const currentGroupProfile = ref<Record<string, string> | null>(null);
...
const [FormDrawer, formDrawerApi] = useVbenDrawer({
  ...
  async onConfirm() {
    ...
    if (formMode.value === 'create') {
      ...
      const group = groupOptions.find((item) => item.value === values.groupId);
      ...
      cars.value = [
        {
          ...
          groupName: group?.label || `集团${values.groupId}`,
...
function formatBindingLogs(logs: GroupCarRow['bindingLogs']) {
  if (!logs.length) {
    return '暂无绑定操作日志';
  }
...
/** 匹配筛选条件 */
function matchCar(item: GroupCarRow, formValues: Record<string, any>) {
  const groupMatch = !formValues.groupId || item.groupId === formValues.groupId;
  const plateMatch =
...
async function handleExport() {
  ...
  exportToExcel(buildExportRows(list), '集团车辆', '集团车辆列表.xlsx');
}
...
/** 打开编辑抽屉 */
function handleEdit(row: GroupCarRow) {
  formMode.value = 'edit';
...
/** 打开详情抽屉 */
function handleDetail(row: GroupCarRow) {
  currentRow.value = row;
  detailDrawerRef.value?.open();
}

/** 打开集团详情弹窗 */
function handleOpenGroup(row: GroupCarRow) {
  currentGroupProfile.value =
    groupProfileMap[row.groupId as keyof typeof groupProfileMap];
...
/** 审核通过车辆 */
function handleApprove(row: GroupCarRow) {
  cars.value = cars.value.map((item) => {
    if (item.id !== row.id) {
      return item;
    }
...
/** 重新绑定车辆 */
function handleRebind(row: GroupCarRow) {
  cars.value = cars.value.map((item) => {
    if (item.id !== row.id) {
      return item;
    }
...
function handleImportCars() {
  ...
  cars.value = [
    {
      ...
      groupId: 1,
      groupName: '泉州智联企业集团',
      plateNo: `闽C77${String(id).padStart(3, '0').slice(-3)}`,
      plateColor: '蓝牌',
      carType: '小型车',
      bindTime: now,
      status: '待审核',
  ...
  ElMessage.success('导入完成，已模拟新增 1 条集团车辆数据');
  handleRefresh();
}
</script>
    ...
    <ElDialog v-model="groupDialogVisible" title="集团详情" width="520px">
      <ElDescriptions v-if="currentGroupProfile" :column="1" border>
        ...
        <ElDescriptionsItem label="联系手机号">
          {{ currentGroupProfile.phone }}
        </ElDescriptionsItem>
        <ElDescriptionsItem label="集团类型">
          {{ currentGroupProfile.type }}
        </ElDescriptionsItem>
