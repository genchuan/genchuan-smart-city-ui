...
import { getBikeChargeMonitorChart } from '#/api/genchuan/industry/chargePark/inspectOp/deviceMonitor/bikeChargeMonitor';
...
async function fetchChartData() {
  try {
    const response = await getBikeChargeMonitorChart();
    normalizeChartData(response);
  } catch (error) {
    console.error('获取两轮充电监测看板失败，使用静态数据:', error);
...
<template>
  <div class="bike-charge-monitor-visualization">
...
<style scoped>
.bike-charge-monitor-visualization {
  display: flex;
  flex-wrap: nowrap;
  gap: 20px;
  width: 100%;
