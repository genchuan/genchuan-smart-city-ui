...
} from '#/api/genchuan/chargePark/vehiclePass/leaveMgmt/abnormalLeave';
...
async function handleExport() {
  if (USE_REAL_API) {
    try {
      await exportAbnormalLeave(dataObj.searchParams);
...
async function handleDelete(row) {
  const loadingInstance = ElLoading.service({
    text: $t('ui.actionMessage.deleting', [row.carNo]),
  });
  try {
    dataObj.apilist = dataObj.apilist.filter((v) => v.id !== row.id);
    ElMessage.success($t('ui.actionMessage.deleteSuccess', [row.carNo]));
    handleRefresh();
  } finally {
