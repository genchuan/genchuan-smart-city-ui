...
import { getEmployReportChart } from '#/api/genchuan/industry/educationTeaching/studentMgmt/decisionAnalysis/employReport/data.js';
...
const chartOptions = [
  { value: 'pie', label: '合作企业类型分布' },
  { value: 'bar', label: '各系部合作企业数' },
  { value: 'line', label: '就业率趋势' },
];
...
const fetchChartData = async (customParams = null) => {
  ...
  try {
    ...
    const res = await getEmployReportChart(params);
    ...
    if (data) {
      ...
      if (data.barData && data.barData.length) {
        ...
        barSeriesData.value = [{ name: '合作企业数', data: data.barData.map(item => item.enterpriseNum) }];
      }
      if (data.lineData) {
  ...
  <div v-loading="loading" class="chart-box">
    ...
    <div class="chart-area">
      ...
      <div v-else-if="activeChart === 'bar'">
        <Bar :x-data="barXAxis" :series-data="barSeriesData" :title="currentChartTitle" y-name="企业数" @barClick="handleBarClick" />
      </div>
      <div v-else-if="activeChart === 'line'">
        <LineChart :x-data="lineXAxis" :series-data="lineSeriesData" :title="currentChartTitle" y-name="就业率(%)" @lineClick="handleLineClick" />
      </div>
    </div>
  </div>
</template>
