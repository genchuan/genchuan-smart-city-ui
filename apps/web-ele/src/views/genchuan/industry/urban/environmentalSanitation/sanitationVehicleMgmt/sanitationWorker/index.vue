...
<template>
  <div class="park-lot-table-new">
    ...
    <Grid>
      ...
      <template #status="{ row }">
        <el-text @click="handleOpenStatusFilter(row.status)" type="primary">{{ row.status }}</el-text>
      </template>
      ...
      <template #bottom>
        <div class="common-total" @click="dataObj.totalShow = !dataObj.totalShow">
          ...
          <span>本页统计：任务总数{{
              ...
              dataObj.list.filter(v => v.status === '待排班').length
            }}; 待考勤{{
              dataObj.list.filter(v => v.status === '待考勤').length
            }}; 考核待审核{{
              dataObj.list.filter(v => v.status === '考核待审核').length
