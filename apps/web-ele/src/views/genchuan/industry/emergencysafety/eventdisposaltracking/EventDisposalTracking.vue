...
import {
  ...
  saveFollowUpRecord,
  updateEventProgress
} from '#/api/genchuan/industry/emergencysafety/SubdomainScenarioTopics.js';
...
const fetchTrackingEvents = async () => {
  try {
    ...
    eventList.value = await fetchTrackingEventList(params);
  } catch (error) {
...
const fetchEventTimelineData = async (eventId) => {
  try {
    eventTimeline.value = await fetchEventTimeline(eventId);
  } catch (error) {
...
const fetchFollowUpRecordsData = async (eventId) => {
  try {
    followUpRecords.value = await fetchFollowUpRecords(eventId);
  } catch (error) {
