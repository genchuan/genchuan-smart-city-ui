            ...
            resolve([
              {
                ...
                merchantId: 'M2025001',
                progressFeedback: '企业提出的停车收费备案诉求已受理，材料审核完成，备案登记办结',
              ...
              },
              {
                ...
                merchantId: 'M2025002',
                progressFeedback: '已完成停车场消防设施联合检查，发现3处隐患，企业已全部整改完毕，复检合格',
              ...
              },
              {
                ...
                merchantId: 'M2025003',
                progressFeedback: '应急消防器材采购合同已签订，物资已到货80%，剩余物资预计今日送达并完成验收',
              ...
              },
              {
                ...
                merchantId: 'M2025004',
                progressFeedback: '企业提出的停车场出入口扩建诉求已完成现场勘查，规划方案正在审批中',
              ...
              },
              {
                ...
                merchantId: 'M2025005',
                progressFeedback: '企业申请的园区停车场规划许可已审核通过，相关施工手续已同步办结发放',
                satisfactionEvaluation: '满意'
              },
              {
                ...
                merchantId: 'M2025006',
                progressFeedback: '完成漳州高速口停车场安全隐患排查，整改占道经营、消防通道堵塞等问题2项',
                satisfactionEvaluation: '非常满意'
              },
              {
                ...
                merchantId: 'M2025007',
                progressFeedback: '漳州开发区应急物资储备库补货完成，防汛、防疫类物资备货量达需求标准120%',
                satisfactionEvaluation: '满意'
              },
              {
                ...
                merchantId: 'M2025008',
                progressFeedback: '企业反映的停车场收费公示不规范问题，已完成现场核查并督促整改到位',
                satisfactionEvaluation: '基本满意'
              },
              {
                ...
                merchantId: 'M2025009',
                progressFeedback: '完成漳州景区配套停车场安全检查，整改观光车停放、标识不清等安全问题3项',
                satisfactionEvaluation: '一般'
              }
            ]);
          }, 500);
    ...
    return requestClient
      ...
      .then((response) => {
        if (
          ...
          response.totalCount && response.responseRate && response.satisfactionRate
        ) {
          return response;
        }
    ...
    return requestClient
      ...
      .then((response) => {
        if (
          ...
          typeof response === 'object' &&
          !Array.isArray(response) &&
          response.legend && Array.isArray(response.legend) &&
...
export const fetchHighFrequencyCoopTop10 = (params = {}) => {
  try {
    return requestClient
      ...
      .catch((error) => {
        console.log('高频协同事项TOP10接口调用失败-使用模拟数据兜底', error.message);
        return new Promise((resolve) => {
          setTimeout(() => {
            resolve([
              { rank:1, coopStatId: 'CST2025001', coopType: 'high', coopCount: 89, top10CoopItem: '企业停车收费备案诉求响应' },
              { rank:2, coopStatId: 'CST2025002', coopType: 'high', coopCount: 76, top10CoopItem: '停车场消防设施联合安全检查' },
              { rank:3, coopStatId: 'CST2025003', coopType: 'medium', coopCount: 65, top10CoopItem: '应急消防器材采购供应对接' },
              { rank:4, coopStatId: 'CST2025004', coopType: 'medium', coopCount: 58, top10CoopItem: '停车场出入口扩建规划审批' },
              { rank:5, coopStatId: 'CST2025005', coopType: 'medium', coopCount: 49, top10CoopItem: '园区停车场规划许可办理' },
              { rank:6, coopStatId: 'CST2025006', coopType: 'low', coopCount: 42, top10CoopItem: '高速口停车场占道经营整改' },
              { rank:7, coopStatId: 'CST2025007', coopType: 'low', coopCount: 37, top10CoopItem: '应急物资储备库补货验收' },
              { rank:8, coopStatId: 'CST2025008', coopType: 'low', coopCount: 31, top10CoopItem: '停车场收费公示规范整改' },
              { rank:9, coopStatId: 'CST2025009', coopType: 'medium', coopCount: 26, top10CoopItem: '景区配套停车场安全检查' },
              { rank:10, coopStatId: 'CST2025010', coopType: 'high', coopCount: 22, top10CoopItem: '停车场监控设备升级改造' }
            ]);
          }, 500);
        });
      });
  ...
  try {
    return requestClient
      ...
      .then(res => {
        ...
        console.log('协同核心指标接口异常，使用兜底数据', err.message);
        return Promise.resolve({ totalCoopCount: 896, avgHandleCycle: 3.2, finishRate: 0.925, highPriorityRate: 0.46 });
      });
  } catch (error) {
    console.error('fetchCoopCoreIndicators 异常:', error);
    return Promise.resolve({ totalCoopCount: 0, avgHandleCycle: 0, finishRate: 0, highPriorityRate: 0 });
  }
};
...
export const fetchCoopTypeCount = (params = {}) => {
  try {
    return requestClient
      ...
      .catch(err => {
        console.log('类型协同数接口异常，使用兜底数据', err.message);
        return Promise.resolve({
...
export const fetchCoopAreaCount = (params = {}) => {
  try {
    return requestClient
      ...
      .then(res => res || { xAxis: [], series: [] })
      .catch(err => {
        console.log('区域协同数接口异常，使用兜底数据', err.message);
        return Promise.resolve({
...
export const fetchCoopIndustryRatio = (params = {}) => {
  try {
    return requestClient
      ...
      .catch(err => {
        console.log('行业协同占比接口异常，使用兜底数据', err.message);
        return Promise.resolve({
          legend: ['停车服务', '消防安防', '规划审批', '物资供应', '运营管理'],
          series: [{ name: '行业协同占比', data: [{value:35, name:'停车服务'},{value:25, name:'消防安防'},{value:20, name:'规划审批'},{value:12, name:'物资供应'},{value:8, name:'运营管理'}]}]
        });
      });
  } catch (error) {
...
export const fetchCoopAreaRatio = (params = {}) => {
  try {
    return requestClient
      ...
      .then(res => res || { legend: [], series: [] })
      .catch(err => {
        console.log('区域协同占比接口异常，使用兜底数据', err.message);
        return Promise.resolve({
          legend: ['主城区', '高新区', '经开区', '文旅区', '周边区县'],
          series: [{ name: '区域协同占比', data: [{value:32, name:'主城区'},{value:24, name:'高新区'},{value:18, name:'经开区'},{value:15, name:'文旅区'},{value:11, name:'周边区县'}]}]
        });
      });
  } catch (error) {
...
export const fetchCoopTrendData = (params = {}) => {
  try {
    return requestClient
      ...
      .then(res => res || { xAxis: [], series: [] })
      .catch(err => {
        console.log('协同趋势折线图接口异常，使用兜底数据', err.message);
        return Promise.resolve({
          xAxis: ['01-01','01-02','01-03','01-04','01-05','01-06','01-07','01-08','01-09','01-10'],
          series: [{ name: '协同事件数', data: [68, 75, 59, 82, 96, 78, 92, 85, 102, 98] }]
        });
      });
  } catch (error) {
    ...
    return requestClient
      ...
      .catch((error) => {
        ...
        return new Promise((resolve) => {
          setTimeout(() => {
            resolve([
              { coopEfficiencyId: 'CE20250110001', coopType: 'high', responseDuration: 2.5, disposalDuration: 8.2, collaborationCost: 1260, effectAchievementRate: 0.98, problemRecurrenceRate: 0.02 },
              { coopEfficiencyId: 'CE20250110002', coopType: 'high', responseDuration: 1.8, disposalDuration: 6.5, collaborationCost: 980, effectAchievementRate: 0.96, problemRecurrenceRate: 0.03 },
              { coopEfficiencyId: 'CE20250110003', coopType: 'medium', responseDuration: 4.2, disposalDuration: 12.6, collaborationCost: 850, effectAchievementRate: 0.92, problemRecurrenceRate: 0.05 },
              { coopEfficiencyId: 'CE20250110004', coopType: 'medium', responseDuration: 3.6, disposalDuration: 10.8, collaborationCost: 720, effectAchievementRate: 0.90, problemRecurrenceRate: 0.06 },
              { coopEfficiencyId: 'CE20250110005', coopType: 'medium', responseDuration: 5.1, disposalDuration: 14.2, collaborationCost: 650, effectAchievementRate: 0.88, problemRecurrenceRate: 0.07 },
              { coopEfficiencyId: 'CE20250110006', coopType: 'low', responseDuration: 8.5, disposalDuration: 20.5, collaborationCost: 420, effectAchievementRate: 0.85, problemRecurrenceRate: 0.10 },
              { coopEfficiencyId: 'CE20250110007', coopType: 'low', responseDuration: 7.2, disposalDuration: 18.6, collaborationCost: 380, effectAchievementRate: 0.82, problemRecurrenceRate: 0.12 },
              { coopEfficiencyId: 'CE20250110008', coopType: 'high', responseDuration: 3.0, disposalDuration: 9.5, collaborationCost: 1150, effectAchievementRate: 0.95, problemRecurrenceRate: 0.04 },
              { coopEfficiencyId: 'CE20250110009', coopType: 'medium', responseDuration: 4.8, disposalDuration: 13.8, collaborationCost: 780, effectAchievementRate: 0.89, problemRecurrenceRate: 0.08 },
              { coopEfficiencyId: 'CE20250110010', coopType: 'low', responseDuration: 9.2, disposalDuration: 22.3, collaborationCost: 350, effectAchievementRate: 0.80, problemRecurrenceRate: 0.15 }
            ]);
          }, 500);
        });
      });
    ...
    return requestClient
      ...
      .then((response) => {
        if (
          ...
          typeof response === 'object' &&
          !Array.isArray(response) &&
          response.avgResponseDuration && response.avgDisposalDuration && response.avgEffectAchievementRate
        ) {
          return response;
        }
...
export const fetchCoopEfficiencyTypeCount = (params = {}) => {
  try {
      ...
      .catch((error) => {
        ...
        return new Promise((resolve) => {
          setTimeout(() => {
            resolve({
              xAxis: ['高优先级', '中优先级', '低优先级', '紧急协同', '常规协同'],
              series: [{ name: '平均处置时长(小时)', data: [7.8, 12.5, 20.3, 5.6, 14.2] }]
            });
          }, 500);
        });
      });
    ...
    return Promise.resolve({ xAxis: [], series: [{ name: '平均处置时长(小时)', data: [] }] });
  }
};
...
export const fetchCoopEfficiencyAreaCount = (params = {}) => {
  try {
      ...
      .catch((error) => {
        ...
        return new Promise((resolve) => {
          setTimeout(() => {
            resolve({
              xAxis: ['主城区', '高新区', '经开区', '文旅区', '周边区县'],
              series: [{ name: '平均响应时长(小时)', data: [3.2, 4.1, 3.8, 5.5, 6.8] }]
            });
          }, 500);
        });
      });
    ...
    return Promise.resolve({ xAxis: [], series: [{ name: '平均响应时长(小时)', data: [] }] });
  }
};
...
export const fetchCoopEfficiencyRecurrenceRatio = (params = {}) => {
  try {
    return requestClient
      ...
      .catch((error) => {
        ...
        return new Promise((resolve) => {
          setTimeout(() => {
            resolve({
              legend: ['无复发', '低复发(≤5%)', '中复发(5%-10%)', '高复发(>10%)'],
    ...
    return Promise.resolve({ legend: [], series: [{ name: '问题复发率占比', data: [] }] });
  }
};
...
export const fetchCoopEfficiencyTrendData = (params = {}) => {
  try {
    return requestClient
      ...
      .then(res => res || { xAxis: [], series: [] })
      .catch(err => {
        console.log('协同效率趋势折线图接口异常，使用兜底数据', err.message);
        return Promise.resolve({
          xAxis: ['01-01','01-02','01-03','01-04','01-05','01-06','01-07','01-08','01-09','01-10'],
          series: [{ name: '综合效率评分', data: [82, 85, 83, 86, 88, 87, 90, 89, 92, 93] }]
        });
      });
  } catch (error) {
    ...
    return requestClient
      ...
      .catch((error) => {
        ...
        return new Promise((resolve) => {
          setTimeout(() => {
            resolve([
              { crossRegionCoopId: 'CRC20250112001', coopTask: '跨区泊位调度', launchAreaCode: '漳州市主城区', cooperateAreaCode: '漳州市高新区', progressNode: '调度完成，泊位已分配', coopCompleteRate: 100 },
              { crossRegionCoopId: 'CRC20250112002', coopTask: '跨区设备抢修', launchAreaCode: '漳州市经开区', cooperateAreaCode: '漳州市文旅区', progressNode: '设备维修完毕，验收合格', coopCompleteRate: 100 },
              { crossRegionCoopId: 'CRC20250112003', coopTask: '跨区逃费追缴', launchAreaCode: '漳州市高新区', cooperateAreaCode: '漳州市主城区', progressNode: '追缴完成，费用已到账', coopCompleteRate: 100 },
              { crossRegionCoopId: 'CRC20250112004', coopTask: '跨区泊位调度', launchAreaCode: '漳州市文旅区', cooperateAreaCode: '漳州市周边区县', progressNode: '调度中，泊位协调中', coopCompleteRate: 75 },
              { crossRegionCoopId: 'CRC20250112005', coopTask: '跨区设备抢修', launchAreaCode: '漳州市主城区', cooperateAreaCode: '漳州市经开区', progressNode: '配件到位，开始维修', coopCompleteRate: 60 },
              { crossRegionCoopId: 'CRC20250112006', coopTask: '跨区逃费追缴', launchAreaCode: '漳州市周边区县', cooperateAreaCode: '漳州市高新区', progressNode: '信息核实中，待追缴', coopCompleteRate: 30 },
              { crossRegionCoopId: 'CRC20250112007', coopTask: '跨区泊位调度', launchAreaCode: '漳州市经开区', cooperateAreaCode: '漳州市文旅区', progressNode: '调度完成，高峰期保障到位', coopCompleteRate: 100 },
              { crossRegionCoopId: 'CRC20250112008', coopTask: '跨区设备抢修', launchAreaCode: '漳州市高新区', cooperateAreaCode: '漳州市周边区县', progressNode: '抢修完成，设备正常运行', coopCompleteRate: 100 },
              { crossRegionCoopId: 'CRC20250112009', coopTask: '跨区逃费追缴', launchAreaCode: '漳州市文旅区', cooperateAreaCode: '漳州市主城区', progressNode: '追缴中，欠费用户已联系', coopCompleteRate: 50 },
              { crossRegionCoopId: 'CRC20250112010', coopTask: '跨区泊位调度', launchAreaCode: '漳州市周边区县', cooperateAreaCode: '漳州市经开区', progressNode: '调度规划中，方案制定完成', coopCompleteRate: 40 }
            ]);
          }, 500);
        });
      });
    ...
    return requestClient
      ...
      .then((response) => {
        if (
          ...
          typeof response === 'object' &&
          !Array.isArray(response) &&
          response.totalCount && response.completeRate && response.avgCoopDuration
        ) {
          return response;
        }
...
export const fetchCrossRegionAreaCount = (params = {}) => {
  try {
    ...
    return Promise.resolve({ xAxis: [], series: [{ name: '协同事项数', data: [] }] });
  }
};
...
export const fetchCrossRegionRateTrendData = (params = {}) => {
  try {
    return requestClient
      ...
      .then(res => res || { xAxis: [], series: [] })
      .catch(err => {
        console.log('协同完成率趋势折线图接口异常，使用兜底数据', err.message);
        return Promise.resolve({
          xAxis: ['01-01','01-02','01-03','01-04','01-05','01-06','01-07','01-08','01-09','01-10'],
          series: [{ name: '协同完成率(%)', data: [82, 85, 83, 87, 89, 86, 90, 88, 91, 92] }]
        });
      });
  } catch (error) {
            ...
            resolve([
              {
                ...
                coopRule: '每日早晚高峰巡逻，违规占道立即整改，违停车辆短信提醒',
                responsibilityDivision: '社区居委会：秩序巡查；城管局：违停执法；物业：现场整改',
                coopResult: '停车场出入口拥堵问题解决，违停率下降90%，居民投诉量清零',
                completeTime: 1736486400000
              },
              {
                specialCoopId: 'SPC20250110002',
                coopScene: '物业-停车运营服务协同',
                coopRule: '车位预约登记，包月缴费绿色通道，充电桩维保定期巡检',
                responsibilityDivision: '物业公司：运营服务；充电桩企业：设备维保；街道办：监督协调',
                coopResult: '车位利用率提升40%，包月缴费效率提升80%，充电桩故障率低于2%',
                completeTime: 1736572800000
              },
              {
                specialCoopId: 'SPC20250110003',
                coopScene: '社区-停车场秩序协同',
                coopRule: '节假日增派人员值守，临时车位扩容，人车分流规划整改',
                responsibilityDivision: '社区居委会：人员调配；住建局：规划整改；交警中队：交通疏导',
                coopResult: '节假日停车通行效率提升60%，未发生拥堵及剐蹭事故',
                completeTime: 1736659200000
              },
              {
                specialCoopId: 'SPC20250110004',
                coopScene: '物业-停车运营服务协同',
                coopRule: '智能停车系统升级，线上缴费全覆盖，车位导航精准推送',
                responsibilityDivision: '物业公司：系统运维；科技公司：技术支持；市场监管局：价格监督',
                coopResult: '停车缴费时长缩短至10秒内，车位找车效率提升70%，用户好评率98%',
                completeTime: 1736745600000
              },
              {
                specialCoopId: 'SPC20250110005',
                coopScene: '社区-停车场秩序协同',
                coopRule: '非机动车专区规划，充电桩合规整改，消防通道专项清理',
                responsibilityDivision: '社区居委会：场地规划；消防大队：合规检查；物业：日常维护',
                coopResult: '非机动车乱停放问题解决，消防通道畅通率100%，充电安全零事故',
                completeTime: 1736832000000
              }
            ]);
          }, 500);
        });
...
export const fetchSpecialCoopIndicators = (params = {}) => {
  try {
    return requestClient
      ...
      .then((response) => {
        if (response && typeof response === 'object' && !Array.isArray(response) && response.totalCount && response.completeRate && response.averageCycle) {
          return response;
        }
        throw new Error('真实接口返回无专属协同核心数据，使用模拟数据兜底');
      })
      .catch((error) => {
        console.warn('专属协同指标接口调用失败-使用模拟数据兜底', error.message);
        return new Promise((resolve) => {
          setTimeout(() => {
            resolve({
...
export const fetchSpecialCoopSceneCount = (params = {}) => {
  try {
    return requestClient
      ...
      .then((response) => {
        if (response && typeof response === 'object' && !Array.isArray(response) && response.xAxis && response.series) {
          return response;
        }
        throw new Error('真实接口返回无场景协同数数据，使用模拟数据兜底');
      })
      .catch((error) => {
        console.warn('场景协同数对比接口调用失败-使用模拟数据兜底', error.message);
        return new Promise((resolve) => {
          setTimeout(() => {
            resolve({
    ...
    return Promise.resolve({ xAxis: [], series: [{ name: '协同完成数', data: [] }] });
  }
};
...
export const fetchSpecialCoopDeptCount = (params = {}) => {
  try {
    return requestClient
      ...
      .catch((error) => {
        console.warn('责任单位协同数对比接口调用失败-使用模拟数据兜底', error.message);
        return new Promise((resolve) => {
          setTimeout(() => {
            resolve({
...
export const fetchSpecialCoopSceneRatio = (params = {}) => {
  try {
    return requestClient
      ...
      .then((response) => {
        if (response && typeof response === 'object' && !Array.isArray(response) && response.legend && response.series) {
          return response;
        }
        throw new Error('真实接口返回无协同场景占比数据，使用模拟数据兜底');
      })
      .catch((error) => {
        console.warn('协同场景占比饼图接口调用失败-使用模拟数据兜底', error.message);
        return new Promise((resolve) => {
          setTimeout(() => {
            resolve({
    ...
    return Promise.resolve({ legend: [], series: [{ name: '协同场景占比', data: [] }] });
  }
};
...
export const fetchSpecialCoopStatusRatio = (params = {}) => {
  try {
    return requestClient
      ...
      .catch((error) => {
        console.warn('协同状态占比饼图接口调用失败-使用模拟数据兜底', error.message);
        return new Promise((resolve) => {
          setTimeout(() => {
            resolve({
              legend: ['已完成', '进行中', '待协同'],
    ...
    return Promise.resolve({ legend: [], series: [{ name: '协同状态占比', data: [] }] });
  }
};
