...
const getTitle = computed(() => {
  return formData.value?.faultId ? textObj.editText : textObj.addText;
});
...
async function handleDelete(row) {
  const loadingInstance = ElLoading.service({
    text: $t('ui.actionMessage.deleting', [row.faultId]),
  });
  try {
...
async function handleDeleteBatch() {
  ...
  try {
    dataObj.apilist = dataObj.apilist.filter(
      (v) => !checkedIds.value.includes(v.faultId),
    );
    checkedIds.value = [];
...
function handleRowCheckboxChange({ records }) {
  checkedIds.value = records.map((item) => item.faultId);
}
...
<template>
  <div class="park-lot-table-new">
    ...
    <Grid>
      ...
      <template #disposalStatusName="{ row }">
        <el-tag :type="getStatusType(row.disposalStatusName)">
          {{ row.disposalStatusName }}
        </el-tag>
      </template>
      ...
      <template #bottom>
        <div class="common-total" @click="changeTotalShow">
          ...
          <span> 本页统计：故障数: {{ dataObj.total }} </span>
        </div>
