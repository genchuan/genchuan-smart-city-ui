...
import {
  ...
  archiveEnrollReport,
  exportEnrollReport,
  getEnrollReportDetail,
  getRegisterMgmtPage,
  getCheckinPage,
  getUserInfo,
} from '#/api/genchuan/industry/educationTeaching/studentMgmt/decisionAnalysis/enrollReport/data.js';
...
import {
  ...
  useGridColumns,
  useSearchFormSchema,
} from '#/api/genchuan/industry/educationTeaching/studentMgmt/decisionAnalysis/enrollReport/form.js';
...
import AdmitDetailDialog from '../components/AdmitDetailDialog.vue';
  ...
  async onConfirm() {
    ...
    try {
      ...
      const params = {
        ...
        sourcePlace: obj.sourcePlace || '',
        majorName: obj.majorName || '',
      };
      const response = await createEnrollReport(params);
...
// 明细弹窗引用
const sourceDetailRef = ref(null);
const applyDetailRef = ref(null);
const admitDetailRef = ref(null);
const checkinDetailRef = ref(null);
...
const getTableData = async (pageObj) => {
  ...
  try {
    ...
    const response = await getEnrollReportPage(params);
    if (response?.code === 200) {
...
const handleExport = async () => {
  try {
    ...
    const blob = await exportEnrollReport(params);
    downloadFileFromBlobPart({ fileName: '招生报表数据.xlsx', source: blob });
...
const handleExportRow = async (row) => {
  try {
    ...
    const loadingInstance = ElLoading.service({ text: '正在导出...' });
    const blob = await exportEnrollReport({ id: row.id });
    downloadFileFromBlobPart({ fileName: `招生报表_${row.reportCycle}.xlsx`, source: blob });
    ElMessage.success('导出成功');
    loadingInstance.close();
  } catch (error) {
    if (error !== 'cancel') {
...
const handleOpenDetail = async (row) => {
  ...
  try {
    const response = await getEnrollReportDetail({ id: row.id });
...
const handleGenerate = async (row) => {
  try {
    ...
    const loadingInstance = ElLoading.service({ text: '正在生成报表...' });
    const response = await createEnrollReport({ id: row.id });
    if (response?.code === 200) {
...
const handleArchive = async (row) => {
  try {
    ...
    const response = await archiveEnrollReport({ ids: [row.id] });
    if (response?.code === 200) {
...
const handleFieldDrill = (type, row) => {
  switch (type) {
    case 'sourceTotal':
      sourceDetailRef.value?.open();
      break;
    case 'applyNum':
      applyDetailRef.value?.open();
      break;
    case 'admitNum':
      admitDetailRef.value?.open();
      break;
    case 'checkinNum':
      checkinDetailRef.value?.open();
      break;
    case 'reportCycle':
...
const handleStatsFilter = (type, value) => {
  if (type === 'pie') {
    // 饼图钻取：生源地分布、专业选择分布等
    if (value === '本地生源' || value === '外地生源') {
      sourceDetailRef.value?.open({ sourcePlace: value });
    } else {
      applyDetailRef.value?.open({ major: value });
    }
  } else if (type === 'bar') {
    // 柱状图钻取：各专业报名人数、录取人数
    applyDetailRef.value?.open({ major: value });
...
<template>
  <div class="park-lot-table-new">
    ...
    <DetailDrawer ref="detailDrawerRef" :title="`${detailData.reportCycle || '招生报表'}详情`"
    ...
    <!-- 明细弹窗 -->
    <SourceDetailDialog ref="sourceDetailRef" />
    <ApplyDetailDialog ref="applyDetailRef" />
    <AdmitDetailDialog ref="admitDetailRef" />
    <CheckinDetailDialog ref="checkinDetailRef" />
    ...
    <Grid>
      ...
      <!-- 生源总数 -->
      <template #sourceTotal="{ row }">
        <span style="color: #409eff; cursor: pointer" @click="handleFieldDrill('sourceTotal', row)">
          {{ row.sourceTotal }}
        </span>
      </template>
      ...
      <template #applyNum="{ row }">
        <span style="color: #409eff; cursor: pointer" @click="handleFieldDrill('applyNum', row)">
          {{ row.applyNum }}
        </span>
      </template>

      <!-- 录取数 -->
      <template #admitNum="{ row }">
        <span style="color: #409eff; cursor: pointer" @click="handleFieldDrill('admitNum', row)">
          {{ row.admitNum }}
        </span>
      </template>

      <!-- 报到数 -->
      <template #checkinNum="{ row }">
        <span style="color: #409eff; cursor: pointer" @click="handleFieldDrill('checkinNum', row)">
          {{ row.checkinNum }}
        </span>
      </template>
