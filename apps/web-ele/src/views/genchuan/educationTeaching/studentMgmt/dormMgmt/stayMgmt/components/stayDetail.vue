...
const formatStayDate = (timestamp) => {
  if (!timestamp) return '-';
  const date = new Date(parseInt(timestamp));
  if (isNaN(date.getTime())) return timestamp;
  const year = date.getFullYear();
...
const drawerTitle = computed(() => {
  const id = detailObj.value?.studentId ? `学号 ${detailObj.value.studentId} 留宿申请` : '留宿申请详情';
...
<template>
  <DetailDrawer :title="drawerTitle">
    <div class="detail-card">
      <!-- 基础信息 -->
      <div class="detail-section">📋 留宿基础信息</div>
      ...
      <div class="detail-card-row"><div class="detail-row-left">留宿日期：</div><div class="detail-row-right">{{ formatStayDate(detailObj.stayDate) }}</div></div>
      <div class="detail-card-row"><div class="detail-row-left">留宿原因：</div><div class="detail-row-right">{{ detailObj.stayReason || '-' }}</div></div>
