...
import { passOpReportApi } from '#/api/genchuan/industry/chargePark/vehiclePass/api-map';
...
async function loadChartData() {
  try {
    ...
    const res = await passOpReportApi.getChart(params);

    // Always update card values
    if (res?.cardData) {
      cards[0].value = res.cardData.totalEnter || 0;
      cards[1].value = res.cardData.totalLeave || 0;
      cards[2].value = res.cardData.successRate
    ...
    const hasChartData =
      ...
      (res.trend?.length > 0 ||
        ...
        res.hourDistribution?.length > 0);

    if (hasChartData) {
      state.chartData = {
        trend: res.trend || [],
        hourDistribution: res.distribution || res.hourDistribution || [],
      };
      state.hasData = true;
      initCharts();
    } else {
...
function initPieChart() {
  ...
  const option = {
    ...
    legend: { bottom: 10, data: ['入场量', '离场量'] },
    xAxis: {
      type: 'category',
      data: state.chartData.trend.map((item) => item.date),
    },
    yAxis: { type: 'value', name: '通行数量' },
    series: [
      {
        name: '入场量',
        type: 'line',
        data: state.chartData.trend.map((item) => item.enter),
        smooth: true,
      ...
      },
      {
        name: '离场量',
        type: 'line',
        data: state.chartData.trend.map((item) => item.leave),
        smooth: true,
...
function initBarChart() {
  ...
  const option = {
    ...
    xAxis: {
      type: 'category',
      data: state.chartData.hourDistribution.map((item) => item.hour),
    },
    yAxis: { type: 'value', name: '通行量' },
    series: [
      {
        type: 'bar',
        data: state.chartData.hourDistribution.map((item) => item.count),
        itemStyle: {
