...
const open = async (type: string, id?: number) => {
  ...
  dialogTitle.value = type === 'create' ? '新增事件关联分析' : '编辑事件关联分析'
  formType.value = type
  resetForm()
  // 修改时，设置数据
  if (id) {
  ...
  <ElDialog :title="dialogTitle" v-model="dialogVisible" width="600px" append-to-body>
    ...
    >
      <ElFormItem label="关联分析编号" prop="analysisNumber">
        <ElInput v-model="formData.analysisNumber" placeholder="请输入关联分析编号" />
      </ElFormItem>
      <ElFormItem label="主事件编号" prop="mainEventNumber">
        <ElInput v-model="formData.mainEventNumber" placeholder="请输入主事件编号" />
      </ElFormItem>
      <ElFormItem label="主事件名称" prop="mainEventName">
        <ElInput v-model="formData.mainEventName" placeholder="请输入主事件名称" />
      </ElFormItem>
      <ElFormItem label="关联事件编号" prop="relatedEventNumber">
        <ElInput v-model="formData.relatedEventNumber" placeholder="请输入关联事件编号" />
      </ElFormItem>
      <ElFormItem label="关联事件名称" prop="relatedEventName">
        <ElInput v-model="formData.relatedEventName" placeholder="请输入关联事件名称" />
      </ElFormItem>
      <ElFormItem label="关联类型" prop="associationType">
        <ElSelect v-model="formData.associationType" placeholder="请选择关联类型">
      ...
      <ElFormItem label="关联强度" prop="associationStrength">
        <ElInput v-model="formData.associationStrength" placeholder="请输入关联强度" />
      </ElFormItem>
      ...
      <ElFormItem label="关联证据" prop="relatedEvidence">
        <ElInput v-model="formData.relatedEvidence" placeholder="请输入关联证据" />
      </ElFormItem>
      <ElFormItem label="建议措施" prop="recommendedMeasure">
        <ElInput v-model="formData.recommendedMeasure" placeholder="请输入建议措施" />
      </ElFormItem>
    </ElForm>
    <template #footer>
