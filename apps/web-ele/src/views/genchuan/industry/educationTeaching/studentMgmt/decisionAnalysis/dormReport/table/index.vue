...
import {
  ...
  getStayMgmtPage,
  getUserInfo,
} from '#/api/genchuan/industry/educationTeaching/studentMgmt/decisionAnalysis/dormReport/data.js';
...
import {
  ...
  useGridColumns,
  useSearchFormSchema,
} from '#/api/genchuan/industry/educationTeaching/studentMgmt/decisionAnalysis/dormReport/form.js';
import BedDetailDialog from '../components/BedDetailDialog.vue';
import StayDetailDialog from '../components/StayDetailDialog.vue';
  ...
  async onConfirm() {
    ...
    try {
      ...
      const params = {
        ...
        dormName: obj.dormName || '',
        className: obj.className || '',
      };
      const response = await createDormReport(params);
...
// 明细弹窗引用
const bedDetailRef = ref(null);
const stayDetailRef = ref(null);
...
const getTableData = async (pageObj) => {
  ...
  try {
    ...
    const response = await getDormReportPage(params);
    if (response?.code === 200) {
...
const handleExport = async () => {
  try {
    ...
    const blob = await exportDormReport(params);
    downloadFileFromBlobPart({ fileName: '宿舍报表数据.xlsx', source: blob });
...
const handleExportRow = async (row) => {
  try {
    ...
    const loadingInstance = ElLoading.service({ text: '正在导出...' });
    const blob = await exportDormReport({ id: row.id });
    downloadFileFromBlobPart({ fileName: `宿舍报表_${row.reportCycle}.xlsx`, source: blob });
    ElMessage.success('导出成功');
    loadingInstance.close();
  } catch (error) {
    if (error !== 'cancel') {
...
const handleOpenDetail = async (row) => {
  ...
  try {
    const response = await getDormReportDetail({ id: row.id });
...
const handleGenerate = async (row) => {
  try {
    ...
    const loadingInstance = ElLoading.service({ text: '正在生成报表...' });
    const response = await createDormReport({ id: row.id });
    if (response?.code === 200) {
...
const handleArchive = async (row) => {
  try {
    ...
    const response = await archiveDormReport({ ids: [row.id] });
    if (response?.code === 200) {
...
const handleFieldDrill = (type, row) => {
  switch (type) {
    case 'bedTotal':
      bedDetailRef.value?.open({ status: 'all' });
      break;
    case 'bedUsedNum':
      bedDetailRef.value?.open({ status: 'allocated' });
      break;
    case 'bedFreeNum':
      bedDetailRef.value?.open({ status: 'free' });
      break;
    case 'stayNum':
      stayDetailRef.value?.open();
      break;
    case 'reportCycle':
...
<template>
  <div class="park-lot-table-new">
    ...
    <DetailDrawer ref="detailDrawerRef" :title="`${detailData.reportCycle || '宿舍报表'}详情`"
    ...
    <!-- 明细弹窗 -->
    <BedDetailDialog ref="bedDetailRef" />
    <StayDetailDialog ref="stayDetailRef" />
    ...
    <Grid>
      ...
      <!-- 总床位数 -->
      <template #bedTotal="{ row }">
        <span style="color: #409eff; cursor: pointer" @click="handleFieldDrill('bedTotal', row)">
          {{ row.bedTotal }}
        </span>
      </template>

      <!-- 已用床位数 -->
      <template #bedUsedNum="{ row }">
        <span style="color: #409eff; cursor: pointer" @click="handleFieldDrill('bedUsedNum', row)">
          {{ row.bedUsedNum }}
        </span>
      </template>

      <!-- 空余床位数 -->
      <template #bedFreeNum="{ row }">
        <span style="color: #409eff; cursor: pointer" @click="handleFieldDrill('bedFreeNum', row)">
          {{ row.bedFreeNum }}
        </span>
      </template>

      <!-- 留宿人数 -->
      <template #stayNum="{ row }">
        <span style="color: #409eff; cursor: pointer" @click="handleFieldDrill('stayNum', row)">
          {{ row.stayNum }}
        </span>
      </template>
