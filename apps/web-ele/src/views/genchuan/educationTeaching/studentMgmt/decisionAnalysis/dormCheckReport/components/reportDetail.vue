...
const drawerTitle = computed(() => {
  const name = props.detailObj?.reportType || `${props.detailObj?.timeScale}宿舍考勤报表`;
...
const detailColumns = [
  ...
  { prop: 'abnormalCount', label: '异常人数', minWidth: 100 },
  { prop: 'inRate', label: '在寝率(%)', minWidth: 100 },
  { prop: 'totalStudentCount', label: '班级总人数', minWidth: 100 },
];
...
async function loadFullDetail() {
  ...
  try {
    const [chartRes, classRes] = await Promise.allSettled([
      getDormCheckReportChart(params),
      getDormCheckReportCheckCount(params),
    ]);
    ...
    if (classRes.status === 'fulfilled') {
      ...
      console.warn('班级统计接口失败', classRes.reason);
      classStats.value = [];
    }
  } catch (error) {
      ...
      <el-tabs v-model="activeTab" class="data-tabs" type="border-card">
        ...
        <el-tab-pane label="全维度统计" name="dimension">
          <div class="dimension-stats">
            ...
            <Bar
              v-if="currentBarData.xData.length"
            ...
            />
            <el-empty v-else description="暂无班级统计数据"/>
          </div>
        </el-tab-pane>
        ...
        <el-tab-pane label="明细数据" name="detail">
          <div class="detail-table-wrapper">
            <el-table :data="classStats" border stripe style="width: 100%">
              <el-table-column
            ...
            <el-empty v-if="!classStats.length" description="暂无明细数据"/>
          </div>
        </el-tab-pane>
      </el-tabs>
    </div>
