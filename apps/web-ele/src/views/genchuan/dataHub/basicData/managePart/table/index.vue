...
import { ElLoading, ElMessage, ElTag, ElDrawer, ElTreeSelect } from 'element-plus';
...
watch([() => props.treeData, () => props.tabType], ([newTreeData, newTabType]) => {
  if (newTreeData && newTreeData.length > 0) {
    // 根据tabType重新设置表单schema
    const newSchema = newTabType === 'instance'
    ...
    try {
      ...
      if (props.tabType === 'instance') {
        ...
        if (!submitData.parentId) {
          ...
          if (selectedNode) {
            submitData.parentCategoryName = selectedNode.label || selectedNode.categoryName;
          }
        }
...
async function handleExport() {
  if (props.tabType === 'instance') {
    ...
    exportToExcel({
      data: dataObj.list,
      columns: useInstanceGridColumns().filter(col => col.field && col.field !== 'actions'),
...
async function handleDelete(row) {
  const deleteName = props.tabType === 'instance' ? row.partName : row.categoryName;
  try {
...
async function handleDeleteBatch() {
  ...
  try {
    if (props.tabType === 'instance') {
      ...
      // 分类批量删除
      // 将id数组转换为数字类型
      const ids = checkedIds.value.map(id => Number(id));
...
watch(
  [() => props.filterCategoryId, () => props.tabType],
  () => {
    handleRefresh();
    // 重新初始化状态计数
    initStatusCounts();
  ...
  try {
    if (props.tabType === 'instance') {
      ...
      if (response) {
        ...
        dataObj.statistics.totalInstances = dataObj.list.reduce((sum, item) => sum + Number(item.monitorCount || 0), 0);
        // 正常运行统计 - 字典值2表示正常
        dataObj.statistics.auditedCount = dataObj.list.filter(item => item.runStatus === '2').length;

        // 更新状态计数（如果接口返回了统计数据）
        if (response.statistics) {
          dataObj.statusCounts.total = response.statistics.total || dataObj.total;
          // 正常: 2, 异常: 1, 离线: 3, 维护中: 4
          dataObj.statusCounts.enabled = response.statistics.normalCount || dataObj.list.filter(item => item.runStatus === '2').length;
          dataObj.statusCounts.maintenance = response.statistics.faultCount || dataObj.list.filter(item => item.runStatus === '4').length;
          dataObj.statusCounts.disabled = response.statistics.stopCount || dataObj.list.filter(item => item.runStatus === '3').length;
          dataObj.statusCounts.abnormal = dataObj.list.filter(item => item.runStatus === '1').length;
        }
      } else {
      ...
      if (response) {
        ...
        dataObj.statistics.auditedCount = dataObj.list.filter(item => item.auditStatus === '已审核').length;
      } else {
        ElMessage.error(response.message || '获取数据失败');
      }
    }
...
const queryFormSchema = computed(() => {
  const schema = props.tabType === 'instance'
...
watch([() => props.treeData, () => props.tabType], ([newTreeData, newTabType]) => {
  if (newTreeData && newTreeData.length > 0) {
    ...
    const newSchema = (newTabType === 'instance'
      ? useInstanceFormSchema(newTreeData)
      : useFormSchema(newTreeData)).map((v) => {
      delete v.rules;
      return { ...v };
    });
...
const handleCategoryCodeClick = (categoryCode) => {
  filterCategoryCode.value = filterCategoryCode.value === categoryCode ? '' : categoryCode;
  gridApi.query();
};
...
const handleParentCategoryNameClick = (parentCategoryName) => {
  filterParentCategoryName.value = filterParentCategoryName.value === parentCategoryName ? '' : parentCategoryName;
  gridApi.query();
};
...
const handleCategoryTypeClick = (categoryType) => {
  filterCategoryType.value = filterCategoryType.value === categoryType ? '' : categoryType;
  gridApi.query();
};
  ...
  try {
    if (props.tabType === 'instance') {
      ...
      if (response && response.list) {
        ...
        // 正常: 2, 异常: 1, 离线: 3, 维护中: 4
        dataObj.statusCounts.enabled = allData.filter(item => item.runStatus === '2').length;
        dataObj.statusCounts.abnormal = allData.filter(item => item.runStatus === '1').length;
        dataObj.statusCounts.disabled = allData.filter(item => item.runStatus === '3').length;
        dataObj.statusCounts.maintenance = allData.filter(item => item.runStatus === '4').length;
      }
    } else {
      // 分类状态统计
      ...
      if (response && response.list) {
        const allData = response.list;
        dataObj.statusCounts.total = response.total;
        dataObj.statusCounts.enabled = allData.filter(item => item.status === '1').length;
        dataObj.statusCounts.disabled = allData.filter(item => item.status === '0').length;
        dataObj.statusCounts.maintenance = allData.filter(item => item.status === '2').length;
      }
    }
  } catch (error) {
    ...
    <Grid :key="props.tabType">
      ...
      <template #bottom>
        <div class="common-total" @click="changeTotalShow">
          ...
          <span v-if="props.tabType === 'instance'"> 本页统计：部件实例数量: {{ dataObj.list.length }} </span>
          <span v-else> 本页统计：分类数量: {{ dataObj.list.length }} </span>
        </div>
        <div class="common-total-bottom" v-if="dataObj.totalShow">
          <span v-if="props.tabType === 'instance'"> 全部统计：部件实例数量{{ dataObj.statistics.totalCategories }}; 关联监测部件数{{ dataObj.statistics.totalInstances }}; 正常{{ dataObj.statusCounts.enabled || 0 }}; 异常{{ dataObj.statusCounts.abnormal || 0 }}; 离线{{ dataObj.statusCounts.disabled || 0 }}; 维护中{{ dataObj.statusCounts.maintenance || 0 }} </span>
          <span v-else> 全部统计：分类数量{{ dataObj.statistics.totalCategories }}; 关联实例数{{ dataObj.statistics.totalInstances }}; 已审核{{ dataObj.statistics.auditedCount }} </span>
        </div>
      </template>
    </Grid>
