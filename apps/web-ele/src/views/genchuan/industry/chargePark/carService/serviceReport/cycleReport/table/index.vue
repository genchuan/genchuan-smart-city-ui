      ...
      validator: (rule, value, callback) => {
        if (createForm.statType === '自定义') {
          ...
          if (
            ...
            new Date(value) <= new Date(createForm.statStartTime)
          ) {
...
const handleRowExport = async (id) => {
  try {
    await ElMessageBox.confirm('确认导出该报表的完整明细数据吗？', '提示', {
      type: 'info',
    });
...
const renderCompareCharts = () => {
  ...
  const previousTotalData = totalMetrics.map(
    (m) => Number(previous[m.key]) || 0,
  );
  ...
  if (rateChartRef.value) {
    ...
    rateChart.setOption({
      tooltip: {
        ...
        formatter: '{b}<br/>当前周期: {c0}%<br/>对比周期: {c1}%',
      },
...
const handleCompare = async (id, type) => {
  const res =
    type === 'yoy' ? await compareYoy({ id }) : await compareMom({ id });
...
const handleChartDrill = async (event) => {
  ...
  // 只有当前激活 tab 的 table 实例才响应,防止之前访问过的其它 tab(仍挂载)也开抽屉
  if (
    filters._activeCycleType !== undefined &&
    filters._activeCycleType !== props.cycleType
  )
    return;
  ...
  <div class="park-lot-table-new">
    <Grid>
      ...
      <template #rescueCompleteRate="{ row }">
        <el-text @click="openDimensionDetail(row.id, 'rescue')" type="primary">
          {{ row.rescueCompleteRate }}%
        </el-text>
      </template>
      <template #reserveSuccessRate="{ row }">
        <el-text @click="openDimensionDetail(row.id, 'reserve')" type="primary">
          {{ row.reserveSuccessRate }}%
        </el-text>
      </template>
      <template #complaintHandleRate="{ row }">
        <el-text
          @click="openDimensionDetail(row.id, 'complaint')"
          type="primary"
        >
          {{ row.complaintHandleRate }}%
        </el-text>
      </template>
      <template #findCarSuccessRate="{ row }">
        <el-text @click="openDimensionDetail(row.id, 'findCar')" type="primary">
          {{ row.findCarSuccessRate }}%
        </el-text>
      </template>
      <template #spacePushSuccessRate="{ row }">
        <el-text
          @click="openDimensionDetail(row.id, 'spacePush')"
          type="primary"
        >
          {{ row.spacePushSuccessRate }}%
        </el-text>
      </template>
      <template #effectiveWordingCount="{ row }">
        <el-text @click="openDimensionDetail(row.id, 'wording')" type="primary">
          {{ row.effectiveWordingCount }}
        </el-text>
      </template>
      <template #generateStatus="{ row }">
        <el-tag
          @click="filterByField('generateStatus', row.generateStatus)"
          style="cursor: pointer"
        >
          {{ row.generateStatus }}
        </el-tag>
      ...
      <template #yearOnYearGrowthRate="{ row }">
        <el-text @click="handleCompare(row.id, 'yoy')" type="primary">
          {{ row.yearOnYearGrowthRate }}%
        </el-text>
      </template>
      <template #monthOnMonthGrowthRate="{ row }">
        <el-text @click="handleCompare(row.id, 'mom')" type="primary">
          {{ row.monthOnMonthGrowthRate }}%
        </el-text>
      </template>
      <template #operator="{ row }">
        <el-text
          @click="showOperatorDetail(row.operatorUserId, row.operator)"
          type="primary"
        >
          {{ row.operator }}
        </el-text>
    ...
    <CreateDrawer>
      ...
      >
        ...
        <el-form-item
          v-if="createForm.statType && createForm.statType !== '自定义'"
          label="统计区间"
        >
          <el-text type="info">
            {{ createForm.statStartTime }} 至
            {{ createForm.statEndTime }}
          </el-text>
        </el-form-item>
        ...
        <div class="compare-period">
          <div class="period-item">
            <el-tag type="info" effect="plain">
              当前周期：{{ compareData.current?.statWindow || '-' }}
            </el-tag>
          </div>
          <div class="period-item">
            <el-tag type="info" effect="plain">
              对比周期：{{ compareData.previous?.statWindow || '-' }}
            </el-tag>
          </div>
        </div>
