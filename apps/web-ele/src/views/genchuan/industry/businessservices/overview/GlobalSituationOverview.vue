        ...
        <div class="bottom_left" style="min-width: 2vw;">
          ...
          <div class="panel" ref="regionPatternPanel">
            <div class="panel-header">
              ...
              <div class="header-actions">
                <!-- 时间范围筛选 -->
                <el-radio-group v-model="overviewTimeRange" placeholder="时间范围" size="small">
...
import {
  ...
  viewReport
} from '#/api/genchuan/industry/businessservices/overview/GlobalSituationOverview.js';
...
const refreshEnterpriseData = async () => {
  try {
    enterpriseResources.value = await fetchEnterpriseResourceDistribution();
  } catch (error) {
...
const refreshServiceData = async () => {
  try {
    serviceOverview.value = await fetchServiceItemsOverview();
  } catch (error) {
    ElMessage.error('刷新失败: ' + (error.message || '未知错误'));
  }
};
