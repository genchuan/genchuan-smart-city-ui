...
import WaterHydrologyParamForm from './WaterHydrologyParamForm.vue';

/** 水源水文参数管理 列表 */
defineOptions({ name: 'WaterHydrologyParam' });
...
const handleDelete = async (id: number) => {
  try {
    // 删除的二次确认
    await confirm('是否确认删除该水源水文参数数据？', '系统提示');
    // 发起删除
    await WaterHydrologyParamApi.deleteWaterHydrologyParam(id);
...
<template>
  <div class="p-4">
    ...
    <ElCard class="mb-4" shadow="never">
      ...
      >
        ...
        <ElFormItem label="含水层厚度(米)" prop="aquiferThickness">
          <ElInput
            v-model="queryParams.aquiferThickness"
        ...
        <ElFormItem label="渗透系数(m/d)" prop="permeabilityCoefficient">
          <ElInput
            v-model="queryParams.permeabilityCoefficient"
        ...
        <ElFormItem label="数据采集人" prop="dataCollector">
          <ElInput
            v-model="queryParams.dataCollector"
    ...
    <ElCard shadow="never">
      ...
      >
        ...
        <ElTableColumn
          label="含水层厚度(米)"
          align="center"
          prop="aquiferThickness"
          min-width="140"
        />
        <ElTableColumn
          label="渗透系数(m/d)"
          align="center"
          prop="permeabilityCoefficient"
          min-width="140"
        />
        <ElTableColumn
          label="数据采集人"
    ...
    <!-- 表单弹窗：添加/修改 -->
    <WaterHydrologyParamForm ref="formRef" @success="getList" />
  </div>
</template>
