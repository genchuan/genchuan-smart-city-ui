...
export const PatrolTaskManagementApi = {
  ...
  getPatrolTaskManagementPage: async (params: any) => {
    return await requestClient.get(`/smartcity/patrol-task-management/page`, { params });
  },

  // 查询巡査任务管理详情
  getPatrolTaskManagement: async (id: number) => {
    return await requestClient.get(`/smartcity/patrol-task-management/get`, { params: { id } });
  },

  // 新增巡査任务管理
  createPatrolTaskManagement: async (data: any) => {
    return await requestClient.post(`/smartcity/patrol-task-management/create`, data);
  },

  // 修改巡査任务管理
  updatePatrolTaskManagement: async (data: any) => {
    return await requestClient.put(`/smartcity/patrol-task-management/update`, data);
  },

  // 删除巡査任务管理
  deletePatrolTaskManagement: async (id: number) => {
    return await requestClient.delete(`/smartcity/patrol-task-management/delete`, { params: { id } });
  },

  // 导出巡査任务管理 Excel
  exportPatrolTaskManagement: async (params) => {
    return await requestClient.download(`/smartcity/patrol-task-management/export-excel`, params);
  },
};
