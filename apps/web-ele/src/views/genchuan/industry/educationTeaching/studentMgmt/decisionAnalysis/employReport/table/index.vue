...
import {
  ...
  archiveEmployReport,
  exportEmployReport,
  getEmployReportDetail,
  getCoopEnterprisePage,
  getStudyUpPage,
  getUserInfo,
} from '#/api/genchuan/industry/educationTeaching/studentMgmt/decisionAnalysis/employReport/data.js';
...
import {
  ...
  useGridColumns,
  useSearchFormSchema,
} from '#/api/genchuan/industry/educationTeaching/studentMgmt/decisionAnalysis/employReport/form.js';
import CoopEnterpriseDetailDialog from '../components/CoopEnterpriseDetailDialog.vue';
import StudyUpDetailDialog from '../components/StudyUpDetailDialog.vue';
...
const [FormDrawer, formDrawerApi] = useVbenDrawer({
  ...
  async onConfirm() {
    ...
    try {
      ...
      const response = await createEmployReport(params);
...
// 明细弹窗引用
const coopEnterpriseDetailRef = ref(null);
const studyUpDetailRef = ref(null);
...
const getTableData = async (pageObj) => {
  ...
  try {
    ...
    const response = await getEmployReportPage(params);
    if (response?.code === 200) {
...
const handleExport = async () => {
  try {
    ...
    const blob = await exportEmployReport(params);
    downloadFileFromBlobPart({ fileName: '就业报表数据.xlsx', source: blob });
...
const handleExportRow = async (row) => {
  try {
    ...
    const loadingInstance = ElLoading.service({ text: '正在导出...' });
    const blob = await exportEmployReport({ id: row.id });
    downloadFileFromBlobPart({ fileName: `就业报表_${row.reportCycle}.xlsx`, source: blob });
    ElMessage.success('导出成功');
    loadingInstance.close();
  } catch (error) {
    if (error !== 'cancel') {
...
const handleOpenDetail = async (row) => {
  ...
  try {
    const response = await getEmployReportDetail({ id: row.id });
...
const handleGenerate = async (row) => {
  try {
    ...
    const loadingInstance = ElLoading.service({ text: '正在生成报表...' });
    const response = await createEmployReport({ id: row.id });
    if (response?.code === 200) {
...
const handleArchive = async (row) => {
  try {
    ...
    const response = await archiveEmployReport({ ids: [row.id] });
    if (response?.code === 200) {
...
const handleFieldDrill = (type, row) => {
  switch (type) {
    case 'coopEnterpriseNum':
      coopEnterpriseDetailRef.value?.open();
      break;
    case 'studyUpNum':
      studyUpDetailRef.value?.open();
      break;
    case 'reportCycle':
...
const handleStatsFilter = (type, value) => {
  if (type === 'pie') {
    // 饼图钻取：合作企业类型分布 / 升学院校类型分布
    if (value === '制造业' || value === '服务业' || value === '互联网') {
      coopEnterpriseDetailRef.value?.open({ enterpriseType: value });
    } else {
      studyUpDetailRef.value?.open({ schoolType: value });
    }
  } else if (type === 'bar') {
    // 柱状图钻取：各系部合作企业数 -> 筛选系部
    coopEnterpriseDetailRef.value?.open({ deptName: value });
  } else if (type === 'line') {
    // 折线图钻取
...
<template>
  <div class="park-lot-table-new">
    ...
    <DetailDrawer ref="detailDrawerRef" :title="`${detailData.reportCycle || '就业报表'}详情`"
    ...
    <!-- 明细弹窗 -->
    <CoopEnterpriseDetailDialog ref="coopEnterpriseDetailRef" />
    <StudyUpDetailDialog ref="studyUpDetailRef" />
    ...
    <Grid>
      ...
      <!-- 合作企业数 -->
      <template #coopEnterpriseNum="{ row }">
        <span style="color: #409eff; cursor: pointer" @click="handleFieldDrill('coopEnterpriseNum', row)">
          {{ row.coopEnterpriseNum }}
        </span>
      </template>

      <!-- 升学人数 -->
      <template #studyUpNum="{ row }">
        <span style="color: #409eff; cursor: pointer" @click="handleFieldDrill('studyUpNum', row)">
          {{ row.studyUpNum }}
        </span>
      </template>
