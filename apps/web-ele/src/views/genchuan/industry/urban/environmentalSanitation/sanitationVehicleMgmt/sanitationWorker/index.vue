...
// 导入原有模拟数据（用于非“全部”标签页）
import { dataList } from '#/api/genchuan/industry/urban/environmentalSanitation/sanitationVehicleMgmt/sanitationWorker/data.js';

// 导入表单配置（新增了接口相关的 schema）
import {
  textObj,
  useFormSchema,                         // 原有表单（模拟数据）
  useUserEditSchema,                      // 新增：接口表单
  useUserSearchSchema,                    // 新增：搜索表单
  getColumnsByStatus,
  getJobTypeOptions,
  getTeamOptions,
  getAreaOptions,
  ...
  getDeptOptions,
} from '#/api/genchuan/industry/urban/environmentalSanitation/sanitationVehicleMgmt/sanitationWorker/form.js';
...
function getTagDisplayText(field, id) {
  ...
  switch (field) {
    case 'jobTypeId':
      options = loadedOptions.jobType;
      break;
    case 'teamId':
      options = loadedOptions.team;
      break;
    case 'areaCode':
      options = loadedOptions.area;
      break;
    case 'personStatusId':
      options = loadedOptions.personStatus;
      break;
    case 'deptId':
...
const counts = ref({
  ...
  statusCounts: {
    ...
    待排班: mockDataList.filter(v => v.status === '待排班').length,
    待考勤: mockDataList.filter(v => v.status === '待考勤').length,
    考核待审核: mockDataList.filter(v => v.status === '考核待审核').length,
...
function convertUserItem(item) {
  return {
    ...
    // 保留原始时间戳（毫秒），表格列通过 formatter 格式化显示
    entryTime: item.entryTime ? new Date(item.entryTime).getTime() : null,
  };
}
...
const getTableData = async ({ page }) => {
  if (isApiTab.value) {
    ...
    try {
      ...
      dataObj.total = total;
      dataObj.list = listData.map(convertUserItem);
      return dataObj;
    } catch (error) {
...
const [ApiEditDrawer, apiEditDrawerApi] = useVbenDrawer({
  ...
  async onConfirm() {
    ...
    // 处理可能为JSON的字段（如 skillTags）
    const submitData = { ...formValues };
    if (submitData.skillTags && typeof submitData.skillTags === 'string') {
      try {
        // 如果传入的是数组字符串，尝试解析，但保存时应该已经是JSON字符串，所以保持原样
        // 这里不做额外处理，因为表单中 skillTags 是 Input 组件，用户输入JSON字符串
      } catch {}
    }
    ...
    try {
      if (isAdd) {
        ...
        // 删除派生字段
        delete originalData.jobTypeName;
        delete originalData.teamName;
        delete originalData.areaName;
        delete originalData.personStatusName;
        delete originalData.deptName;
        delete originalData.roleName;
        delete originalData.createName;
        delete originalData.updateName;
  ...
  async onOpenChange(isOpen) {
    if (isOpen) {
      ...
      if (apiFormData.value?.id) {
        // 确保 entryTime 是时间戳，匹配 DatePicker 的 valueFormat: 'x'
        const formValues = {
          ...apiFormData.value,
          entryTime: apiFormData.value.entryTime
            ? new Date(apiFormData.value.entryTime).getTime()
            : null,
        };
        await apiEditFormApi.setValues(formValues);
      } else {
...
async function handleDelete(row) {
  const loading = ElLoading.service({ text: $t('ui.actionMessage.deleting', [row.userName || row.toiletName]) });
  try {
    if (isApiTab.value) {
...
async function handleDeleteBatch() {
  ...
  try {
    if (isApiTab.value) {
      await deleteUserBatch(checkedIds.value);
    } else {
      // 模拟数据批量删除
...
async function handleExport() {
  if (isApiTab.value) {
    ...
    try {
      const response = await exportUserExcel(params);
      const blob = response.data || response;
      const url = URL.createObjectURL(blob);
      const link = document.createElement('a');
      link.href = url;
      link.download = `环卫人员管理_${new Date().toLocaleDateString()}.xlsx`;
...
async function loadOptions() {
  try {
    ...
    loadedOptions.jobType = extractData(jobTypeRes);
    loadedOptions.team = extractData(teamRes);
    loadedOptions.area = extractData(areaRes);
    loadedOptions.personStatus = extractData(personStatusRes);
    loadedOptions.dept = extractData(deptRes);

    // 更新接口编辑表单
    await apiEditFormApi.updateSchema([
      { fieldName: 'jobTypeId', componentProps: { options: loadedOptions.jobType } },
      { fieldName: 'teamId', componentProps: { options: loadedOptions.team } },
      { fieldName: 'areaCode', componentProps: { options: loadedOptions.area } },
      { fieldName: 'personStatusId', componentProps: { options: loadedOptions.personStatus } },
...
<!--        </div>-->
      ...
      <template #userName="{ row }">
        <el-text @click="handleOpenDetail(row)" type="primary">{{ row.userName || row.toiletName }}</el-text>
      </template>
      <template #jobType="{ row }">
        <el-text @click="handleFilterTagClick('jobTypeId', row.jobTypeId)" type="primary">
          {{ row.jobTypeName || row.location }}
        </el-text>
      </template>
      <template #team="{ row }">
        <el-text @click="handleFilterTagClick('teamId', row.teamId)" type="primary">
          {{ row.teamName || '-' }}
        </el-text>
      </template>
      <template #area="{ row }">
      ...
      <template #status="{ row }">
        <el-text @click="handleFilterTagClick('personStatusId', row.personStatusId)" type="primary">
          {{ row.personStatusName || row.status }}
        </el-text>
      </template>
