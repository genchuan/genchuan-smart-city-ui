...
const getTableData = (pageObj) => {
  ...
  if (activeName.value === '待处理') {
    ...
    filteredList = taskObj.apilist.filter(v => v.initiator !== '');
  }
...
const createLabel = (item) => {
  ...
  if (item.label === '待处理') {
    ...
    count = taskObj.apilist.filter((v) => v.initiator !== '').length;
    ...
    <Grid>
      ...
      <template #bottom>
        <div class="common-total" @click="changeTotalShow">
          ...
          <span> 本页统计：任务数量{{ taskObj.list.length }}; 待处理: {{ taskObj.list.filter(v => v.taskStatus === '待处理').length }}; 处理中: {{ taskObj.list.filter(v => v.taskStatus === '处理中').length }} </span>
        </div>
