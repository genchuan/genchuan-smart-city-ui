...
const drawerTitle = computed(() => {
  const targetBerthNo = detailObj.value?.targetBerthNo || '停车计费';
  return `${targetBerthNo}详情`;
});
...
<template>
  <DetailDrawer :title="drawerTitle" class="parking-fee-detail-drawer">
    <div class="detail-card">
      ...
      <div class="detail-card-row">
        <div class="detail-row-left">当前停放车辆:</div>
        <div class="detail-row-right">
          {{ detailObj.currentParkingVehicle || '-' }}
        </div>
      </div>
      <div class="detail-card-row">
      ...
      <div class="detail-card-row">
        ...
        <div class="detail-row-right">
          {{
            detailObj.receivableAmount
              ? `¥${detailObj.receivableAmount.toFixed(2)}`
              : '-'
          }}
        </div>
      </div>
