...
import {
  ...
  updateUrbanVillage,
  deleteUrbanVillage,
  deleteUrbanVillageBatch,
  exportUrbanVillageExcel,
} from '#/api/genchuan/industry/urban/environmentalSanitation/sanitationSceneMgmt/urbanVillage/data.js';

// 导入原有模拟数据（用于非“全部”标签页）
import { dataList } from '#/api/genchuan/industry/urban/environmentalSanitation/sanitationSceneMgmt/urbanVillage/data.js';

// 导入表单配置（新增了接口相关的 schema）
import {
  textObj,
  useFormSchema,                         // 原有表单（模拟数据）
  useUrbanVillageEditSchema,              // 新增：接口表单
  useUrbanVillageSearchSchema,            // 新增：搜索表单
  getColumnsByStatus,
  getAreaOptions,
  getUserOptions,
  ...
  getReviewResultOptions,
  getProblemTypeOptions,
  getPlanStatusOptions,
} from '#/api/genchuan/industry/urban/environmentalSanitation/sanitationSceneMgmt/urbanVillage/form.js';
...
const counts = ref({
  ...
  statusCounts: {
    ...
    处置待复核: mockDataList.filter(v => v.status === '处置待复核').length,
...
// ---------- 转换函数（接口数据 -> 表格显示） ----------
function convertUrbanVillageItem(item) {
  // 解析 JSON 字符串字段
  const parseJSON = (str) => {
    if (!str) return [];
    try {
  ...
  let problemPhotoUrl = item.problemPhotoUrl ? parseJSON(item.problemPhotoUrl) : [];
  let reformPhotoUrl = item.reformPhotoUrl ? parseJSON(item.reformPhotoUrl) : [];

  // 新增：将 staffIds 映射为用户名称（依赖 loadedOptions.user）
  const mapIdsToLabels = (ids, options) => {
    if (!ids || !options || !options.length) return [];
    return ids.map(id => {
...
const getTableData = async ({ page }) => {
  if (isApiTab.value) {
    ...
    try {
      const res = await getUrbanVillagePage(params);
      const listData = res.data?.list || res.list || [];
      const total = res.data?.total || res.total || 0;
      counts.value.total = total; // 更新计数
      dataObj.total = total;
      dataObj.list = listData.map(convertUrbanVillageItem);
      return dataObj;
    } catch (error) {
...
const [SearchForm, searchFormApi] = useVbenForm({
  ...
  layout: 'horizontal',
  schema: useUrbanVillageSearchSchema(),
  showCollapseButton: true,
  submitButtonOptions: { content: '查询' },
});
...
const [ApiEditForm, apiEditFormApi] = useVbenForm({
  commonConfig: { componentProps: { class: 'w-full' }, formItemClass: 'col-span-2', labelWidth: 80 },
  layout: 'horizontal',
  schema: useUrbanVillageEditSchema(),
  showDefaultActions: false,
});
...
const [ApiEditDrawer, apiEditDrawerApi] = useVbenDrawer({
  ...
  async onConfirm() {
    ...
    const arrayFields = ['staffIds', 'problemPhotoUrl', 'reformPhotoUrl'];
    ...
    try {
      if (isAdd) {
        await createUrbanVillage(submitData);
      } else {
        // 合并原有数据（避免丢失字段）
        ...
        delete originalData.reviewName;
        delete originalData.reviewResultName;
        ...
        await updateUrbanVillage(fullData);
      }
...
async function handleDelete(row) {
  ...
  try {
    if (isApiTab.value) {
      await deleteUrbanVillage(row.id);
    } else {
      // 模拟数据删除
...
async function handleDeleteBatch() {
  ...
  try {
    if (isApiTab.value) {
      await deleteUrbanVillageBatch(checkedIds.value);
    } else {
      // 模拟数据批量删除
...
async function handleExport() {
  if (isApiTab.value) {
    ...
    try {
      const response = await exportUrbanVillageExcel(params);
      const blob = response.data || response;
      const url = URL.createObjectURL(blob);
      const link = document.createElement('a');
      link.href = url;
      link.download = `城中村环境管理_${new Date().toLocaleDateString()}.xlsx`;
...
async function loadOptions() {
  try {
    ...
    loadedOptions.reviewResult = extractData(reviewResultRes);
    ...
    await apiEditFormApi.updateSchema([
      ...
      { fieldName: 'reviewResultId', componentProps: { options: loadedOptions.reviewResult } },
...
<!--      </template>-->
      ...
      <template #reformPhotoUrl="{ row }">
        <a v-if="row.reformPhotoUrl" :href="row.reformPhotoUrl" target="_blank">查看</a>
        <span v-else>-</span>
      </template>
