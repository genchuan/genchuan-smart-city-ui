...
const pieOptions = computed(() => [
  {title: '升学意向分布', data: intentionPieData.value, type: 'intention'},
  {title: '院校类型选择分布', data: schoolTypePieData.value, type: 'schoolType'},
]);
...
const handleCardClick = (cardInfo) => {
  ...
  window.dispatchEvent(new CustomEvent('studyup-chart-filter', {
...
const handleBarClick = (schoolName) => {
  window.dispatchEvent(new CustomEvent('studyup-chart-filter', {
    detail: {type: 'schoolName', value: schoolName}
  }));
};
...
const handlePieClick = (item) => {
  ...
  if (pieType === 'intention') {
    window.dispatchEvent(new CustomEvent('studyup-chart-filter', {
      detail: {type: 'intention', value: item.name}
    }));
  } else if (pieType === 'schoolType') {
    window.dispatchEvent(new CustomEvent('studyup-chart-filter', {
      detail: {type: 'schoolType', value: item.name}
    }));
  }
};
...
const loadData = async () => {
  ...
  try {
    ...
    if (countRes.status === 'fulfilled') countData.value = countRes.value;
    else {
