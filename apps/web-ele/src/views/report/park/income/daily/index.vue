...
// 引入封装后的详情抽屉组件
import IncomeDetailDrawer from '#/views/report/park/income/daily/detail.vue';
...
async function handleDelete(row) {
  const loadingInstance = ElLoading.service({
    text: $t('ui.actionMessage.deleting', [row.areaName + ' - ' + row.parkType + ' - ' + row.payWay]),
  });
  try {
    reportObj.apilist = reportObj.apilist.filter((v) => v.id !== row.id);
    ElMessage.success($t('ui.actionMessage.deleteSuccess', [row.areaName + ' - ' + row.parkType + ' - ' + row.payWay]));
    handleRefresh();
  } finally {
...
const handleExportPDF = (row) => {
  // 这里可以调用PDF导出方法
  ElMessage.success(`正在导出 ${row.areaName} - ${row.parkType} 的收入数据PDF`);
  // window.open(`/api/export/pdf/${row.id}`, '_blank');
};
...
<template>
  <div class="park-lot-table-new">
    ...
    <IncomeDetailDrawer
      ...
      :title="`日收入详情 - ${reportObj.detailObj.areaName} ${reportObj.detailObj.parkType}`"
    />
    <Drawer title="搜索">
    ...
    <Grid>
      ...
      <template #avgOrderAmount="{ row }">
        <el-tag type="warning" size="small">
          ¥{{ row.avgOrderAmount.toFixed(2) }}
        </el-tag>
      </template>
      <template #onlinePayRatio="{ row }">
      ...
      <template #bottom>
        <div class="common-total" @click="changeTotalShow">
          ...
          <span> 本页统计：数据量{{ reportObj.list.length }};
            ...
            订单总数: {{ reportObj.list.reduce((sum, v) => sum + v.orderCount, 0) }};
            平均客单价: ¥{{ (reportObj.list.reduce((sum, v) => sum + v.avgOrderAmount, 0) / reportObj.list.length).toFixed(2) }};
            </span>
        </div>
