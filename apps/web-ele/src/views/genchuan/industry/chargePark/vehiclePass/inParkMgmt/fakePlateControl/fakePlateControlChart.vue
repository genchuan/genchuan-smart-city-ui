...
import { fakePlateControlApi } from '#/api/genchuan/industry/chargePark/vehiclePass/api-map';
...
async function loadChartData() {
  try {
    ...
    const res = await fakePlateControlApi.getChart(params);
    ...
    const hasChartData =
      res &&
      (res.fakeIdentifyTrend?.length > 0 || res.stationFakeCount?.length > 0);

    if (hasChartData) {
      state.chartData = {
        trend: res.fakeIdentifyTrend || [],
        stationCount: res.stationFakeCount || [],
      };
      state.hasData = true;
      await nextTick();
      initCharts();
