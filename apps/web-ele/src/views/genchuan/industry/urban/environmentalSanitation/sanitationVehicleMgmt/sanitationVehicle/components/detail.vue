...
<template>
  <DetailDrawer :title="drawerTitle">
    <div class="detail-card">
      ...
      <div class="detail-row"><span class="label">车辆牌照：</span>{{ detailObj.licensePlate || detailObj.toiletName || '-' }}</div>
      <div class="detail-row"><span class="label">车辆类型：</span>{{ detailObj.vehicleTypeName || detailObj.location || '-' }}</div>
      <div class="detail-row"><span class="label">所属部门：</span>{{ detailObj.deptName || detailObj.area || '-' }}</div>
      <div class="detail-row"><span class="label">车辆型号：</span>{{ detailObj.model || detailObj.openHours || '-' }}</div>
      <div class="detail-row"><span class="label">作业路线：</span>{{ detailObj.routeName || detailObj.stallCount || '-' }}</div>
      <div class="detail-row"><span class="label">驾驶员：</span>{{ detailObj.driverName || detailObj.manager || '-' }}</div>
      <div class="detail-row"><span class="label">车辆状态：</span>{{ detailObj.vehicleStatusName || detailObj.status || '-' }}</div>

      <!-- 接口数据特有字段 -->
      <template v-if="isApiData">
        <div class="detail-row"><span class="label">最近维护时间：</span>{{ detailObj.lastMaintenanceTime ? new Date(detailObj.lastMaintenanceTime).toLocaleString() : '-' }}</div>
        <div class="detail-row"><span class="label">累计作业时长：</span>{{ detailObj.totalWorkHours ?? '-' }}小时</div>
        <div class="detail-row"><span class="label">违规告警次数：</span>{{ detailObj.alarmCount ?? '-' }}</div>
        <div class="detail-row"><span class="label">车辆完好率：</span>{{ detailObj.intactRate ?? '-' }}%</div>
        <div class="detail-row"><span class="label">未完成任务数：</span>{{ detailObj.unfinishedTaskCount ?? '-' }}</div>
        <div class="detail-row"><span class="label">平均作业效率：</span>{{ detailObj.averageEfficiency ?? '-' }}吨/小时</div>
        <div class="detail-row"><span class="label">维护周期：</span>{{ detailObj.maintenanceCycle ?? '-' }}天</div>
        <div class="detail-row"><span class="label">计划状态：</span>{{ detailObj.planStatusName || '-' }}</div>
        <div class="detail-row"><span class="label">作业状态：</span>{{ detailObj.workStatusName || '-' }}</div>
        <div class="detail-row"><span class="label">违规类型：</span>{{ detailObj.violationTypeName || '-' }}</div>
        <div class="detail-row"><span class="label">违规状态：</span>{{ detailObj.violationStatusName || '-' }}</div>
        <div class="detail-row"><span class="label">维护类型：</span>{{ detailObj.maintenanceTypeName || '-' }}</div>
        <div class="detail-row"><span class="label">任务类型：</span>{{ detailObj.taskTypeName || '-' }}</div>
        <div class="detail-row"><span class="label">车辆照片：</span>
