...
import {
  alarmShareChargeMonitor,
  exportShareChargeMonitor,
  getShareChargeMonitorDetail,
  getShareChargeMonitorLocation,
  getShareChargeMonitorPage,
} from '#/api/genchuan/industry/chargePark/inspectOp/deviceMonitor/shareChargeMonitor';
...
async function getTableData({ page }) {
  ...
  try {
    ...
    dataObj.list = list.map((item) => normalizeShareChargeMonitorRow(item));
  } catch (error) {
    console.error('获取共享充电监测数据失败，使用静态数据:', error);
...
async function handleExport() {
  try {
    const data = await exportShareChargeMonitor({
      ...dataObj.searchParams,
      monitorStatus:
        filterMonitorStatus.value || dataObj.searchParams.monitorStatus,
      alarmStatus: filterAlarmStatus.value || dataObj.searchParams.alarmStatus,
...
async function handleLocate(row) {
  try {
    const response = await getShareChargeMonitorLocation({ id: row.id });
    const location = {
      ...row,
      ...response,
      id: row.id,
...
<template>
  <div class="park-lot-table-new">
    <DetailDrawer
      ...
      :fields="detailFields"
      :title="`${dataObj.detailObj.deviceCode || '共享充电监测'}详情`"
    />
