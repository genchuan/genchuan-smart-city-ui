...
function getTagDisplayText(field, id) {
  ...
  switch (field) {
    case 'institutionTypeId':
      options = loadedOptions.institutionType;
      break;
    case 'areaCode':
...
<!--        </div>-->
      ...
      <template #institutionType="{ row }">
        <el-text @click="handleFilterTagClick('institutionTypeId', row.institutionTypeId)" type="primary">
          {{ row.institutionTypeName || row.institutionType }}
        </el-text>
      </template>
      <template #area="{ row }">
