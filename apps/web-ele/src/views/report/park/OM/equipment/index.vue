...
// 引入封装后的详情抽屉组件
import DeviceRunDetailDrawer from '#/views/report/park/OM/equipment/detail.vue';
...
async function handleDelete(row) {
  const loadingInstance = ElLoading.service({
    text: $t('ui.actionMessage.deleting', [row.areaName + ' - ' + row.deviceType + ' - ' + row.status]),
  });
  try {
    reportObj.apilist = reportObj.apilist.filter((v) => v.id !== row.id);
    ElMessage.success($t('ui.actionMessage.deleteSuccess', [row.areaName + ' - ' + row.deviceType + ' - ' + row.status]));
    handleRefresh();
  } finally {
...
const getTableData = (pageObj) => {
  ...
  } else if (activeName.value === '在线状态') {
    filteredList = reportObj.apilist.filter(v => v.status === '在线');
  } else if (activeName.value === '故障状态') {
    filteredList = reportObj.apilist.filter(v => v.status === '故障');
  }
...
const handleOpenDetail = (row) => {
  ...
  // 通过ref调用组件的open方法
  deviceRunDetailDrawerRef.value.open();
  console.log(row);
};

// 查看设备地图
const handleViewMap = (row) => {
  // 这里可以跳转到设备地图页面，传递区域和设备类型参数
  window.open(`/map/device?area=${row.areaName}&type=${row.deviceType}`, '_blank');
};
...
const createLabel = (item) => {
  ...
  } else if (item.label === '在线状态') {
    count = reportObj.apilist.filter((v) => v.status === '在线').length;
  } else if (item.label === '故障状态') {
    count = reportObj.apilist.filter((v) => v.status === '故障').length;
...
// 定义组件ref，用于调用组件方法
const deviceRunDetailDrawerRef = ref(null);
</script>
...
<template>
  <div class="park-lot-table-new">
    ...
    <DeviceRunDetailDrawer
      ref="deviceRunDetailDrawerRef"
      :detail-obj="reportObj.detailObj"
      :title="`设备运行详情 - ${reportObj.detailObj.areaName} ${reportObj.detailObj.deviceType}`"
    />
    <Drawer title="搜索">
    ...
    <Grid>
      ...
      <template #status="{ row }">
        <el-tag
          :type="row.status === '在线' ? 'success' :
                 row.status === '离线' ? 'danger' :
                 row.status === '故障' ? 'warning' :
                 row.status === '维护中' ? 'primary' : 'info'"
          size="small"
        >
          {{ row.status }}
        </el-tag>
      </template>
      <template #totalDevice="{ row }">
        <el-tag type="primary" size="small">
          {{ row.totalDevice }}
        </el-tag>
      </template>
      <template #onlineDevice="{ row }">
        <el-tag type="success" size="small">
          {{ row.onlineDevice }}
        </el-tag>
      </template>
      <template #normalDevice="{ row }">
        <el-tag type="success" size="small">
          {{ row.normalDevice }}
        </el-tag>
      </template>
      <template #offlineDevice="{ row }">
        <el-tag type="danger" size="small">
          {{ row.offlineDevice }}
        </el-tag>
      </template>
      <template #faultDevice="{ row }">
        <el-tag type="warning" size="small">
          {{ row.faultDevice }}
        </el-tag>
      </template>
      <template #onlineRate="{ row }">
        <el-tag
          :type="parseFloat(row.onlineRate) > 95 ? 'success' :
                 parseFloat(row.onlineRate) > 90 ? 'primary' :
                 parseFloat(row.onlineRate) > 85 ? 'warning' : 'danger'"
          size="small"
        >
          {{ row.onlineRate }}
        </el-tag>
      ...
      <template #bottom>
        <div class="common-total" @click="changeTotalShow">
          ...
          <span> 本页统计：数据量{{ reportObj.list.length }};
            总设备数: {{ reportObj.list.reduce((sum, v) => sum + v.totalDevice, 0) }};
            在线设备数: {{ reportObj.list.reduce((sum, v) => sum + v.onlineDevice, 0) }};
            故障设备数: {{ reportObj.list.reduce((sum, v) => sum + v.faultDevice, 0) }};
            </span>
        </div>
