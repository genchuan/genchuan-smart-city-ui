...
import {
  ...
  submitFeedback
} from '#/api/genchuan/industry/waterresourceswateraffairs/overview/GlobalSituationOverview.js';
...
const changeTrendResource = async (resourceId) => {
  ...
  try {
    // 调用API获取趋势数据
    resourceTrendData.value = await fetchResourceTrend(resourceId);
  } catch (error) {
...
const getIndicatorTrend = async (indicatorId) => {
  try {
    indicatorTrendData.value = await fetchIndicatorTrend(indicatorId); // 赋值趋势数据（包含xAxis、series、unit）
  } catch (error) {
