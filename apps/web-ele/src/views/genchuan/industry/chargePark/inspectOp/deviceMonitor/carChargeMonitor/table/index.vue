...
import {
  alarmCarChargeMonitor,
  exportCarChargeMonitor,
  getCarChargeMonitorDetail,
  getCarChargeMonitorLocation,
  getCarChargeMonitorPage,
} from '#/api/genchuan/industry/chargePark/inspectOp/deviceMonitor/carChargeMonitor';
...
const emit = defineEmits(['locateDevice']);
...
function buildQueryParams(page) {
  return {
    ...
    deviceId: filterDeviceId.value || dataObj.searchParams.deviceId,
...
async function getTableData({ page }) {
  ...
  try {
    const response = await getCarChargeMonitorPage(queryParams);
    ...
    dataObj.list = list.map((item) => normalizeCarChargeMonitorRow(item));
  } catch (error) {
    console.error('获取汽车充电监测数据失败，使用静态数据:', error);
...
async function handleExport() {
  try {
    const data = await exportCarChargeMonitor({
      ...dataObj.searchParams,
      monitorStatus:
        filterMonitorStatus.value || dataObj.searchParams.monitorStatus,
      alarmStatus: filterAlarmStatus.value || dataObj.searchParams.alarmStatus,
...
async function handleOpenDetail(row) {
  try {
    const response = await getCarChargeMonitorDetail(row.id);
    dataObj.detailObj = normalizeCarChargeMonitorRow(response || row);
  } catch (error) {
    console.error('获取汽车充电监测详情失败，使用行数据:', error);
...
async function handleLocate(row) {
  try {
    const response = await getCarChargeMonitorLocation({ id: row.id });
    const location = {
      ...row,
      ...response,
      id: row.id,
    ...
    ElMessage.success(`已定位到设备：${location.deviceCode}`);
  } catch (error) {
    ...
    emit('locateDevice', row);
    ElMessage.success(`已定位到设备：${row.deviceCode}`);
  }
}
...
<template>
  <div class="park-lot-table-new">
    <DetailDrawer
      ...
      :fields="detailFields"
      :title="`${dataObj.detailObj.deviceCode || '汽车充电监测'}详情`"
    />
    ...
    <Grid>
      <template #table-title>
        <div class="charge-filter-tags">
          <ElTag
            v-if="filterDeviceId"
            closable
            type="success"
            @close="cancelFilter('deviceId')"
          >
            设备：{{ filterDeviceId }}
          </ElTag>
          <ElTag
