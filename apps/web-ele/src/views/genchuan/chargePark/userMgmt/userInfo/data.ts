...
import type {
  UserInfoChartVO,
  UserInfoVO,
} from '#/api/genchuan/chargePark/userMerchant/userMgmt/userInfo';
...
export function getOperatorDetail(name: string) {
  return (
    operatorMap[name] || {
      account: name,
      dept: '未分配部门',
...
export function buildUserRowFromApi(
  ...
  const row: UserRow = {
    ...
    updater: data.updater || fallback.updater || data.creator || '-',
    updateTime: formatApiTime(data.updateTime ?? fallback.updateTime),
    remark: data.remark || fallback.remark || '',
    cars: data.cars || fallback.cars || [],
    walletLogs: data.walletLogs || fallback.walletLogs || [],
    auditSummary: data.auditSummary || fallback.auditSummary || '创建用户',
    auditLogs: data.auditLogs || fallback.auditLogs || [],
  };
...
export function buildUserInfoQueryParams(formValues: Record<string, any>) {
  ...
  if (Array.isArray(params.registerTime) && params.registerTime.length === 2) {
    params.registerTime = `${dayjs(params.registerTime[0]).format('YYYY-MM-DD HH:mm:ss')},${dayjs(params.registerTime[1]).format('YYYY-MM-DD HH:mm:ss')}`;
  } else {
