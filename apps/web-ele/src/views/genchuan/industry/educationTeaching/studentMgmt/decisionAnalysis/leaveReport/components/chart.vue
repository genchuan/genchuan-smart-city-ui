...
import { getLeaveReportChart } from '#/api/genchuan/industry/educationTeaching/studentMgmt/decisionAnalysis/leaveReport/data.js';
...
const cardLabelMap = {
  leaveTotal: '离校总人数',
  handleFinishRate: '办理完成率(%)',
  checkoutFinishRate: '退宿完成率(%)',
  parentConfirmRate: '家长确认率(%)',
};
...
const chartOptions = [
  { value: 'pie', label: '离校去向分布' },
  { value: 'line', label: '办理进度趋势' },
];
...
const fetchChartData = async (customParams = null) => {
  ...
  try {
    ...
    const res = await getLeaveReportChart(params);
  ...
  <div v-loading="loading" class="chart-box">
    ...
    <div class="chart-area">
      ...
      <div v-else-if="activeChart === 'line'">
        <LineChart :x-data="lineXAxis" :series-data="lineSeriesData" :title="currentChartTitle" y-name="完成率(%)" @lineClick="handleLineClick" />
      </div>
    </div>
  </div>
</template>
