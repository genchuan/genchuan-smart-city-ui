...
import { ElButton, ElOption, ElOptionGroup, ElPopover, ElSelect, ElTag } from 'element-plus';
...
<template>
  <div class="property-selector">
      ...
      >
        ...
        >
          <div class="option-content">
            <span class="option-label">{{ property.name }}</span>
            <el-tag :type="getDataTypeTagType(property.dataType)" size="small" class="option-tag">
              {{ property.identifier }}
            </el-tag>
          </div>
        </el-option>
    ...
    >
      <template #reference>
        <el-button type="primary" text circle size="small" class="detail-btn" title="查看属性详情">
      ...
      <div class="property-detail-content">
        <div class="detail-header">
          ...
          <span class="header-title">{{ selectedProperty.name }}</span>
        ...
        <div class="detail-items">
          <div class="detail-item">
            <span class="item-label">标识符：</span>
            <span class="item-value">{{ selectedProperty.identifier }}</span>
          </div>

          <div v-if="selectedProperty.description" class="detail-item">
            <span class="item-label">描述：</span>
            <span class="item-value">{{ selectedProperty.description }}</span>
          </div>

          <div v-if="selectedProperty.unit" class="detail-item">
            <span class="item-label">单位：</span>
            <span class="item-value">{{ selectedProperty.unit }}</span>
          </div>

          <div v-if="selectedProperty.range" class="detail-item">
            <span class="item-label">取值范围：</span>
            <span class="item-value">{{ selectedProperty.range }}</span>
          </div>

          <!-- 根据属性类型显示额外信息 -->
          <div v-if="selectedProperty.type === IoTThingModelTypeEnum.PROPERTY && selectedProperty.accessMode" class="detail-item">
            <span class="item-label">访问模式：</span>
            <span class="item-value">{{ getAccessModeLabel(selectedProperty.accessMode) }}</span>
          </div>

          <div v-if="selectedProperty.type === IoTThingModelTypeEnum.EVENT && selectedProperty.eventType" class="detail-item">
            <span class="item-label">事件类型：</span>
            <span class="item-value">{{ getEventTypeLabel(selectedProperty.eventType) }}</span>
          </div>

          <div v-if="selectedProperty.type === IoTThingModelTypeEnum.SERVICE && selectedProperty.callType" class="detail-item">
            <span class="item-label">调用类型：</span>
            <span class="item-value">{{ getThingModelServiceCallTypeLabel(selectedProperty.callType) }}</span>
          </div>
        </div>
      </div>
    </el-popover>
