...
import * as pageApi from '#/api/genchuan/chargePark/stationResource/stationMgmt/stationConfig/index.js';
import * as areaApi from '#/api/genchuan/chargePark/stationResource/areaMgmt/areaInfo/index.js';
import * as stationInfoApi from '#/api/genchuan/chargePark/stationResource/stationMgmt/stationInfo/index.js';
...
function buildOptionsBySource(source, result) {
  ...
  if (source === 'StationInfo') {
    return list.map((item) => ({
      label:
        item.stationNo
          ? `${item.name || item.stationNo} (${item.stationNo})`
          : item.name || item.id || item.stationId,
...
function getFieldLabel(field) {
  const column = tableColumns.find((c) => c.field === field) || searchFields.find(f => f.field === field);
        ...
        <el-tab-pane :name="pageConfig.title">
          ...
          <Grid>
            <template #table-title>
              <div v-if="Object.keys(appliedQuery).length > 0" class="filter-tags-container" style="display: flex; flex-wrap: wrap; align-items: center;">
                <el-tag
