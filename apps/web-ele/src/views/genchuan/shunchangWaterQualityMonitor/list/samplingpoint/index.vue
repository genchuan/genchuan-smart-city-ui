...
import SamplingPointForm from './SamplingPointForm.vue';

/** 采样点规划 列表 */
defineOptions({ name: 'SamplingPoint' });
...
const getList = async () => {
  loading.value = true;
  try {
    const data = await SamplingPointApi.getSamplingPointPage(queryParams);
...
const handleDelete = async (id: number) => {
  try {
    // 删除的二次确认
    await confirm('是否确认删除该采样点规划数据？', '系统提示');
    // 发起删除
    await SamplingPointApi.deleteSamplingPoint(id);
...
const handleExport = async () => {
  try {
    // 导出的二次确认
    await confirm('是否确认导出所有采样点规划数据？', '系统提示');
    // 发起导出
    exportLoading.value = true;
    const data = await SamplingPointApi.exportSamplingPoint(queryParams);
    download.excel(data, '采样点规划.xls');
  } catch {
  } finally {
...
<template>
  <div class="p-4">
    ...
    <ElCard class="mb-4" shadow="never">
      ...
      >
        ...
        <ElFormItem label="覆盖人口" prop="coveredPopulation">
          <ElInput
            v-model="queryParams.coveredPopulation"
            placeholder="请输入覆盖人口"
            clearable
            @keyup.enter="handleQuery"
            style="width: 240px"
        ...
        <ElFormItem label="周边环境描述" prop="surroundingDesc">
          <ElInput
            v-model="queryParams.surroundingDesc"
            placeholder="请输入周边环境描述"
            clearable
            @keyup.enter="handleQuery"
            style="width: 240px"
          />
        ...
        <ElFormItem label="规划依据" prop="planningBasis">
          <ElInput
            v-model="queryParams.planningBasis"
            placeholder="请输入规划依据"
            clearable
            @keyup.enter="handleQuery"
            style="width: 240px"
    ...
    <ElCard shadow="never">
      ...
      >
        ...
        <ElTableColumn
          label="覆盖人口"
          align="center"
          prop="coveredPopulation"
          min-width="100"
        />
        <ElTableColumn
          label="周边环境描述"
          align="center"
          prop="surroundingDesc"
          min-width="150"
        />
        <ElTableColumn
          label="规划依据"
    ...
    <!-- 表单弹窗：添加/修改 -->
    <SamplingPointForm ref="formRef" @success="getList" />
  </div>
</template>
