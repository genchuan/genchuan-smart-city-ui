  ...
  <div class="park-lot-table-new">
    <Grid>
      ...
      <template #stationCount="{ row }">
        <el-text @click="addQuickFilter('stationCount', row.stationCount, `周边场站数: ${row.stationCount}`)" type="primary" style="cursor: pointer">
          {{ row.stationCount }}
        </el-text>
      </template>
      <template #emptyStationCount="{ row }">
        <el-text @click="addQuickFilter('hasEmpty', true, '有空位场站')" type="primary" style="cursor: pointer">
...
const getTableData = async ({ page }) => {
  ...
  if (searchParams.distanceRange) {
    params.distanceRange = searchParams.distanceRange;
    delete params.distanceRange;
  }

  if (searchParams.hasEmpty === true) {
    params.hasEmpty = true;
    delete params.hasEmpty;
  }
...
const activeFilters = computed(() => {
  ...
  if (searchParams.distanceRange) {
    filters.push({ label: `距离区间：${searchParams.distanceRange}`, field: 'distanceRange' });
  }
...
const handleChartRefresh = (event) => {
  ...
  if (filters.distanceRange) {
    addQuickFilter('distanceRange', filters.distanceRange, `距离区间: ${filters.distanceRange}`);
  } else if (filters.stationId) {
    addQuickFilter('stationId', filters.stationId, `场站ID: ${filters.stationId}`);
  } else if (filters.cardType === 'totalStation') {
    if (filters.action === 'clearAll') {
      clearAllFilters();
    } else {
      ElMessage.info('查看所有周边场站列表');
    }
  } else if (filters.cardType === 'emptyStation') {
    addQuickFilter('hasEmpty', true, '有空位场站');
  }
};
