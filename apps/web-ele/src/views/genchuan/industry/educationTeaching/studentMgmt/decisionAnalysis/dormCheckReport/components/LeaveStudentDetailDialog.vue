...
import { getLeaveStudentDetailList } from '#/api/genchuan/industry/educationTeaching/studentMgmt/decisionAnalysis/dormCheckReport/data.js';
...
const columns = [
  ...
  { field: 'leaveTime', title: '请假时间', minWidth: 160 },
];
...
const fetchData = async (params) => {
  ...
  const res = await getLeaveStudentDetailList({
  ...
  if (res.code === 200) {
    ...
    console.error('获取请假学生明细失败', res.msg);
    return { list: [], total: 0 };
  }
};
...
<template>
  <DetailDrawer title="请假学生明细" class="genchuan-detail-drawer">
