...
// 导入模拟数据
import {
  districtList as importedDistrictList,
  mockVehicleData as importedMockVehicleData,
...
const props = defineProps({
  ...
  deviceStats: {
    type: Object,
    default: () => ({ total: 0, online: 0, offline: 0, fault: 0 }),
  },
});
...
const displayVehicleList = computed(() =>
  props.vehicleList.length ? props.vehicleList : mockVehicleData,
);
const filteredVehicleList = computed(() =>
  displayVehicleList.value.filter((v) => v.district === currentDistrict.value),
);
...
const resetLayers = () => {
  Object.values(layers.value).forEach((l) => {
    if (l)
      try {
        l.destroy();
      } catch (e) {
...
const useDeterministicBoundary = (map, config) => {
  ...
  const currentDistrictData = districtList.value.find(
    (d) => d.name === config.name,
  );
  if (!currentDistrictData) {
    drawBoundaryLine(
      map,
      generateDeterministicPolygon(config.center, config.adcode),
      config,
    );
  ...
  if (boundaryCache.has(cacheKey)) {
    ...
    const simulatedBoundary = generateDeterministicPolygon(
      currentDistrictData.center,
      currentDistrictData.adcode,
    );
...
const loadDistrictBoundary = (map, config) => {
  ...
  if (window.TMap && window.TMap.service && window.TMap.service.District) {
    try {
      ...
      districtService
        .search({ keyword: config.adcode || config.name })
        .then((result) => {
...
const switchDistrict = (district) => {
  ...
  mapInstance.value.setCenter(
    new TMap.LatLng(district.center.lat, district.center.lng),
  );
...
const mapCallback = () => {
  ...
  const map = new TMap.Map(container, {
    center: new TMap.LatLng(
      districtConfig.value.center.lat,
      districtConfig.value.center.lng,
    ),
    zoom: 13,
