...
function getFieldLabel(field) {
  const map = { activityType: '活动类型', hostDept: '主办部门', status: '状态', creator: '创建人', createTime: '创建时间', activityName: '活动名称' };
...
const loadDeptOptions = async () => { const res = await getDeptOptions(); deptOptions.value = res; };
loadDeptOptions();
  ...
  try {
    ...
    Object.entries(tagFilters.value).forEach(([field, filterValue]) => {
      filtered = filtered.filter(item => {
        let itemValue;
        switch (field) {
          case 'activityType': itemValue = item.activityType; break;
          case 'hostDept': itemValue = item.hostDept; break;
          case 'status': itemValue = item.status; break;
          case 'creator': itemValue = item.creator; break;
          case 'createTime': itemValue = item.createTime ? getDateFromTimestamp(item.createTime) : ''; break;
          case 'activityName': itemValue = item.activityName; break;
...
async function handleBatchPublish() {
  if (checkedIds.value.length === 0) { ElMessage.warning('请至少选择一个活动'); return; }
  const unPublishRows = checkedRows.value.filter(row => row.status === '未发布');
  if (unPublishRows.length === 0) { ElMessage.warning('请选择状态为【未发布】的活动进行发布'); return; }
  try {
    await ElMessageBox.confirm(`确认发布选中的 ${unPublishRows.length} 个活动？发布后状态将变为“进行中”。`, '批量发布确认', { confirmButtonText: '确认', cancelButtonText: '取消', type: 'warning' });
    const loading = ElLoading.service({ text: '发布中...' });
    try {
      const ids = unPublishRows.map(row => row.id);
      const res = await publishMoralActivity(ids);
      if (res && res !== false) { ElMessage.success('批量发布成功'); handleRefresh(); }
      else { ElMessage.error('批量发布失败'); }
    } finally { loading.close(); }
  } catch {}
}

function handleCreate() { isEditMode.value = false; currentEditId.value = null; createDrawerApi.open(); }
function handleEdit(row) { isEditMode.value = true; currentEditId.value = row.id; createDrawerApi.open(); }

async function handlePublish(row) {
  if (row.status !== '未发布') { ElMessage.warning('只有未发布的活动可以发布'); return; }
  try {
    await ElMessageBox.confirm(`确认发布活动"${row.activityName}"？发布后状态将变为“进行中”。`, '发布确认', { confirmButtonText: '确认', cancelButtonText: '取消', type: 'warning' });
    const loading = ElLoading.service({ text: '发布中...' });
    try {
      const res = await publishMoralActivity([row.id]);
      if (res && res !== false) { ElMessage.success('发布成功'); handleRefresh(); }
...
const [CreateForm, createFormApi] = useVbenForm({
  ...
  handleSubmit: async (values) => {
    ...
    try {
      ...
      if (res && res !== false) { ElMessage.success(isEditMode.value ? '更新成功' : '发布成功'); createDrawerApi.close(); handleRefresh(); }
      else { ElMessage.error(isEditMode.value ? '更新失败' : '发布失败'); }
    } finally { loading.close(); }
  },
  layout: 'horizontal',
  ...
  async onOpenChange(isOpen) {
    if (isOpen) {
      ...
      if (isEditMode.value && currentEditId.value) {
        ...
        } catch (error) { console.error('加载详情失败', error); ElMessage.error('加载详情失败'); createDrawerApi.close(); }
      } else {
...
const [RecordForm, recordFormApi] = useVbenForm({
  ...
  handleSubmit: async (values) => {
    ...
    try {
      ...
      if (res && res !== false) { ElMessage.success('记录保存成功'); recordDrawerApi.close(); handleRefresh(); }
      else { ElMessage.error('记录保存失败'); }
    } finally { loading.close(); }
  },
  layout: 'horizontal',
...
function handleOpenDetail(row) { dataObj.detailObj = row; moralActivityDetailDrawerRef.value.open(); }
...
const [QueryForm] = useVbenForm({
  ...
  layout: 'horizontal',
  schema: useFormSchema().map(v => { delete v.rules; if (v.fieldName === 'hostDept') v.componentProps.options = deptOptions.value; return v; }),
...
<template>
  <div class="park-lot-table-new">
    <MoralActivityDetailDrawer ref="moralActivityDetailDrawerRef" :detail-obj="dataObj.detailObj" @refresh="handleRefresh" />
    <Drawer title="搜索"><QueryForm /></Drawer>
    <CreateDrawer :title="isEditMode ? textObj.editText : textObj.addText"><CreateForm /></CreateDrawer>
    <JoinModal><JoinForm /></JoinModal>
    <RecordDrawer :title="textObj.recordText"><RecordForm /></RecordDrawer>
    <Grid>
      ...
      <template #activityName="{ row }"><el-text @click="handleOpenDetail(row)" type="primary" style="cursor: pointer;">{{ row.activityName }}</el-text></template>
      <template #activityType="{ row }"><el-text @click="handleFilterTagClick('activityType', row.activityType)" type="primary" style="cursor: pointer;">{{ row.activityType }}</el-text></template>
      <template #hostDept="{ row }"><el-text @click="handleFilterTagClick('hostDept', row.hostDept)" type="primary" style="cursor: pointer;">{{ row.hostDept }}</el-text></template>
      <template #status="{ row }"><el-tag :type="getStatusType(row.status)" @click="handleFilterTagClick('status', row.status)" style="cursor: pointer;">{{ row.status }}</el-tag></template>
      <template #creator="{ row }"><el-text @click="handleFilterTagClick('creator', row.creator)" type="primary" style="cursor: pointer;">{{ row.creator || '-' }}</el-text></template>
      <template #createTime="{ row }"><el-text @click="handleFilterTagClick('createTime', getDateFromTimestamp(row.createTime))" type="primary" style="cursor: pointer;">{{ formatTimestamp(row.createTime) }}</el-text></template>
      ...
      <template #publishTime="{ row }"><el-text>{{ formatTimestamp(row.publishTime) }}</el-text></template>
      ...
      <template #actions="{ row }">
        <div class="table-toolbar-tools">
          ...
          <IconButton v-if="row.status === '未发布'" content="发布" icon-name="Upload" @click="handlePublish(row)" />
          <IconButton v-if="row.status === '进行中'" content="报名" icon-name="User" @click="handleJoin(row)" />
          <IconButton v-if="row.status === '进行中'" content="记录" icon-name="EditPen" @click="handleRecord(row)" />
        </div>
      </template>
    </Grid>
  </div>
