...
const drawerTitle = computed(() => {
  const id = detailObj.value?.studentId ? `学号 ${detailObj.value.studentId} 升学详情` : '升学详情';
...
<template>
  <DetailDrawer :title="drawerTitle">
    <div class="detail-card">
      <!-- 升学基础信息 -->
      <div class="detail-section">🎓 升学基础信息</div>
      <div class="detail-card-row"><div class="detail-row-left">学号：</div><div class="detail-row-right">{{ detailObj.studentId || '-' }}</div></div>
      <div class="detail-card-row"><div class="detail-row-left">目标院校名称：</div><div class="detail-row-right">{{ detailObj.schoolName || '-' }}</div></div>
      <div class="detail-card-row"><div class="detail-row-left">院校类型：</div><div class="detail-row-right">{{ detailObj.schoolType || '-' }}</div></div>
      <div class="detail-card-row"><div class="detail-row-left">意向专业：</div><div class="detail-row-right">{{ detailObj.major || '-' }}</div></div>
      <div class="detail-card-row"><div class="detail-row-left">升学规划内容：</div><div class="detail-row-right">{{ detailObj.planContent || '-' }}</div></div>
      ...
      <div class="detail-card-row"><div class="detail-row-left">跟踪记录时间：</div><div class="detail-row-right">{{ formatTimestamp(detailObj.recordTime) }}</div></div>
