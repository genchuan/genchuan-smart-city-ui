...
import { ElDivider, ElDrawer, ElMessage, ElTable } from 'element-plus';
import screenFull from 'screenfull';

import {
  fetchAbnormalWarningDurationSummary,
  fetchAbnormalWarningList,
  fetchAlertEventOverviewList,
  fetchBerthFaultList,
  fetchComplianceWarningList,
  fetchDisposalProgressDurationSummary,
  fetchDisposalProgressTrackingList,
  fetchHiddenDangerWarningList,
  fetchSpecialWarningList,
} from '#/api/genchuan/industry/parkingmgmt/overview/EventEarlyWarningTracking.ts';
...
type DetailDataType =
  | AbnormalWarningItem
  | AlertEventOverviewItem
  | ComplianceWarningItem
  | DisposalProgressTrackingItem
  | FaultListItem
  | HiddenDangerWarningItem
  | null
  | SpecialWarningItem;
...
const openDetailDrawer = (row: DetailDataType) => {
  ...
  else if (row && 'alertId' in row && !('disposalMeasure' in row))
    drawerTitle.value = '预警事件详情';
  else if (row && 'alertId' in row && 'disposalMeasure' in row)
    drawerTitle.value = '处置进度详情';
  else if (row && 'hiddenDangerId' in row) drawerTitle.value = '隐患预警详情';
  else if (row && 'complianceWarningId' in row)
    drawerTitle.value = '合规预警详情';
...
const fetchDisposalProgressDurationSummaryData = async () => {
  try {
    disposalProgressDurationSummary.value =
      await fetchDisposalProgressDurationSummary({});
  } catch (error) {
...
const initTableHeight = () => {
  nextTick(() => {
    ...
    const faultContainer = document.querySelector(
      '.bottom-left2 .table-container',
    );
    const alertContainer = document.querySelector(
      '.top-middle .table-container',
    );
    const hiddenDangerContainer = document.querySelector(
      '.bottom-left1 .table-container',
    );
    const complianceContainer = document.querySelector(
      '.bottom-right1 .table-container',
    );
    const specialContainer = document.querySelector(
      '.bottom-right2 .table-container',
    );

    faultContainer &&
      (faultTableMaxHeight.value = `${faultContainer.clientHeight}px`);
    alertContainer &&
      (alertTableMaxHeight.value = `${alertContainer.clientHeight}px`);
    hiddenDangerContainer &&
      (hiddenDangerTableMaxHeight.value = `${hiddenDangerContainer.clientHeight}px`);
    complianceContainer &&
      (complianceTableMaxHeight.value = `${complianceContainer.clientHeight}px`);
    specialContainer &&
      (specialTableMaxHeight.value = `${specialContainer.clientHeight}px`);

    const disposalPanel = document.querySelector(
      '.top-left.disposal-progress-tracking',
    );
    const disposalHeader = document.querySelector('.top-left .header-actions');
    const disposalCard = document.querySelector(
      '.top-left .progress-card-container',
    );
    ...
    const abnormalPanel = document.querySelector(
      '.top-right.abnormal-warning-monitor',
    );
    const abnormalHeader = document.querySelector('.top-right .header-actions');
    const abnormalCard = document.querySelector(
      '.top-right .abnormal-duration-card',
    );
...
onMounted(() => {
  ...
  faultRefreshTimer = setInterval(fetchFaultListData, 30_000);
  alertRefreshTimer = setInterval(fetchAlertEventListData, 30_000);
  hiddenDangerRefreshTimer = setInterval(fetchHiddenDangerListData, 30_000);
  complianceRefreshTimer = setInterval(fetchComplianceListData, 30_000);
  specialRefreshTimer = setInterval(fetchSpecialListData, 30_000);
  disposalRefreshTimer = setInterval(fetchDisposalProgressListData, 30_000);
  abnormalRefreshTimer = setInterval(fetchAbnormalWarningListData, 30_000);
  abnormalSummaryTimer = setInterval(fetchAbnormalDurationSummaryData, 30_000);
  disposalSummaryTimer = setInterval(
    fetchDisposalProgressDurationSummaryData,
    30_000,
  );
...
<template>
  <div class="page-container" ref="pageContainerRef">
    ...
    <div class="mainbox">
      <div class="top">
        <div
          class="panel top-left disposal-progress-tracking"
          ref="disposalProgressTracking"
        >
          <div class="header-actions">
            ...
            <div class="actions-right">
              <button
                ...
                @click="togglePanelFullscreen('disposalProgressTracking')"
              >
          ...
          <div class="progress-card-container">
            <div class="disposal-duration-item">
              ...
              <span class="item-value">{{
                disposalProgressDurationSummary.occurToDispatchDuration
              }}</span>
            </div>
            <div class="disposal-duration-item">
              <span class="item-label">派单到接单耗时</span>
              <span class="item-value">{{
                disposalProgressDurationSummary.dispatchToReceiveDuration
              }}</span>
            </div>
            <div class="disposal-duration-item">
              <span class="item-label">接单到处置耗时</span>
              <span class="item-value">{{
                disposalProgressDurationSummary.receiveToDisposalDuration
              }}</span>
            </div>
            <div class="disposal-duration-item">
              <span class="item-label">处置到反馈耗时</span>
              <span class="item-value">{{
                disposalProgressDurationSummary.disposalToFeedbackDuration
              }}</span>
            </div>
            <div class="disposal-duration-item">
              <span class="item-label">反馈到验收耗时</span>
              <span class="item-value">{{
                disposalProgressDurationSummary.feedbackToAcceptanceDuration
              }}</span>
            </div>
            <div class="disposal-duration-item">
              <span class="item-label">验收到办结耗时</span>
              <span class="item-value">{{
                disposalProgressDurationSummary.acceptanceToCloseDuration
              }}</span>
            </div>
            <div class="disposal-duration-item alertDisposalTotalDuration">
              <span class="item-label">预警处置总耗时</span>
              <span class="item-value">{{
                disposalProgressDurationSummary.alertDisposalTotalDuration
              }}</span>
            </div>
          </div>
          <div class="disposal-progress-table">
            <ElTable
              ref="disposalTableRef"
              :data="disposalProgressList"
              border
              size="small"
              :height="disposalTableMaxHeight"
              style="width: 100%; table-layout: fixed"
              :empty-text="
                disposalProgressList.length === 0
                  ? '暂无漳州地区处置进度数据'
                  : ''
              "
              :row-class-name="({ row }) => getDisposalProgressRowClass(row)"
            >
              ...
              >
                <template #default="scope">
                  <span
                    class="disposal-result-tag"
                    :class="getDisposalResultTagType(scope.row.disposalResult)"
                    >{{ scope.row.disposalResult }}</span
                  >
                </template>
              </el-table-column>
              <el-table-column
              ...
              >
                <template #default="scope">
                  <span class="maintain-user-id">{{
                    scope.row.maintainUserId
                  }}</span>
                </template>
              </el-table-column>
              <el-table-column
              ...
              >
                <template #default="scope">
                  <span
                    class="attachment-url-text"
                    @click="openAttachmentUrl(scope.row.attachmentUrl)"
        ...
        <div class="panel top-middle" ref="alertEventOverview">
          <div class="header-actions">
            ...
            <div class="actions-right">
              <button
                class="panel-fullscreen-btn"
                @click="togglePanelFullscreen('alertEventOverview')"
              >
                <el-icon color="#00ccff" size="16"><FullScreen /></el-icon>
              </button>
            </div>
          ...
          <div
            class="table-container alert-event-overview-table"
            style="box-sizing: border-box; flex: 1; width: 100%; min-height: 0"
          >
            <ElTable
              ref="alertTableRef"
              :data="alertEventList"
              border
              size="small"
              :max-height="alertTableMaxHeight"
              style="width: 100%; table-layout: fixed"
              :empty-text="
                alertEventList.length === 0 ? '暂无漳州地区预警事件数据' : ''
              "
              :row-class-name="
                ({ row }) => {
                  if (row.disposalStatus === '未处置')
                    return 'alert-row-untreated';
                  if (row.disposalStatus === '处置中')
                    return 'alert-row-disposing';
                  if (row.disposalStatus === '已办结')
                    return 'alert-row-completed';
            ...
            >
              ...
              >
                <template #default="scope">
                  <el-tag
                    size="small"
                    :class="`alert-level-tag ${scope.row.alertLevel === '紧急' ? 'red' : scope.row.alertLevel === '严重' ? 'orange' : scope.row.alertLevel === '一般' ? 'yellow' : 'blue'}`"
                  >
              ...
              >
                <template #default="scope">
                  <el-tag size="small" class="alert-type-tag">
                    {{ scope.row.alertType }}
                  </el-tag>
                </template>
              </el-table-column>
              ...
              >
                <template #default="scope">
                  <el-tag
                    :type="getDisposalStatusTagType(scope.row.disposalStatus)"
                    size="small"
                    :class="`disposal-status-tag ${scope.row.disposalStatus === '未处置' ? 'untreated' : scope.row.disposalStatus === '处置中' ? 'disposing' : scope.row.disposalStatus === '已办结' ? 'completed' : 'timeout'}`"
                  >
        ...
        <div
          class="panel top-right abnormal-warning-monitor"
          ref="abnormalWarning"
        >
          <div class="header-actions">
            ...
            <div class="actions-right">
              <button
                class="panel-fullscreen-btn"
                @click="togglePanelFullscreen('abnormalWarning')"
              >
                <el-icon color="#00ccff" size="16"><FullScreen /></el-icon>
              </button>
            </div>
          ...
          <div class="abnormal-duration-card">
            <div class="abnormal-duration-item">
              ...
              <span class="item-value">{{
                abnormalDurationSummary.avgAbnormalDuration
              }}</span>
            </div>
            <div class="abnormal-duration-item">
              <span class="item-label">最大异常时长</span>
              <span class="item-value">{{
                abnormalDurationSummary.maxAbnormalDuration
              }}</span>
            </div>
            <div class="abnormal-duration-item">
              <span class="item-label">最小异常时长</span>
              <span class="item-value">{{
                abnormalDurationSummary.minAbnormalDuration
              }}</span>
            </div>
            <div class="abnormal-duration-item totalAbnormalDuration">
              <span class="item-label">累计异常时长</span>
              <span class="item-value">{{
                abnormalDurationSummary.totalAbnormalDuration
              }}</span>
            </div>
          </div>
          <div class="abnormal-warning-table">
            <ElTable
              ...
              border
              size="small"
              :height="abnormalTableMaxHeight"
              style="width: 100%; table-layout: fixed"
              :empty-text="
                abnormalList.length === 0 ? '暂无漳州地区异常预警数据' : ''
              "
              :row-class-name="({ row }) => getAbnormalProgressRowClass(row)"
            >
              ...
              >
                <template #default="scope">
                  <span
                    ...
                    :class="getAbnormalTypeTagType(scope.row.abnormalType)"
                    >{{ scope.row.abnormalType }}</span
                  >
                </template>
              </el-table-column>
              <el-table-column
              ...
              >
                <template #default="scope">
                  <span class="influence-range-tag">{{
                    scope.row.influenceRange
                  }}</span>
                </template>
              </el-table-column>
              <el-table-column
              ...
              <el-table-column prop="areaCode" label="区域编码" min-width="100">
                <template #default="scope">
                  <span class="area-code-text">{{ scope.row.areaCode }}</span>
                </template>
              </el-table-column>
              <el-table-column prop="lotId" label="停车场ID" min-width="100">
                <template #default="scope">
                  <span class="lot-id-text">{{ scope.row.lotId }}</span>
                </template>
              </el-table-column>
            </ElTable>
          </div>
        ...
        <div class="panel bottom-left1" ref="hiddenDangerWarning">
          <div class="header-actions">
            ...
            <div class="actions-right">
              <button
                class="panel-fullscreen-btn"
                @click="togglePanelFullscreen('hiddenDangerWarning')"
              >
                <el-icon color="#00ccff" size="16"><FullScreen /></el-icon>
              </button>
            </div>
          ...
          <div
            class="table-container hidden-danger-warning-table"
            style="box-sizing: border-box; flex: 1; width: 100%; min-height: 0"
          >
            <ElTable
              ref="hiddenDangerTableRef"
              :data="hiddenDangerList"
              border
              size="small"
              :max-height="hiddenDangerTableMaxHeight"
              style="width: 100%; table-layout: fixed"
              :empty-text="
                hiddenDangerList.length === 0 ? '暂无漳州地区隐患预警数据' : ''
              "
              :row-class-name="
                ({ row }) => {
                  if (row.rectificationStatus === '未整改')
                    return 'hidden-danger-row-untreated';
                  if (row.rectificationStatus === '整改中')
                    return 'hidden-danger-row-rectifying';
                  if (row.rectificationStatus === '已完成')
                    return 'hidden-danger-row-completed';
                  if (row.rectificationStatus === '验收通过')
                    return 'hidden-danger-row-accepted';
                  return '';
                }
              "
            >
              ...
              >
                <template #default="scope">
                  <el-tag
                    size="small"
                    :type="
                      getHiddenDangerLevelTagType(scope.row.hiddenDangerLevel)
                    "
                    :class="`hidden-danger-level-tag ${scope.row.hiddenDangerLevel === '一般' ? 'general' : scope.row.hiddenDangerLevel === '较大' ? 'larger' : 'major'}`"
                  >
                    {{ scope.row.hiddenDangerLevel }}
                  </el-tag>
                </template>
              </el-table-column>
              <el-table-column
              ...
              >
                <template #default="scope">
                  <el-tag size="small" class="hidden-danger-type-tag">
                    {{ scope.row.hiddenDangerType }}
                  </el-tag>
                </template>
              </el-table-column>
              <el-table-column
              ...
              >
                <template #default="scope">
                  <el-tag
                    :type="
                      getRectificationStatusTagType(
                        scope.row.rectificationStatus,
                      )
                    "
                    size="small"
                    :class="`rectification-status-tag ${scope.row.rectificationStatus === '未整改' ? 'untreated' : scope.row.rectificationStatus === '整改中' ? 'rectifying' : scope.row.rectificationStatus === '已完成' ? 'completed' : 'accepted'}`"
                  >
                    {{ scope.row.rectificationStatus }}
                  </el-tag>
                </template>
        ...
        <div class="panel bottom-left2" ref="faultwarning">
          <div class="header-actions">
            ...
            <div class="actions-right">
              <button
                class="panel-fullscreen-btn"
                @click="togglePanelFullscreen('faultwarning')"
              >
                <el-icon color="#00ccff" size="16"><FullScreen /></el-icon>
              </button>
            </div>
          ...
          <div
            class="table-container fault-warning-table"
            style="box-sizing: border-box; flex: 1; width: 100%; min-height: 0"
          >
            <ElTable
              ref="faultTableRef"
              ...
              border
              size="small"
              :max-height="faultTableMaxHeight"
              style="width: 100%; table-layout: fixed"
              :empty-text="
                faultList.length === 0 ? '暂无漳州地区故障预警数据' : ''
              "
              :row-class-name="
                ({ row }) => {
                  if (row.maintenanceProgress === '未维修')
                    return 'fault-row-danger';
                  if (row.maintenanceProgress === '维修中')
                    return 'fault-row-warning';
                  if (row.maintenanceProgress === '已修复')
                    return 'fault-row-success';
                  return '';
                }
              "
            >
              ...
              >
                <template #default="scope">
                  <el-tag size="small" class="fault-type-tag">
                    {{ scope.row.faultType }}
                  </el-tag>
                </template>
              </el-table-column>
              ...
              />
              <el-table-column
                prop="affectedUserCount"
                label="受影响用户数"
                min-width="100"
              >
                <template #default="scope">
                  <span class="user-count">{{
                    scope.row.affectedUserCount
                  }}</span>
                </template>
              </el-table-column>
              <el-table-column
                prop="maintenanceProgress"
                label="维修进度"
                min-width="130"
              >
                <template #default="scope">
                  <el-tag
                    :type="getProgressTagType(scope.row.maintenanceProgress)"
                    size="small"
                    class="progress-tag"
                  >
                    {{ scope.row.maintenanceProgress }}
                  </el-tag>
                </template>
              </el-table-column>
            </ElTable>
        ...
        <div class="panel bottom-right1" ref="complianceWarning">
          <div class="header-actions">
            ...
            <div class="actions-right">
              <button
                class="panel-fullscreen-btn"
                @click="togglePanelFullscreen('complianceWarning')"
              >
                <el-icon color="#00ccff" size="16"><FullScreen /></el-icon>
              </button>
            </div>
          ...
          <div
            class="table-container compliance-warning-table"
            style="box-sizing: border-box; flex: 1; width: 100%; min-height: 0"
          >
            <ElTable
              ...
              border
              size="small"
              :max-height="complianceTableMaxHeight"
              style="width: 100%; table-layout: fixed"
              :empty-text="
                complianceList.length === 0 ? '暂无漳州地区合规预警数据' : ''
              "
              :row-class-name="
                ({ row }) => {
                  if (row.rectificationStatus === '未整改')
                    return 'compliance-row-untreated';
                  if (row.rectificationStatus === '整改中')
                    return 'compliance-row-rectifying';
                  if (row.rectificationStatus === '已完成')
                    return 'compliance-row-completed';
                  return '';
                }
              "
            >
              <el-table-column
                prop="complianceWarningId"
                label="合规预警ID"
                min-width="130"
              >
                <template #default="scope">
                  <span
                    class="link-text"
                    @click="openDetailDrawer(scope.row)"
                    >{{ scope.row.complianceWarningId }}</span
                  >
                </template>
              </el-table-column>
              <el-table-column
              ...
              >
                <template #default="scope">
                  <el-tag size="small" class="violation-detail-tag">
                    {{ scope.row.violationDetail }}
                  </el-tag>
                </template>
              </el-table-column>
              <el-table-column
                prop="responsibleSubject"
                label="责任主体"
                min-width="100"
              >
                <template #default="scope">
                  <span class="responsible-subject">{{
                    scope.row.responsibleSubject
                  }}</span>
                </template>
              </el-table-column>
              <el-table-column
              ...
              >
                <template #default="scope">
                  <el-tag
                    :type="
                      getComplianceRectificationStatusTagType(
                        scope.row.rectificationStatus,
                      )
                    "
                    size="small"
                    class="rectification-status-tag"
                  >
                    {{ scope.row.rectificationStatus }}
                  </el-tag>
                </template>
        ...
        <div class="panel bottom-right2" ref="specialWarning">
          <div class="header-actions">
            ...
            <div class="actions-right">
              <button
                class="panel-fullscreen-btn"
                @click="togglePanelFullscreen('specialWarning')"
              >
                <el-icon color="#00ccff" size="16"><FullScreen /></el-icon>
              </button>
            </div>
          ...
          <div
            class="table-container special-warning-table"
            style="box-sizing: border-box; flex: 1; width: 100%; min-height: 0"
          >
            <ElTable
              ref="specialTableRef"
              ...
              border
              size="small"
              :max-height="specialTableMaxHeight"
              style="width: 100%; table-layout: fixed"
              :empty-text="
                specialList.length === 0 ? '暂无漳州地区专项预警数据' : ''
              "
              :row-class-name="
                ({ row }) => {
                  if (row.disposalStatus === '未处置')
                    return 'special-row-untreated';
                  if (row.disposalStatus === '处置中')
                    return 'special-row-disposing';
                  if (row.disposalStatus === '已办结')
                    return 'special-row-completed';
                  if (row.disposalStatus === '超时')
                    return 'special-row-timeout';
                  return '';
                }
              "
            >
              <el-table-column
                prop="specialWarningId"
                label="专项预警ID"
                min-width="130"
              >
                <template #default="scope">
                  <span
                    class="link-text"
                    @click="openDetailDrawer(scope.row)"
                    >{{ scope.row.specialWarningId }}</span
                  >
                </template>
              </el-table-column>
              <el-table-column
              ...
              >
                <template #default="scope">
                  <el-tag
                    :type="getWarningSceneTagType(scope.row.warningScene)"
                    size="small"
                    class="special-scene-tag"
                  >
                    {{ scope.row.warningScene }}
                  </el-tag>
                </template>
              </el-table-column>
              <el-table-column
              ...
              >
                <template #default="scope">
                  <el-tag size="small" class="special-alert-type-tag">
                    {{ scope.row.alertType }}
                  </el-tag>
                </template>
              </el-table-column>
      ...
      <div class="detail-content" v-if="currentDetailData">
        <div v-if="currentDetailData && 'faultId' in currentDetailData">
          ...
          <div class="detail-item">
            <label>故障ID：</label
            >{{ (currentDetailData as FaultListItem).faultId }}
          </div>
          <div class="detail-item">
            <label>故障类型：</label
            >{{ (currentDetailData as FaultListItem).faultType }}
          </div>
          <div class="detail-item">
            <label>故障码：</label
            >{{ (currentDetailData as FaultListItem).faultCode }}
          </div>
          <div class="detail-item">
            <label>区域编码：</label
            >{{ (currentDetailData as FaultListItem).areaCode }}
          </div>
          <div class="detail-item">
            <label>停车场ID：</label
            >{{ (currentDetailData as FaultListItem).lotId }}
          </div>
          <div class="detail-item">
            <label>设备类型：</label
            >{{ (currentDetailData as FaultListItem).deviceType }}
          </div>
          <ElDivider content-position="left">详细信息</ElDivider>
          <div class="detail-item long-text">
            <label>故障位置：</label
            >{{ (currentDetailData as FaultListItem).faultLocation }}
          </div>
          <div class="detail-item long-text">
            <label>影响范围：</label
            >{{ (currentDetailData as FaultListItem).influenceRange }}
          </div>
          <div class="detail-item">
            <label>受影响用户数：</label
            >{{ (currentDetailData as FaultListItem).affectedUserCount }} 人
          </div>
          <div class="detail-item">
            <label>维修责任人ID：</label
            >{{ (currentDetailData as FaultListItem).maintainUserId }}
          </div>
          <div class="detail-item">
            <label>维修截止时间：</label
            >{{ (currentDetailData as FaultListItem).maintenanceDeadline }}
          </div>
          <div class="detail-item">
            <label>维修进度：</label
            >{{ (currentDetailData as FaultListItem).maintenanceProgress }}
          </div>
        </div>
        ...
        <div
          v-else-if="
            currentDetailData &&
            'alertId' in currentDetailData &&
            !('disposalMeasure' in currentDetailData)
          "
        >
          <ElDivider content-position="left">基本信息</ElDivider>
          <div class="detail-item">
            <label>预警ID：</label
            >{{ (currentDetailData as AlertEventOverviewItem).alertId }}
          </div>
          <div class="detail-item">
            <label>预警等级：</label
            >{{ (currentDetailData as AlertEventOverviewItem).alertLevel }}
          </div>
          <div class="detail-item">
            <label>预警类型：</label
            >{{ (currentDetailData as AlertEventOverviewItem).alertType }}
          </div>
          <div class="detail-item">
            <label>区域编码：</label
            >{{ (currentDetailData as AlertEventOverviewItem).areaCode }}
          </div>
          <div class="detail-item">
            <label>停车场ID：</label
            >{{ (currentDetailData as AlertEventOverviewItem).lotId }}
          </div>
          <div class="detail-item">
            <label>路侧泊位ID：</label
            >{{ (currentDetailData as AlertEventOverviewItem).roadsideId }}
          </div>
          <div class="detail-item">
            <label>设备ID：</label
            >{{ (currentDetailData as AlertEventOverviewItem).deviceId }}
          </div>
          <ElDivider content-position="left">详细信息</ElDivider>
          <div class="detail-item long-text">
            <label>预警内容：</label
            >{{ (currentDetailData as AlertEventOverviewItem).alertContent }}
          </div>
          <div class="detail-item">
            <label>发生时间：</label
            >{{ (currentDetailData as AlertEventOverviewItem).occurTime }}
          </div>
          <div class="detail-item">
            <label>责任人ID：</label
            >{{ (currentDetailData as AlertEventOverviewItem).maintainUserId }}
          </div>
          <div class="detail-item">
            <label>责任人姓名：</label
            >{{ (currentDetailData as AlertEventOverviewItem).userName }}
          </div>
          <div class="detail-item">
            <label>处置状态：</label
            >{{ (currentDetailData as AlertEventOverviewItem).disposalStatus }}
          </div>
        </div>
        ...
        <div
          ...
          "
        >
          <ElDivider content-position="left">基础业务信息</ElDivider>
          <div class="detail-item">
            <label>预警ID：</label
            >{{ (currentDetailData as DisposalProgressTrackingItem).alertId }}
          </div>
          <div class="detail-item">
            <label>责任人ID：</label
            >{{
              (currentDetailData as DisposalProgressTrackingItem).maintainUserId
            }}
          </div>
          <div class="detail-item">
            <label>处置结果：</label
            >{{
              (currentDetailData as DisposalProgressTrackingItem).disposalResult
            }}
          </div>
          <ElDivider content-position="left">全流程时间节点</ElDivider>
          <div class="detail-item">
            <label>发生时间：</label
            >{{ (currentDetailData as DisposalProgressTrackingItem).occurTime }}
          </div>
          <div class="detail-item">
            <label>派单时间：</label
            >{{
              (currentDetailData as DisposalProgressTrackingItem).dispatchTime
            }}
          </div>
          <div class="detail-item">
            <label>接单时间：</label
            >{{
              (currentDetailData as DisposalProgressTrackingItem).receiveTime
            }}
          </div>
          <div class="detail-item">
            <label>处置时间：</label
            >{{
              (currentDetailData as DisposalProgressTrackingItem).disposalTime
            }}
          </div>
          <div class="detail-item">
            <label>反馈时间：</label
            >{{
              (currentDetailData as DisposalProgressTrackingItem).feedbackTime
            }}
          </div>
          <div class="detail-item">
            <label>验收时间：</label
          ...
          <div class="detail-item">
            <label>办结时间：</label
            >{{ (currentDetailData as DisposalProgressTrackingItem).closeTime }}
          </div>
          <div class="detail-item">
            <label>更新时间：</label
            >{{
              (currentDetailData as DisposalProgressTrackingItem).updateTime
            }}
          </div>
          <ElDivider content-position="left">详细处置信息</ElDivider>
          <div class="detail-item long-text">
            <label>预警内容：</label
            >{{
              (currentDetailData as DisposalProgressTrackingItem).alertContent
            }}
          </div>
          <div class="detail-item long-text">
            <label>处置措施：</label
            >{{
              (currentDetailData as DisposalProgressTrackingItem)
                .disposalMeasure
            }}
          </div>
          <div class="detail-item">
        ...
        <div
          v-else-if="currentDetailData && 'hiddenDangerId' in currentDetailData"
        >
          <ElDivider content-position="left">基础信息</ElDivider>
          <div class="detail-item">
            <label>隐患ID：</label
            >{{ (currentDetailData as HiddenDangerWarningItem).hiddenDangerId }}
          </div>
          <div class="detail-item">
            <label>风险等级：</label
            >{{
              (currentDetailData as HiddenDangerWarningItem).hiddenDangerLevel
            }}
          </div>
          <div class="detail-item">
            <label>隐患类型：</label
            >{{
              (currentDetailData as HiddenDangerWarningItem).hiddenDangerType
            }}
          </div>
          <div class="detail-item">
            <label>区域编码：</label
            >{{ (currentDetailData as HiddenDangerWarningItem).areaCode }}
          </div>
          <div class="detail-item">
            <label>停车场ID：</label
            >{{ (currentDetailData as HiddenDangerWarningItem).lotId }}
          </div>
          <div class="detail-item">
            <label>路侧泊位ID：</label
            >{{ (currentDetailData as HiddenDangerWarningItem).roadsideId }}
          </div>
          <ElDivider content-position="left">详细信息</ElDivider>
          <div class="detail-item long-text">
            <label>影响范围：</label
            >{{ (currentDetailData as HiddenDangerWarningItem).influenceRange }}
          </div>
          <div class="detail-item long-text">
            <label>整改要求：</label
            >{{
              (currentDetailData as HiddenDangerWarningItem)
          ...
          <div class="detail-item">
            <label>责任单位：</label
            >{{
              (currentDetailData as HiddenDangerWarningItem).responsibleUnit
            }}
          </div>
          <div class="detail-item">
            <label>责任人：</label
            >{{
              (currentDetailData as HiddenDangerWarningItem).responsiblePerson
            }}
          </div>
          <div class="detail-item long-text">
            <label>整改措施：</label
          ...
          <div class="detail-item">
            <label>整改进度：</label
            >{{
              (currentDetailData as HiddenDangerWarningItem).rectificationStatus
            }}
          </div>
        </div>
        ...
        <div
          v-else-if="
            currentDetailData && 'complianceWarningId' in currentDetailData
          "
        >
          <ElDivider content-position="left">基础信息</ElDivider>
          <div class="detail-item">
            <label>合规预警ID：</label
            >{{
              (currentDetailData as ComplianceWarningItem).complianceWarningId
            }}
          </div>
          <div class="detail-item">
            <label>责任主体：</label
            >{{
              (currentDetailData as ComplianceWarningItem).responsibleSubject
            }}
          </div>
          <div class="detail-item">
            <label>区域编码：</label
            >{{ (currentDetailData as ComplianceWarningItem).areaCode }}
          </div>
          <div class="detail-item">
            <label>停车场ID：</label
            >{{ (currentDetailData as ComplianceWarningItem).lotId }}
          </div>
          <div class="detail-item">
            <label>预警时间：</label
            >{{ (currentDetailData as ComplianceWarningItem).warningTime }}
          </div>
          <ElDivider content-position="left">合规详情信息</ElDivider>
          <div class="detail-item long-text">
            <label>违规详情：</label
            >{{ (currentDetailData as ComplianceWarningItem).violationDetail }}
          </div>
          <div class="detail-item long-text">
            <label>合规标准：</label
            >{{
              (currentDetailData as ComplianceWarningItem).complianceStandard
            }}
          </div>
          <div class="detail-item long-text">
            <label>整改要求：</label
            >{{
              (currentDetailData as ComplianceWarningItem)
                .rectificationRequirement
            }}
          </div>
          <div class="detail-item long-text">
            <label>处罚建议：</label
            >{{
              (currentDetailData as ComplianceWarningItem).punishmentSuggestion
            }}
          </div>
          <div class="detail-item">
            <label>整改期限：</label
            >{{
              (currentDetailData as ComplianceWarningItem).rectificationDeadline
            }}
          </div>
          <div class="detail-item">
            <label>整改状态：</label
            >{{
              (currentDetailData as ComplianceWarningItem).rectificationStatus
            }}
          </div>
        </div>
        ...
        <div
          v-else-if="
            currentDetailData && 'specialWarningId' in currentDetailData
          "
        >
          <ElDivider content-position="left">基础信息</ElDivider>
          <div class="detail-item">
            <label>专项预警ID：</label
            >{{ (currentDetailData as SpecialWarningItem).specialWarningId }}
          </div>
          <div class="detail-item">
            <label>预警场景：</label
            >{{ (currentDetailData as SpecialWarningItem).warningScene }}
          </div>
          <div class="detail-item">
            <label>预警类型：</label
            >{{ (currentDetailData as SpecialWarningItem).alertType }}
          </div>
          <div class="detail-item">
            <label>区域编码：</label
            >{{ (currentDetailData as SpecialWarningItem).areaCode }}
          </div>
          <div class="detail-item">
            <label>停车场ID：</label
            >{{ (currentDetailData as SpecialWarningItem).lotId }}
          </div>
          <div class="detail-item">
            <label>路侧泊位ID：</label
            >{{ (currentDetailData as SpecialWarningItem).roadsideId }}
          </div>
          <div class="detail-item">
            <label>设备ID：</label
            >{{ (currentDetailData as SpecialWarningItem).deviceId }}
          </div>
          <ElDivider content-position="left">详细信息</ElDivider>
          <div class="detail-item long-text">
            <label>预警内容：</label
            >{{ (currentDetailData as SpecialWarningItem).alertContent }}
          </div>
          <div class="detail-item">
            <label>发生时间：</label
            >{{ (currentDetailData as SpecialWarningItem).occurTime }}
          </div>
          <div class="detail-item">
            <label>处置状态：</label
            >{{ (currentDetailData as SpecialWarningItem).disposalStatus }}
          </div>
          <div class="detail-item long-text">
            <label>处置措施：</label
            >{{ (currentDetailData as SpecialWarningItem).disposalMeasure }}
          </div>
          <div class="detail-item">
            <label>关联视频：</label>
            <span
              ...
              @click="
                openVideoUrl(
                  (currentDetailData as SpecialWarningItem)?.videoUrl,
                )
              "
            >
        ...
        <div v-else-if="currentDetailData && 'abnormalId' in currentDetailData">
          <ElDivider content-position="left">基础异常信息</ElDivider>
          <div class="detail-item">
            <label>异常ID：</label
            >{{ (currentDetailData as AbnormalWarningItem).abnormalId }}
          </div>
          <div class="detail-item">
            <label>异常类型：</label
            >{{ (currentDetailData as AbnormalWarningItem).abnormalType }}
          </div>
          <div class="detail-item">
            <label>区域编码：</label
            >{{ (currentDetailData as AbnormalWarningItem).areaCode }}
          </div>
          <div class="detail-item">
            <label>停车场ID：</label
            >{{ (currentDetailData as AbnormalWarningItem).lotId }}
          </div>
          <ElDivider content-position="left">异常详细信息</ElDivider>
          <div class="detail-item long-text">
            <label>异常原因：</label
            >{{ (currentDetailData as AbnormalWarningItem).abnormalReason }}
          </div>
          <div class="detail-item long-text">
            <label>影响范围：</label
            >{{ (currentDetailData as AbnormalWarningItem).influenceRange }}
          </div>
          <div class="detail-item long-text">
            <label>解除原因：</label
            >{{ (currentDetailData as AbnormalWarningItem).relieveReason }}
          </div>
          <div class="detail-item">
            <label>解除时间：</label
            >{{ (currentDetailData as AbnormalWarningItem).relieveTime }}
          </div>
        </div>
      </div>
    </ElDrawer>
