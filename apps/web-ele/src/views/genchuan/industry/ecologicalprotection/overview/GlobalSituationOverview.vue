...
} from '#/api/genchuan/industry/ecologicalprotection/overview/GlobalSituationOverview.js';
...
const viewAnalysisReport = async (reportId) => {
  try {
    currentReport.value = await viewEcoAnalysisReport(reportId);
...
const refreshCoreIndicators = async () => {
  try {
    coreIndicators.value = await fetchEcoCoreIndicators();
