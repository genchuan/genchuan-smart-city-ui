...
const [RechargeForm, rechargeFormApi] = useVbenForm({
  ...
  schema: [
    ...
    },
    {
      ...
      rules: '{{ $formValues.rechargeType === "custom" ? "required" : undefined }}'
    },
    {
      fieldName: 'payType',
      label: '支付方式',
...
const getTableData = (pageObj) => {
  ...
  const filteredList = dataObj.apilist.filter((v) => {
    ...
    const fieldMatch = (
      ...
      (!filterRechargePackage.value || v.rechargePackage === filterRechargePackage.value) &&
      (!filterPayType.value || v.payType === filterPayType.value) &&
      (!filterUserName.value || v.userName === filterUserName.value) &&
      (!filterPromotionType.value || v.promotionType === filterPromotionType.value) &&
      (!filterApplicablePackage.value || v.applicablePackage === filterApplicablePackage.value) &&
...
const handleFieldClick = (field, value) => {
  switch (field) {
    ...
    case 'rechargePackage':
      filterRechargePackage.value = filterRechargePackage.value === value ? '' : value;
      break;
    case 'payType':
    ...
    case 'userName':
      ...
      break;
    case 'promotionType':
      filterPromotionType.value = filterPromotionType.value === value ? '' : value;
      break;
    case 'applicablePackage':
      filterApplicablePackage.value = filterApplicablePackage.value === value ? '' : value;
      break;
    case 'packageType':
...
const createLabel = (item) => {
  ...
  if (props.tabName === '储值卡管理') {
    ...
    switch (item.label) {
      ...
      case '处理中': {
        count = dataObj.apilist.filter((v) => v.rechargeStatus === '处理中').length;
        break;
      }
      case '成功': {
        count = dataObj.apilist.filter((v) => v.rechargeStatus === '成功').length;
        break;
      }
      case '失败': {
        count = dataObj.apilist.filter((v) => v.rechargeStatus === '失败').length;
        break;
      }
      // No default
    }
...
const handleDisable = async (row) => {
  try {
    const confirmMessage = props.tabName === '充值优惠管理' ? '确定要关闭活动吗？' : '确定要禁用该卡吗？';
...
<template>
  <div class="park-lot-table-new">
    ...
    <DetailDrawer
      ref="detailDrawerRef"
      :title="props.tabName === '储值卡管理' ? `${dataObj.detailObj.prepaidCardId}详情` : props.tabName === '充值优惠管理' ? `${dataObj.detailObj.promotionId}详情` : `${dataObj.detailObj.rechargeNo}详情`"
    ...
    <Grid>
      ...
      <template #cardStatus="{ row }">
        <el-tag :type="row.cardStatus === '正常' ? 'success' : row.cardStatus === '未激活' ? 'warning' : row.cardStatus === '已过期' ? 'info' : row.cardStatus === '已禁用' ? 'danger' : 'danger'">
          {{ row.cardStatus }}
        </el-tag>
      </template>
      ...
      <template #actions="{ row }">
        <div class="table-toolbar-tools">
          ...
          <IconButton
            v-if="props.tabName === '充值订单' || props.tabName === '充值优惠管理'"
            content="编辑"
            icon-name="edit"
          ...
          <IconButton
            v-if="props.tabName === '充值优惠管理' || props.tabName === '储值卡管理'"
            content="禁用"
            icon-name="Lock"
      ...
      <template #bottom>
        <div class="common-total" @click="changeTotalShow">
          ...
          <span v-if="props.tabName === '储值卡管理'">
            ...
            {{ dataObj.list.filter((v) => v.cardStatus === '正常').length }}; 未激活:
            {{ dataObj.list.filter((v) => v.cardStatus === '未激活').length }}; 已过期:
            {{ dataObj.list.filter((v) => v.cardStatus === '已过期').length }}; 已禁用:
            {{ dataObj.list.filter((v) => v.cardStatus === '已禁用').length }}
          </span>
          <span v-else-if="props.tabName === '充值优惠管理'">
            本页统计：充值优惠数量: {{ dataObj.list.length }}; 进行中:
            {{ dataObj.list.filter((v) => v.status === '进行中').length }}; 未开始:
            {{ dataObj.list.filter((v) => v.status === '未开始').length }}; 已结束:
            {{ dataObj.list.filter((v) => v.status === '已结束').length }}
          </span>
          <span v-else>
            本页统计：充值记录数量: {{ dataObj.list.length }}; 成功:
            {{ dataObj.list.filter((v) => v.rechargeStatus === '成功').length }}; 失败:
