...
} from '#/api/genchuan/educationTeaching/studentMgmt/decisionAnalysis/dormCompareReport/form.js';
...
const getTableData = async ({ page }) => {
  ...
  try {
    ...
    const res = await getDormCompareReportPage(params);
...
async function handleBatchExport() {
  ...
  try {
    ...
    try {
      const data = await exportDormCompareReport({ ids: checkedIds.value });
...
async function handleExport(row) {
  try {
    const loading = ElLoading.service({ text: '正在导出...' });
    try {
      const data = await exportDormCompareReport({ ids: [row.id] });
      downloadFileFromBlobPart({ fileName: `${row.timeScale}_宿舍报表_${formatTimestamp(row.statStartTime)}.xls`, source: data });
      ElMessage.success('导出成功');
    } finally {
      loading.close();
    }
...
const [GenerateForm, generateFormApi] = useVbenForm({
  ...
  handleSubmit: async (values) => {
    ...
    try {
      const res = await createDormCompareReport(values);
      if (res === true) {
