...
const open = async (type: string, id?: number) => {
  ...
  dialogTitle.value = type === 'create' ? '新增事件统计分析' : '编辑事件统计分析'
  formType.value = type
  resetForm()
  // 修改时，设置数据
  if (id) {
  ...
  <ElDialog :title="dialogTitle" v-model="dialogVisible" width="600px" append-to-body>
    ...
    >
      <ElFormItem label="统计周期" prop="statisticalCycle">
        <ElInput v-model="formData.statisticalCycle" placeholder="请输入统计周期" />
      </ElFormItem>
      ...
      <ElFormItem label="事件数量" prop="numberOfEvents">
        <ElInput v-model="formData.numberOfEvents" placeholder="请输入事件数量" />
      </ElFormItem>
      ...
      <ElFormItem label="高发时段" prop="highIncidencePeriod">
        <ElInput v-model="formData.highIncidencePeriod" placeholder="请输入高发时段" />
      </ElFormItem>
      ...
      <ElFormItem label="处理成功率" prop="processingSuccessRate">
        <ElInput v-model="formData.processingSuccessRate" placeholder="请输入处理成功率" />
      </ElFormItem>
      <ElFormItem label="严重程度分布" prop="severityDistribution">
        <ElInput v-model="formData.severityDistribution" placeholder="请输入严重程度分布" />
      </ElFormItem>
      <ElFormItem label="涉及部门" prop="involvedDepartments">
        <ElInput v-model="formData.involvedDepartments" placeholder="请输入涉及部门" />
      </ElFormItem>
      <ElFormItem label="责任主体" prop="responsibleParty">
        <ElInput v-model="formData.responsibleParty" placeholder="请输入责任主体" />
      </ElFormItem>
      <ElFormItem label="处理方式占比" prop="proportionOfProcessing">
        <ElInput v-model="formData.proportionOfProcessing" placeholder="请输入处理方式占比" />
      </ElFormItem>
      <ElFormItem label="资源消耗统计" prop="resourceConsumption">
        <ElInput v-model="formData.resourceConsumption" placeholder="请输入资源消耗统计" />
      </ElFormItem>
      <ElFormItem label="不同渠道上报占比" prop="proportionOfReporting">
        <ElInput v-model="formData.proportionOfReporting" placeholder="请输入不同渠道上报占比" />
      </ElFormItem>
      <ElFormItem label="重复事件数量" prop="numberOfRepeatedEvents">
        <ElInput v-model="formData.numberOfRepeatedEvents" placeholder="请输入重复事件数量" />
      </ElFormItem>
      <ElFormItem label="时间序列趋势" prop="timeSeriesTrend">
        <ElInput v-model="formData.timeSeriesTrend" placeholder="请输入时间序列趋势" />
      </ElFormItem>
      <ElFormItem label="关联部件故障次数" prop="numberOfRelated">
        <ElInput v-model="formData.numberOfRelated" placeholder="请输入关联部件故障次数" />
      </ElFormItem>
