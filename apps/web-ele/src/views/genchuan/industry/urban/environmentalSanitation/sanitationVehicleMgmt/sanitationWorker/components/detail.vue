...
<template>
  <DetailDrawer :title="drawerTitle">
    <div class="detail-card">
      ...
      <div class="detail-row"><span class="label">人员姓名：</span>{{ detailObj.userName || detailObj.toiletName || '-' }}</div>
      <div class="detail-row"><span class="label">岗位类型：</span>{{ detailObj.jobTypeName || detailObj.location || '-' }}</div>
      <div class="detail-row"><span class="label">所属班组：</span>{{ detailObj.teamName || '-' }}</div>
      <div class="detail-row"><span class="label">负责区域：</span>{{ detailObj.areaName || detailObj.area || '-' }}</div>
      <div class="detail-row"><span class="label">联系方式：</span>{{ detailObj.phone || '-' }}</div>
      <div class="detail-row"><span class="label">入职时间：</span>{{ detailObj.entryTime ? new Date(detailObj.entryTime).toLocaleDateString() : (detailObj.openHours || '-') }}</div>
      <div class="detail-row"><span class="label">人员状态：</span>{{ detailObj.personStatusName || detailObj.status || '-' }}</div>

      <!-- 接口数据特有字段 -->
      <template v-if="isApiData">
        <div class="detail-row"><span class="label">累计考勤天数：</span>{{ detailObj.totalAttendanceDays ?? '-' }}</div>
        <div class="detail-row"><span class="label">平均考核得分：</span>{{ detailObj.averageScore ?? '-' }}分</div>
        <div class="detail-row"><span class="label">作业完成率：</span>{{ detailObj.workCompletionRate ?? '-' }}%</div>
        <div class="detail-row"><span class="label">未完成任务数：</span>{{ detailObj.unfinishedTaskCount ?? '-' }}</div>
        <div class="detail-row"><span class="label">全勤率：</span>{{ detailObj.fullAttendanceRate ?? '-' }}%</div>
        <div class="detail-row"><span class="label">所属部门：</span>{{ detailObj.deptName || '-' }}</div>
        <div class="detail-row"><span class="label">角色：</span>{{ detailObj.roleName || '-' }}</div>
        <div class="detail-row"><span class="label">技能标签：</span>{{ detailObj.skillTags || '-' }}</div>
        <div class="detail-row"><span class="label">最近作业轨迹：</span>{{ detailObj.lastWorkTrace || '-' }}</div>
      </template>
