...
import {
  ...
  deleteRiverBatch,
  exportRiverExcel,
} from '#/api/genchuan/industry/urban/environmentalSanitation/sanitationSceneMgmt/riverEnvironment/data.js';

// 导入原有模拟数据（用于非“全部”标签页）
import { dataList } from '#/api/genchuan/industry/urban/environmentalSanitation/sanitationSceneMgmt/riverEnvironment/data.js';

// 导入表单配置（新增了接口相关的 schema）
import {
  textObj,
  useFormSchema,                         // 原有表单（模拟数据）
  useRiverEditSchema,                     // 新增：接口表单
  useRiverSearchSchema,                   // 新增：搜索表单
  getColumnsByStatus,
  getAreaOptions,
  getUserOptions,
  ...
  getMonitorStatusOptions,
  getProblemTypeOptions,
  getHandleStatusOptions,
  getDeptOptions,
  getToolOptions,
} from '#/api/genchuan/industry/urban/environmentalSanitation/sanitationSceneMgmt/riverEnvironment/form.js';
...
const counts = ref({
  ...
  statusCounts: {
    ...
    监测待执行: mockDataList.filter(v => v.status === '监测待执行').length,
...
// 映射 ID 到 label（通用函数，用于数组）
const mapIdsToLabels = (ids, options) => {
  if (!ids || !options) return [];
  return ids.map(id => {
    const idStr = id ? String(id).trim() : '';
    const found = options.find(opt => {
      const optVal = opt.value ? String(opt.value).trim() : '';
      return optVal === idStr;
    });
...
// 单独处理 deptId（单个值）
const getDeptLabel = (deptId, options) => {
  if (!deptId || !options) return deptId || '';
  const idStr = String(deptId).trim();
  const found = options.find(opt => {
    const optVal = opt.value ? String(opt.value).trim() : '';
    return optVal === idStr;
  });
  return found ? found.label : deptId;
};

function convertRiverItem(item) {
  // 解析 JSON 数组字段
  const parseJSON = (str) => {
    if (!str) return [];
    try {
  ...
  const staffIds = item.staffIds ? parseJSON(item.staffIds) : [];
  const toolIds = item.toolIds ? parseJSON(item.toolIds) : [];
  const monitorIndicators = item.monitorIndicators ? parseJSON(item.monitorIndicators) : [];
  const problemMediaUrl = item.problemMediaUrl ? parseJSON(item.problemMediaUrl) : [];

  // 生成映射后的标签
  const staffLabels = mapIdsToLabels(staffIds, loadedOptions.user);
  const toolLabels = mapIdsToLabels(toolIds, loadedOptions.tool);
  const deptLabel = getDeptLabel(item.deptId, loadedOptions.dept);
  ...
  return {
    ...
    planMonitorTime: item.planMonitorTime ? new Date(item.planMonitorTime).toLocaleString() : '-',
    lastMonitorTime: item.lastMonitorTime ? new Date(item.lastMonitorTime).toLocaleString() : '-',
    nextMonitorRemindTime: item.nextMonitorRemindTime ? new Date(item.nextMonitorRemindTime).toLocaleString() : '-',
...
const getTableData = async ({ page }) => {
  if (isApiTab.value) {
    ...
    try {
      const res = await getRiverPage(params);
      const listData = res.data?.list || res.list || [];
      const total = res.data?.total || res.total || 0;
      counts.value.total = total; // 更新计数
      dataObj.total = total;
      dataObj.list = listData.map(convertRiverItem);
      return dataObj;
    } catch (error) {
...
const [ApiEditDrawer, apiEditDrawerApi] = useVbenDrawer({
  ...
  async onConfirm() {
    ...
    const arrayFields = ['staffIds', 'toolIds', 'monitorIndicators', 'problemMediaUrl'];
    ...
    try {
      if (isAdd) {
        ...
        delete originalData.cleaningTypeName;
        delete originalData.staffsName;
        delete originalData.toolsName;
        delete originalData.monitorTypeName;
        delete originalData.monitorName;
        delete originalData.monitorStatusName;
        delete originalData.problemTypeName;
        delete originalData.reportName;
        delete originalData.deptName;
        delete originalData.handleName;
        delete originalData.handleStatusName;
...
async function handleDeleteBatch() {
  ...
  try {
    if (isApiTab.value) {
      await deleteRiverBatch(checkedIds.value);
    } else {
      // 模拟数据批量删除
...
async function handleExport() {
  if (isApiTab.value) {
    ...
    try {
      const response = await exportRiverExcel(params);
      const blob = response.data || response;
      const url = URL.createObjectURL(blob);
      const link = document.createElement('a');
      link.href = url;
      link.download = `河道环境管理_${new Date().toLocaleDateString()}.xlsx`;
...
async function loadOptions() {
  try {
    ...
    loadedOptions.cleaningType = extractData(cleaningTypeRes);
    loadedOptions.monitorType = extractData(monitorTypeRes);
    loadedOptions.monitorStatus = extractData(monitorStatusRes);
    loadedOptions.problemType = extractData(problemTypeRes);
    loadedOptions.handleStatus = extractData(handleStatusRes);
    loadedOptions.dept = extractData(deptRes);
    loadedOptions.tool = extractData(toolRes);
    ...
    await apiEditFormApi.updateSchema([
      ...
      { fieldName: 'cleaningTypeId', componentProps: { options: loadedOptions.cleaningType } },
      { fieldName: 'staffIds', componentProps: { options: loadedOptions.user } },
      { fieldName: 'toolIds', componentProps: { options: loadedOptions.tool } },
      { fieldName: 'monitorTypeId', componentProps: { options: loadedOptions.monitorType } },
      { fieldName: 'monitorBy', componentProps: { options: loadedOptions.user } },
      { fieldName: 'monitorStatusId', componentProps: { options: loadedOptions.monitorStatus } },
      ...
      { fieldName: 'handleStatusId', componentProps: { options: loadedOptions.handleStatus } },
    ]);
...
<!--      </template>-->
      ...
      <template #consumableName="{ row }">
        <el-text @click="handleOpenDetail(row)" type="primary">{{ row.consumableName }}</el-text>
      </template>
