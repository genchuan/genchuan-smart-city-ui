...
// 引入封装后的详情抽屉组件
import TrafficDetailDrawer from '#/views/report/park/traffic/flowdist/detail.vue';
...
async function handleDelete(row) {
  const loadingInstance = ElLoading.service({
    text: $t('ui.actionMessage.deleting', [row.areaName + ' - ' + row.parkType + ' - ' + row.timePeriod]),
  });
  try {
    reportObj.apilist = reportObj.apilist.filter((v) => v.id !== row.id);
    ElMessage.success($t('ui.actionMessage.deleteSuccess', [row.areaName + ' - ' + row.parkType + ' - ' + row.timePeriod]));
    handleRefresh();
  } finally {
...
const getTableData = (pageObj) => {
  ...
  } else if (activeName.value === '早高峰') {
    filteredList = reportObj.apilist.filter(v => v.timePeriod === '早高峰');
  } else if (activeName.value === '热点区域') {
    filteredList = reportObj.apilist.filter(v => v.hotspotFlag === '是');
  }
...
// 搜索表单查询
function onSubmit() {
  const formValues = QueryForm.form.values;
  let filtered = reportObj.apilist;

  // 时间范围筛选
  if (formValues.dateRange && formValues.dateRange.length === 2) {
    const [startDate, endDate] = formValues.dateRange;
    filtered = filtered.filter(v => {
      const statDate = new Date(v.statDate);
  ...
  if (formValues.areaName) {
    filtered = filtered.filter(v => v.areaName === formValues.areaName);
  }

  // 车场类型筛选
  if (formValues.parkType) {
    filtered = filtered.filter(v => v.parkType === formValues.parkType);
  }

  // 时段筛选
  if (formValues.timePeriod) {
    filtered = filtered.filter(v => v.timePeriod === formValues.timePeriod);
  }

  reportObj.apilist = filtered;
  gridApi.query();
  drawerApi.close();
}
...
const createLabel = (item) => {
  ...
  } else if (item.label === '早高峰') {
    count = reportObj.apilist.filter((v) => v.timePeriod === '早高峰').length;
  } else if (item.label === '热点区域') {
    count = reportObj.apilist.filter((v) => v.hotspotFlag === '是').length;
...
<template>
  <div class="park-lot-table-new">
    ...
    <TrafficDetailDrawer
      ...
      :title="`车流分布详情 - ${reportObj.detailObj.areaName} ${reportObj.detailObj.parkType}`"
    />
    <Drawer title="筛选车流分布">
    ...
    <Grid>
      ...
      <template #timePeriod="{ row }">
        <el-tag
          :type="row.timePeriod === '早高峰' ? 'danger' :
                 row.timePeriod === '晚高峰' ? 'warning' :
                 row.timePeriod === '平峰' ? 'success' : 'info'"
          size="small"
        >
          {{ row.timePeriod }}
        </el-tag>
      </template>
      <template #totalTraffic="{ row }">
        <el-tag type="primary" size="small">
          {{ row.totalTraffic }}
        </el-tag>
      </template>
      <template #avgTraffic="{ row }">
        <el-tag type="info" size="small">
          {{ row.avgTraffic }}
        </el-tag>
      </template>
      <template #hotspotFlag="{ row }">
        <el-tag
          :type="row.hotspotFlag === '是' ? 'danger' : 'success'"
          size="small"
        >
          {{ row.hotspotFlag }}
        </el-tag>
      ...
      <template #bottom>
        <div class="common-total" @click="changeTotalShow">
          ...
          <span> 本页统计：数据量{{ reportObj.list.length }};
            总车流量: {{ reportObj.list.reduce((sum, v) => sum + v.totalTraffic, 0) }};
            平均车流量: {{ Math.round(reportObj.list.reduce((sum, v) => sum + v.avgTraffic, 0) / reportObj.list.length) }};
            热点区域: {{ reportObj.list.filter(v => v.hotspotFlag === '是').length }}个;
            </span>
        </div>
