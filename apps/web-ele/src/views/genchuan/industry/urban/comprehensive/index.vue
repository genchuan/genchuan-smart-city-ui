...
const activeName = ref('综合管廊监测');
const secondShow = ref(true);
const activeSubTab = ref('实时监测');
const activeReportTab = ref('日报');

// 初始化时设置综合管廊监测标签页的 secondShow 为 true
const initTabState = () => {
  const monitorTab = tabArray.value.find((v) => v.label === '综合管廊监测');
  if (monitorTab) {
...
const handleSubTabChange = (subTab) => {
  if (activeName.value === '综合管廊监测') {
    activeSubTab.value = subTab;
  } else if (activeName.value === '综合管廊监测报表') {
