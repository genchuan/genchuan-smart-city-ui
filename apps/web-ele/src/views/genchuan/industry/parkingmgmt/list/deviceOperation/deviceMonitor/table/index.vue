...
const getTitle = computed(() => {
  return formData.value?.monitorId ? textObj.editText : textObj.addText;
});
...
async function handleDeleteBatch() {
  ...
  try {
    dataObj.apilist = dataObj.apilist.filter(
      (v) => !checkedIds.value.includes(v.monitorId),
    );
    checkedIds.value = [];
...
function handleRowCheckboxChange({ records }) {
  checkedIds.value = records.map((item) => item.monitorId);
}
...
<!--   详情抽屉-->
    ...
    <Grid>
      ...
      <template #isAbnormal="{ row }">
        <el-tag :type="getStatusType(row.isAbnormal)">
          {{ row.isAbnormal }}
        </el-tag>
      </template>
      ...
      <template #bottom>
        <div class="common-total" @click="changeTotalShow">
          ...
          <span> 本页统计：监控记录数: {{ dataObj.total }} </span>
        </div>
