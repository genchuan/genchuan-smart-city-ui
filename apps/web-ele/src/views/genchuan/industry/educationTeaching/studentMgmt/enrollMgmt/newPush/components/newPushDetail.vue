...
const drawerTitle = computed(() => {
  const name = detailObj.value?.taskName ? `${detailObj.value.taskName} 推送详情` : '推送详情';
...
<template>
  <DetailDrawer :title="drawerTitle">
    <div class="detail-card">
      <!-- 推送基础信息 -->
      <div class="detail-section">📋 推送任务信息</div>
      <div class="detail-card-row"><div class="detail-row-left">推送任务名称：</div><div class="detail-row-right">{{ detailObj.taskName || '-' }}</div></div>
      <div class="detail-card-row"><div class="detail-row-left">推送内容：</div><div class="detail-row-right">{{ detailObj.pushContent || '-' }}</div></div>
      <div class="detail-card-row"><div class="detail-row-left">推送人数：</div><div class="detail-row-right">{{ detailObj.pushNum || '-' }}</div></div>
      <div class="detail-card-row"><div class="detail-row-left">推送时间：</div><div class="detail-row-right">{{ formatTimestamp(detailObj.pushTime) }}</div></div>
      <div class="detail-card-row"><div class="detail-row-left">推送完成率：</div><div class="detail-row-right">{{ detailObj.finishRate ? detailObj.finishRate + '%' : '-' }}</div></div>
