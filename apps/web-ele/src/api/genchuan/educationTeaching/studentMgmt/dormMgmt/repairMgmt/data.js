...
export function getRepairMgmtPage(params) {
  ...
  return requestClient
    .get('/studentmgmt/repair-mgmt/page', { params: convertedParams })
    .then((res) => {
      if (res && res.list) {
...
export function createRepairMgmt(data) {
  ...
  return requestClient
    .post('/studentmgmt/repair-mgmt/create', convertedData)
    .catch((error) => {
      console.warn('申请接口失败，模拟成功', error);
      return true;
    });
}
...
export function assignRepairMgmt(data) {
  return requestClient
    .put('/studentmgmt/repair-mgmt/assign', data)
    .catch((error) => {
      console.warn('派单接口失败，模拟成功', error);
      return true;
    });
}
...
export function feedbackRepairMgmt(data) {
  return requestClient
    .put('/studentmgmt/repair-mgmt/feedback', data)
    .catch((error) => {
      console.warn('反馈接口失败，模拟成功', error);
      return true;
    });
}
...
export function acceptRepairMgmt(data) {
  return requestClient
    .put('/studentmgmt/repair-mgmt/accept', data)
    .catch((error) => {
      console.warn('验收接口失败，模拟成功', error);
      return true;
    });
}
...
export function updateRepairMgmt(data) {
  const convertedData = convertZhToEn(data);
  return requestClient
    .put('/studentmgmt/repair-mgmt/update', convertedData)
    .catch((error) => {
...
export function exportRepairMgmt(params) {
  const convertedParams = convertZhToEn(params);
  return requestClient
    .download('/studentmgmt/repair-mgmt/export-excel', convertedParams)
    .catch((error) => {
...
export function getRepairMgmtDetail(params) {
  return requestClient
    .get('/studentmgmt/repair-mgmt/get', { params })
...
export function getRepairMgmtChart(params) {
  const convertedParams = convertZhToEn(params);
  return requestClient
    .get('/studentmgmt/repair-mgmt/chart', { params: convertedParams })
    .then((res) => {
      if (
        res && // 转换 typeDistribution 中的英文类型为中文
        res.typeDistribution &&
        Array.isArray(res.typeDistribution)
      ) {
    ...
    .catch((error) => {
      console.warn('看板接口失败，使用模拟数据', error);
      return {
        totalRepairCount: 86,
...
export function getRepairMgmtCount(params) {
  const convertedParams = convertZhToEn(params);
  return requestClient
    .get('/studentmgmt/repair-mgmt/chart/repairCount', {
      params: convertedParams,
    })
    .catch((error) => {
      console.warn('统计接口失败，使用模拟数据', error);
      return {
        typeStatistics: [
