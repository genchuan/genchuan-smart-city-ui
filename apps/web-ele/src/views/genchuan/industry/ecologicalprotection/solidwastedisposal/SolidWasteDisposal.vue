...
} from '#/api/genchuan/industry/ecologicalprotection/SubdomainScenarioTopics.js';
...
const initData = async () => {
  try {
    ...
    // 2. 获取类型分布数据
    typePieData.value = await fetchSolidWasteTypeDistribution();
    ...
    // 4. 获取处置设施数据
    facilities.value = await fetchSolidWasteFacilities();

    // 5. 获取表格数据
    wasteData.value = await fetchSolidWasteDetailData();
  } catch (error) {
