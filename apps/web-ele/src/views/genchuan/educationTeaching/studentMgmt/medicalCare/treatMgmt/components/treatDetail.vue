...
const drawerTitle = computed(() => {
  const id = detailObj.value?.studentId ? `学号 ${detailObj.value.studentId} 就诊详情` : '就诊详情';
...
<template>
  <DetailDrawer :title="drawerTitle">
    <div class="detail-card">
      <!-- 基础信息 -->
      <div class="detail-section">🏥 就诊基础信息</div>
      <div class="detail-card-row"><div class="detail-row-left">学号：</div><div class="detail-row-right">{{ detailObj.studentId || '-' }}</div></div>
      <div class="detail-card-row"><div class="detail-row-left">年级：</div><div class="detail-row-right">{{ detailObj.grade || '-' }}</div></div>
      <div class="detail-card-row"><div class="detail-row-left">就诊类型：</div><div class="detail-row-right">{{ detailObj.treatType || '-' }}</div></div>
      <div class="detail-card-row"><div class="detail-row-left">症状描述：</div><div class="detail-row-right">{{ detailObj.symptom || '-' }}</div></div>
      <div class="detail-card-row"><div class="detail-row-left">就诊登记时间：</div><div class="detail-row-right">{{ formatTimestamp(detailObj.registerTime) }}</div></div>
      <div class="detail-card-row"><div class="detail-row-left">就诊内容：</div><div class="detail-row-right">{{ detailObj.treatContent || '-' }}</div></div>
      <div class="detail-card-row"><div class="detail-row-left">预约时间：</div><div class="detail-row-right">{{ formatTimestamp(detailObj.applyTime) }}</div></div>
      ...
      <div class="detail-card-row"><div class="detail-row-left">家长反馈时间：</div><div class="detail-row-right">{{ formatTimestamp(detailObj.feedbackTime) }}</div></div>
