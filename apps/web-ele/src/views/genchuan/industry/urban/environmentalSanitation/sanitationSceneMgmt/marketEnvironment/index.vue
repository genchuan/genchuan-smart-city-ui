...
import {
  ...
  deleteMarketBatch,
  exportMarketExcel,
} from '#/api/genchuan/industry/urban/environmentalSanitation/sanitationSceneMgmt/marketEnvironment/data.js';

// 导入原有模拟数据（用于非“全部”标签页）
import { dataList } from '#/api/genchuan/industry/urban/environmentalSanitation/sanitationSceneMgmt/marketEnvironment/data.js';

// 导入表单配置（新增了接口相关的 schema）
import {
  textObj,
  useFormSchema,                         // 原有表单（模拟数据）
  useMarketEditSchema,                    // 新增：接口表单
  useMarketSearchSchema,                  // 新增：搜索表单
  getColumnsByStatus,
  getAreaOptions,
  getUserOptions,
...
} from '#/api/genchuan/industry/urban/environmentalSanitation/sanitationSceneMgmt/marketEnvironment/form.js';
...
const counts = ref({
  ...
  statusCounts: {
    ...
    收运待执行: mockDataList.filter(v => v.status === '收运待执行').length,
    污水待处置: mockDataList.filter(v => v.status === '污水待处置').length,
    卫生待核查: mockDataList.filter(v => v.status === '卫生待核查').length,
...
// ---------- 转换函数（接口数据 -> 表格显示） ----------
function convertMarketItem(item) {
  // 解析 JSON 字符串字段
  const parseJSON = (str) => {
    if (!str) return [];
    try {
  ...
  // 处理各种数组字段
  let cleaningTime = item.cleaningTime ? parseJSON(item.cleaningTime) : [];
  let staffIds = item.staffIds ? parseJSON(item.staffIds) : [];
  let garbageTypeIds = item.garbageTypeIds ? parseJSON(item.garbageTypeIds) : [];

  // 映射ID为label（新增）
  const mapIdsToLabels = (ids, options) => {
    if (!ids || !options) return [];
    return ids.map(id => {
      // options中的value可能包含空格，需要trim
      const found = options.find(opt => opt.value.trim() === id.trim());
      return found ? found.label : id;
    });
  };

  const staffLabels = mapIdsToLabels(staffIds, loadedOptions.user);
  const garbageTypeLabels = mapIdsToLabels(garbageTypeIds, loadedOptions.garbageType);
  ...
  return {
    ...
    staffIds,
    garbageTypeIds,
    staffLabels,          // 新增：人员label数组
    garbageTypeLabels,    // 新增：垃圾类型label数组
    ...
    lastSewageCleaningTime: item.lastSewageCleaningTime ? new Date(item.lastSewageCleaningTime).toLocaleString() : '-',
    nextSewageCleaningTime: item.nextSewageCleaningTime ? new Date(item.nextSewageCleaningTime).toLocaleString() : '-',
    hygieneCheckDate: item.hygieneCheckDate ? new Date(item.hygieneCheckDate).toLocaleString() : '-',
    reformDeadline: item.reformDeadline ? new Date(item.reformDeadline).toLocaleString() : '-',
  };
}
...
const getTableData = async ({ page }) => {
  if (isApiTab.value) {
    ...
    try {
      ...
      dataObj.total = total;
      dataObj.list = listData.map(convertMarketItem);
      return dataObj;
    } catch (error) {
...
const [ApiEditDrawer, apiEditDrawerApi] = useVbenDrawer({
  ...
  async onConfirm() {
    ...
    const arrayFields = ['staffIds', 'garbageTypeIds', 'cleaningTime'];
    ...
    try {
      if (isAdd) {
        ...
        delete originalData.garbageTypesName;
        delete originalData.vehicleName;
        delete originalData.checkName;
        delete originalData.handleStatusName;
        delete originalData.checkResultName;
...
async function handleDeleteBatch() {
  ...
  try {
    if (isApiTab.value) {
      await deleteMarketBatch(checkedIds.value);
    } else {
      // 模拟数据批量删除
...
async function handleExport() {
  if (isApiTab.value) {
    ...
    try {
      const response = await exportMarketExcel(params);
      const blob = response.data || response;
      const url = URL.createObjectURL(blob);
      const link = document.createElement('a');
      link.href = url;
      link.download = `集贸市场环境管理_${new Date().toLocaleDateString()}.xlsx`;
...
async function loadOptions() {
  try {
    ...
    loadedOptions.garbageType = extractData(garbageTypeRes);
    loadedOptions.vehicle = extractData(vehicleRes);
    loadedOptions.handleStatus = extractData(handleStatusRes);
    loadedOptions.checkResult = extractData(checkResultRes);
    ...
    await apiEditFormApi.updateSchema([
      ...
      {fieldName: 'garbageTypeIds', componentProps: {options: loadedOptions.garbageType}},
      {fieldName: 'vehicleId', componentProps: {options: loadedOptions.vehicle}},
      {fieldName: 'checkBy', componentProps: {options: loadedOptions.user}},
    ]);
...
<!--      </template>-->
      ...
      <template #repairBy="{ row }">
        <el-text @click="handleOpenDetail(row)" type="primary">{{ row.repairBy }}</el-text>
      </template>
