...
import { getRefundRecordChart } from '#/api/genchuan/industry/chargePark/orderTrade/refundMgmt/index.js';
import Card from '#/components/stats/card.vue';

const state = reactive({
  cardList: [
    { title: '退款总额', value: 0, color: '#FF6B6B', suffix: '元' },
    { title: '退款成功率', value: 0, color: '#4ECDC4', suffix: '%' },
    { title: '退款总数', value: 0, color: '#13ce66' },
  ],
  trendData: [],
});
...
const fetchOrderChartData = async () => {
  try {
    ...
    state.cardList[0].value = res.totalRefundAmount;
    state.cardList[1].value = res.refundSuccessRate;
    // 退款总数通过趋势数据计算
