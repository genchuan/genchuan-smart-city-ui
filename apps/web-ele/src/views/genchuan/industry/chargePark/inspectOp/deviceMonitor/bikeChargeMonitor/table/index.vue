...
import {
  alarmBikeChargeMonitor,
  exportBikeChargeMonitor,
  getBikeChargeMonitorDetail,
  getBikeChargeMonitorLocation,
  getBikeChargeMonitorPage,
} from '#/api/genchuan/industry/chargePark/inspectOp/deviceMonitor/bikeChargeMonitor';
...
async function getTableData({ page }) {
  ...
  try {
    const response = await getBikeChargeMonitorPage(queryParams);
    ...
    dataObj.list = list.map((item) => normalizeBikeChargeMonitorRow(item));
  } catch (error) {
    console.error('获取两轮充电监测数据失败，使用静态数据:', error);
...
async function handleExport() {
  try {
    const data = await exportBikeChargeMonitor({
      ...dataObj.searchParams,
      monitorStatus:
        filterMonitorStatus.value || dataObj.searchParams.monitorStatus,
      alarmStatus: filterAlarmStatus.value || dataObj.searchParams.alarmStatus,
...
async function handleLocate(row) {
  try {
    const response = await getBikeChargeMonitorLocation({ id: row.id });
    const location = {
      ...row,
      ...response,
      id: row.id,
...
<template>
  <div class="park-lot-table-new">
    <DetailDrawer
      ...
      :fields="detailFields"
      :title="`${dataObj.detailObj.deviceCode || '两轮充电监测'}详情`"
    />
