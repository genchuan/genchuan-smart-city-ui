...
const initChart = () => {
  ...
  const option = {
    ...
    tooltip: {
      trigger: 'item',
      backgroundColor: 'rgba(255, 255, 255, 0.9)', // 优化：tooltip背景改为白色（适配黑色字体）
      borderColor: 'rgba(0, 0, 0, 0.2)', // 优化：tooltip边框改为浅黑色
      borderWidth: 1,
      textStyle: {
        color: '#000000', // 修改2：tooltip字体改为黑色
    ...
    series: formattedSeries.map((item) => ({
      ...
      labelLine: {
        ...
        length: 2,
        lineStyle: {
          color: 'rgba(0, 0, 0, 0.5)', // 修改4：标签引导线改为浅黑色
        },
      },
      emphasis: {
        scale: true,
...
const handleResize = () => {
  ...
  chartInstance.value.setOption({
    title: {
      textStyle: { fontSize: titleFontSize, color: '#000000' }, // 同步修改：响应式调整时标题字体保持黑色
    },
    tooltip: {
      textStyle: { fontSize: tooltipFontSize, color: '#000000' }, // 同步修改：响应式调整时tooltip字体保持黑色
    },
    series: [
      {
        label: { fontSize: labelFontSize, color: '#000000' }, // 同步修改：响应式调整时标签字体保持黑色
        labelLine: {
          lineStyle: { color: 'rgba(0, 0, 0, 0.5)' } // 同步修改：响应式调整时引导线保持浅黑色
        }
      },
    ],
  });
