...
import ProjectBasicInfoForm from './ProjectBasicInfoForm.vue';
...
const handleDelete = async (id: number) => {
  try {
    // 删除的二次确认
    await confirm('是否确认删除该工程基本信息数据？', '系统提示');
    // 发起删除
    await ProjectBasicInfoApi.deleteProjectBasicInfo(id);
...
<template>
  <div class="p-4">
    ...
    <!-- 表单弹窗：添加/修改 -->
    <ProjectBasicInfoForm ref="formRef" @success="getList" />
  </div>
</template>
