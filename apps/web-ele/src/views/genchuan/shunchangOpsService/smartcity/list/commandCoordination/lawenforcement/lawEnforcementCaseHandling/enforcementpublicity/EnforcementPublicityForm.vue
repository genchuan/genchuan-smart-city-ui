  ...
  if (id) {
    ...
    try {
      ...
      formData.value = {
        ...res,
        // 将字符串时间戳转为数字
        announcementStartTime: res.announcementStartTime ? Number(res.announcementStartTime) : undefined,
        announcementDeadline: res.announcementDeadline ? Number(res.announcementDeadline) : undefined
      }
    } finally {
  ...
  <ElDialog :title="dialogTitle" v-model="dialogVisible" width="600px" append-to-body>
    ...
    >
      <ElFormItem label="执法公示编号" prop="publicityNumber">
        <ElInput v-model="formData.publicityNumber" placeholder="请输入执法公示编号" />
      </ElFormItem>
      <ElFormItem label="公示标题" prop="publicityTitle">
        <ElInput v-model="formData.publicityTitle" placeholder="请输入公示标题" />
      </ElFormItem>
      <ElFormItem label="公示内容" prop="publicityContent">
        <ElInput v-model="formData.publicityContent" type="textarea" placeholder="请输入公示内容" />
      </ElFormItem>
      ...
      <ElFormItem label="执法地点" prop="enforcementLocation">
        <ElInput v-model="formData.enforcementLocation" placeholder="请输入执法地点" />
      </ElFormItem>
      <ElFormItem label="执法依据" prop="enforcementBasis">
        <ElInput v-model="formData.enforcementBasis" placeholder="请输入执法依据" />
      </ElFormItem>
      <ElFormItem label="执法结果" prop="enforcementResults">
        <ElInput v-model="formData.enforcementResults" placeholder="请输入执法结果" />
      </ElFormItem>
