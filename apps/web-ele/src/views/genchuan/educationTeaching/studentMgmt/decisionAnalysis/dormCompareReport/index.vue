...
// 柱状图钻取：宿舍得分排名 -> 跳转宿舍评比明细弹窗
const handleDormRankBarClick = (drillInfo) => {
  console.log('宿舍排名柱状图钻取:', drillInfo);
  const currentTable = getCurrentTableRef();
  if (currentTable) {
    currentTable.handleStatsFilter('dormRankBar', drillInfo.dormNo);
  }
};

// 柱状图钻取：各楼栋文明宿舍数量 -> 筛选该楼栋文明宿舍列表
const handleBuildingBarClick = (drillInfo) => {
  console.log('楼栋文明宿舍柱状图钻取:', drillInfo);
  const currentTable = getCurrentTableRef();
  if (currentTable) {
    currentTable.handleStatsFilter('buildingBar', drillInfo.buildingName);
  }
};
...
<template>
  <div class="common-index">
    <Chart
      v-if="showStats"
      ref="chartRef"
      @dorm-rank-bar-click="handleDormRankBarClick"
      @building-bar-click="handleBuildingBarClick"
    />
