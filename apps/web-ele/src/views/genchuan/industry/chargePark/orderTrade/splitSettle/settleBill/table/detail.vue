...
const drawerTitle = computed(() => {
  const billNo = detailObj.value?.billNo || '分账结算';
  return title.value || `${billNo} 详情`;
});

// 状态映射 - SettleBillStatusEnum
const statusMap = {
...
<template>
  <DetailDrawer :title="drawerTitle">
    <div class="detail-card">
      ...
      <div class="detail-card-row">
        <div class="detail-row-left">收费总额:</div>
        <div class="detail-row-right">{{ detailObj.totalAmount ? `¥${detailObj.totalAmount.toFixed(2)}` : '¥0.00' }}</div>
      </div>

      <div class="detail-card-row">
        <div class="detail-row-left">分成金额:</div>
        <div class="detail-row-right">{{ detailObj.splitAmount ? `¥${detailObj.splitAmount.toFixed(2)}` : '¥0.00' }}</div>
      </div>
      ...
      <div class="detail-card-row">
        ...
        <div class="detail-row-right">{{ detailObj.settleTime || '-' }}</div>
      </div>
