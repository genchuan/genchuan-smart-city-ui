...
<template>
  <div class="main-condition-inner">
    <!-- 触发事件类型选择 -->
    ...
    <!-- 设备属性条件配置 -->
    <div v-if="isDevicePropertyTrigger" class="device-property-section">
      <!-- 产品设备选择 -->
      <el-row :gutter="16">
        <el-col :span="12">
    ...
    <!-- 设备状态条件配置 -->
    <div v-else-if="isDeviceStatusTrigger" class="device-status-section">
      <el-row :gutter="16">
        <el-col :span="12">
          <el-form-item label="产品" required>
            <ProductSelector
    ...
    <!-- 其他触发类型的提示 -->
    <div v-else class="other-trigger-hint">
      <p class="hint-title">
        当前触发事件类型：{{ getTriggerTypeLabel(triggerType) }}
      </p>
      <p class="hint-subtitle">此触发类型暂不需要配置额外条件</p>
    </div>
  </div>
</template>
