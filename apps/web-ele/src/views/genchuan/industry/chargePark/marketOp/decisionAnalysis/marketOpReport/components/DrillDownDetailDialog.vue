...
const chartTypeMap = {
  ...
  ruleType: {
    ...
    apiParam: 'ruleType',
    apiName: '获取规则/消耗规则',
    api: getCycleReportPieDrillData,
  },
  configType: {
    chartType: 'pie',
    apiParam: 'configType',
  ...
  couponPackageType: {
    chartType: 'pie',
    apiParam: 'packageType',
    apiName: '新人券包/活动券包/会员券包',
    api: getCycleReportPieDrillData,
  },
  ...
  activityType: {
    ...
    api: getCycleReportBarDrillData,
  },
  prizeType: {
    chartType: 'bar',
    apiParam: 'prizeType',
  ...
  exchangeCategory: {
    chartType: 'bar',
    apiParam: 'exchangeCategoryType',
    api: getCycleReportBarDrillData,
  },
  pointActivityType: {
    chartType: 'bar',
    apiParam: 'pointActivityType',
    api: getCycleReportBarDrillData,
  },
  // 折线图类型 - lineType 参数值
  joinTrend: {
    chartType: 'line',
    apiParam: 'activityTrend',
    lineName: '活动参与趋势',
    api: getCycleReportLineDrillData,
  },
  lotteryTrend: {
    chartType: 'line',
    apiParam: 'lotteryTrend',
    lineName: '抽奖量趋势',
    api: getCycleReportLineDrillData,
  },
  couponSendTrend: {
    chartType: 'line',
    apiParam: 'couponTrend',
    lineName: '优惠券发放趋势',
    api: getCycleReportLineDrillData,
  },
  orderTrend: {
    ...
    switch (chartType) {
      ...
      case 'cardType': {
        mockList.push({
          ...
          price: [329, 349, 499, 199, 899, 699, 299, 599, 399][
            Math.floor(Math.random() * 9)
          ],
          saleCount: Math.floor(Math.random() * 100) + 10,
          status: String(Math.floor(Math.random() * 2)),
          createTime: String(1_776_876_559_000 + Math.random() * 100_000_000),
        });
        break;
      }
      case 'configType': {
      ...
      case 'couponSendTrend': {
        mockList.push({
          ...
          userName: `user${String(i).padStart(3, '0')}`,
          receiveTime: String(1_773_842_400_000 + Math.random() * 100_000_000),
          status: String(Math.floor(Math.random() * 2)),
          verifyTime:
            Math.random() > 0.5
              ? String(1_774_000_000_000 + Math.random() * 100_000_000)
              : null,
          createTime: String(1_776_843_537_000 + Math.random() * 100_000_000),
        });
        break;
      }
      case 'couponType': {
      ...
      case 'exchangeCategory': {
        mockList.push({
          ...
          costPoint: [50, 100, 130, 200, 500, 800, 1000][
            Math.floor(Math.random() * 7)
          ],
          payStatus: String(Math.floor(Math.random() * 4)),
          payTime: String(1_774_450_533_000 + Math.random() * 100_000_000),
          createTime: String(1_776_877_089_000 + Math.random() * 100_000_000),
        });
        break;
      }
      // ========== 折线图钻取数据 ==========
      case 'joinTrend': {
        mockList.push({
          ...
          startTime: String(1_775_016_000_000 + Math.random() * 100_000_000),
          endTime: String(1_780_000_000_000 + Math.random() * 100_000_000),
          joinCount: Math.floor(Math.random() * 500) + 100,
          status: String(Math.floor(Math.random() * 3)),
          creator: '亘川智城',
          createTime: String(1_776_839_732_000 + Math.random() * 100_000_000),
        });
        break;
      }
      case 'lotteryTrend': {
        mockList.push({
          ...baseData,
          userName: `user${String(i).padStart(3, '0')}`,
          lotteryTime: String(1_776_371_400_000 + Math.random() * 100_000_000),
          costPoint: [100, 150, 200][Math.floor(Math.random() * 3)],
          status: String(Math.floor(Math.random() * 3)),
          createTime: String(1_776_840_858_000 + Math.random() * 100_000_000),
        });
        break;
      }
      case 'orderTrend': {
        mockList.push({
          ...
          userName: `user${String(i).padStart(3, '0')}`,
          amount: [49.9, 129, 299, 399, 899, 2499, 3599][
            Math.floor(Math.random() * 7)
          ],
          payStatus: String([-1, 2, 3][Math.floor(Math.random() * 3)]),
          payTime:
            Math.random() > 0.5
              ? String(1_774_709_733_000 + Math.random() * 100_000_000)
              : null,
          createTime: String(1_776_876_559_000 + Math.random() * 100_000_000),
        });
        break;
      ...
      case 'prizeType': {
        mockList.push({
          ...
          stock: Math.floor(Math.random() * 15_000) + 100,
          sendCount: Math.floor(Math.random() * 3500),
          status: String(Math.floor(Math.random() * 2)),
          createTime: String(1_777_273_598_000 + Math.random() * 100_000_000),
        });
        break;
      }
      // ========== 饼图钻取数据 ==========
      ...
      case 'stockTrend': {
        mockList.push({
          ...
          warnThreshold: [5, 10, 15, 20, 50, 80, 100][
            Math.floor(Math.random() * 7)
          ],
          status: String(Math.floor(Math.random() * 3)),
          warnStatus: String(Math.floor(Math.random() * 3)),
          syncTime: String(1_776_834_000_000 + Math.random() * 100_000_000),
          createTime: String(1_776_876_559_000 + Math.random() * 100_000_000),
        });
        break;
      }
    ...
    <div class="drill-down-wrapper">
      ...
      <Grid>
        ...
        <template #endTime="{ row }">
          {{
            row.endTime
              ? formatDate(new Date(Number(row.endTime)), 'YYYY-MM-DD HH:mm:ss')
              : '-'
          }}
        </template>
