...
import {
  ...
  getWorkReportDetail,
  getUserInfo,
} from '#/api/genchuan/industry/educationTeaching/studentMgmt/decisionAnalysis/workReport/data.js';
...
import {
  ...
  useGridColumns,
  useSearchFormSchema,
} from '#/api/genchuan/industry/educationTeaching/studentMgmt/decisionAnalysis/workReport/form.js';
import StudentDetailDialog from '../components/StudentDetailDialog.vue';
import HonorDetailDialog from '../components/HonorDetailDialog.vue';
import AssessDetailDialog from '../components/AssessDetailDialog.vue';
import ViolateDetailDialog from '../components/ViolateDetailDialog.vue';
import MentalDetailDialog from '../components/MentalDetailDialog.vue';
...
const [FormDrawer, formDrawerApi] = useVbenDrawer({
  ...
  async onConfirm() {
    ...
    try {
      ...
      const response = await createWorkReport(params);
...
const honorDetailRef = ref(null);
const assessDetailRef = ref(null);
const violateDetailRef = ref(null);
const mentalDetailRef = ref(null);
...
const getTableData = async (pageObj) => {
  ...
  try {
    ...
    const response = await getWorkReportPage(params);
    if (response?.code === 200) {
...
const handleExport = async () => {
  try {
    ...
    const blob = await exportWorkReport(params);
    downloadFileFromBlobPart({fileName: '学工报表数据.xlsx', source: blob});
...
const handleExportRow = async (row) => {
  try {
    ...
    const loadingInstance = ElLoading.service({text: '正在导出...'});
    const blob = await exportWorkReport({id: row.id});
    downloadFileFromBlobPart({fileName: `学工报表_${row.reportCycle}.xlsx`, source: blob});
    ElMessage.success('导出成功');
    loadingInstance.close();
  } catch (error) {
    if (error !== 'cancel') {
...
const handleOpenDetail = async (row) => {
  ...
  try {
    const response = await getWorkReportDetail({id: row.id});
...
const handleGenerate = async (row) => {
  try {
    await ElMessageBox.confirm(`确认生成“${row.reportCycle} - ${row.className || '全年级'}”报表吗？`, '生成确认', {
      confirmButtonText: '确认',
      cancelButtonText: '取消',
      type: 'info',
    });
    const loadingInstance = ElLoading.service({text: '正在生成报表...'});
    const response = await createWorkReport({id: row.id});
    if (response?.code === 200) {
...
const handleArchive = async (row) => {
  try {
    ...
    const response = await archiveWorkReport({ids: [row.id]});
    if (response?.code === 200) {
...
const getActionButtons = (row) => {
  ...
  if (generateStatus === '已生成') {
    ...
    else base.push({label: '归档', onClick: () => handleArchive(row)}, {
      label: '打印',
...
const handleFieldDrill = (type, row) => {
  switch (type) {
    case 'studentTotal':
      ...
      break;
    case 'studentRegularNum':
      studentDetailRef.value?.open();
      break;
    case 'warnStudentNum':
      mentalDetailRef.value?.open();
      break;
    case 'newHonorNum':
      honorDetailRef.value?.open();
      break;
    case 'newViolateNum':
      violateDetailRef.value?.open();
      break;
    case 'newAssessNum':
      assessDetailRef.value?.open();
      break;
    case 'reportCycle':
...
<template>
  <div class="park-lot-table-new">
    ...
    <DetailDrawer ref="detailDrawerRef" :title="`${detailData.reportCycle || '学工报表'}详情`"
    ...
    <!-- 明细弹窗 -->
    <StudentDetailDialog ref="studentDetailRef" :row="{}"/>
    <HonorDetailDialog ref="honorDetailRef" :row="{}"/>
    <AssessDetailDialog ref="assessDetailRef" :row="{}"/>
    <ViolateDetailDialog ref="violateDetailRef" :row="{}"/>
    <MentalDetailDialog ref="mentalDetailRef" :row="{}"/>
    ...
    <Grid>
      ...
      <template #table-title>
        <div class="tabel-tabs"
          ...
          <ElTag v-if="activeFilterTags.reportCycle" type="primary" closable
                 @close="removeFilterTag('reportCycle')">报表周期：{{ activeFilterTags.reportCycle }}
          </ElTag>
      ...
      <template #statisticalPeriod="{ row }">
        <span>{{ formatTimestamp(row.statStartTime) }} - {{
            formatTimestamp(row.statEndTime)
          }}</span>
      </template>
      ...
      <!-- 学生总数 -->
      <template #studentTotal="{ row }">
        <span style="color: #409eff; cursor: pointer"
              @click="handleFieldDrill('studentTotal', row)">
          {{ row.studentTotal }}
        </span>
      </template>

      <!-- 在籍人数 -->
      <template #studentRegularNum="{ row }">
        <span style="color: #409eff; cursor: pointer"
              @click="handleFieldDrill('studentRegularNum', row)">
          {{ row.studentRegularNum }}
        </span>
      </template>

      <!-- 异常预警人数 -->
      <template #warnStudentNum="{ row }">
        <span style="color: #409eff; cursor: pointer"
              @click="handleFieldDrill('warnStudentNum', row)">
          {{ row.warnStudentNum }}
        </span>
      </template>

      <!-- 新增荣誉数 -->
      <template #newHonorNum="{ row }">
        <span style="color: #409eff; cursor: pointer" @click="handleFieldDrill('newHonorNum', row)">
          {{ row.newHonorNum }}
        </span>
      </template>

      <!-- 新增违纪数 -->
      <template #newViolateNum="{ row }">
        <span style="color: #409eff; cursor: pointer"
              @click="handleFieldDrill('newViolateNum', row)">
          {{ row.newViolateNum }}
        </span>
      </template>

      <!-- 新增考评数 -->
      <template #newAssessNum="{ row }">
        <span style="color: #409eff; cursor: pointer"
              @click="handleFieldDrill('newAssessNum', row)">
          {{ row.newAssessNum }}
        </span>
      </template>
      ...
      <template #actions="{ row }">
        <div class="table-toolbar-tools" style="gap: 8px;">
          <template v-for="btn in getActionButtons(row)" :key="btn.label">
            <IconButton :content="btn.label"
                        :icon-name="btn.label === '生成' ? 'Plus' : (btn.label === '查看' ? 'View' : (btn.label === '导出' ? 'download' : (btn.label === '归档' ? 'FolderAdd' : 'Printer')))"
                        @click="btn.onClick"/>
          </template>
        </div>
      </template>
