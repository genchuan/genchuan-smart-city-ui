...
// 导入真实接口（用于“全部”标签页）
import {
  getCommercialStreetPage,
  createCommercialStreet,
  updateCommercialStreet,
  deleteCommercialStreet,
  deleteCommercialStreetBatch,
  exportCommercialStreetExcel,
} from '#/api/genchuan/industry/urban/environmentalSanitation/sanitationSceneMgmt/commercialStreet/data.js';

// 导入原有模拟数据（用于非“全部”标签页）
import { dataList } from '#/api/genchuan/industry/urban/environmentalSanitation/sanitationSceneMgmt/commercialStreet/data.js';

// 导入表单配置（新增了接口相关的 schema）
import {
  textObj,
  useFormSchema,                         // 原有表单（模拟数据）
  useCommercialStreetEditSchema,          // 新增：接口表单
  useCommercialStreetSearchSchema,        // 新增：搜索表单
  getColumnsByStatus,
  getAreaOptions,
  getUserOptions,
  getOperationStatusOptions,
  getFacilityOptions,                     // 新增：设施选项接口
...
// ---------- 转换函数（接口数据 -> 表格显示） ----------
function convertCommercialStreetItem(item) {
  // 解析 JSON 字符串字段
  const parseJSON = (str) => {
    if (!str) return [];
    try {
  ...
  // 处理保洁人员ID数组
  let cleanerIds = item.cleanerIds ? parseJSON(item.cleanerIds) : [];
  // 处理负责人员ID数组
  ...
  const cleanerLabels = mapIdsToLabels(cleanerIds, loadedOptions.user);
...
const getTableData = async ({ page }) => {
  if (isApiTab.value) {
    ...
    try {
      const res = await getCommercialStreetPage(params);
      const listData = res.data?.list || res.list || [];
      const total = res.data?.total || res.total || 0;
      counts.value.total = total; // 更新计数
      dataObj.total = total;
      dataObj.list = listData.map(convertCommercialStreetItem);
      return dataObj;
    } catch (error) {
...
const [SearchForm, searchFormApi] = useVbenForm({
  ...
  layout: 'horizontal',
  schema: useCommercialStreetSearchSchema(),
  showCollapseButton: true,
  submitButtonOptions: { content: '查询' },
});
...
const [ApiEditForm, apiEditFormApi] = useVbenForm({
  commonConfig: { componentProps: { class: 'w-full' }, formItemClass: 'col-span-2', labelWidth: 80 },
  layout: 'horizontal',
  schema: useCommercialStreetEditSchema(),
  showDefaultActions: false,
});
...
const [ApiEditDrawer, apiEditDrawerApi] = useVbenDrawer({
  ...
  async onConfirm() {
    ...
    // 处理数组字段转JSON字符串（cleanerIds, staffIds, facilityIds 等）
    ...
    if (submitData.facilityIds && Array.isArray(submitData.facilityIds)) {
      submitData.facilityIds = JSON.stringify(submitData.facilityIds);
    }

    try {
      if (isAdd) {
        await createCommercialStreet(submitData);
      } else {
        // 合并原有数据（避免丢失字段）
        ...
        delete originalData.cleanersName;
        ...
        delete originalData.maintainStatusName;
        ...
        if (fullData.facilityIds && Array.isArray(fullData.facilityIds)) {
          fullData.facilityIds = JSON.stringify(fullData.facilityIds);
        }
        await updateCommercialStreet(fullData);
      }
    ...
    if (isOpen) {
      ...
      if (apiFormData.value?.id) {
        ...
        if (editData.cleanerIds && typeof editData.cleanerIds === 'string') {
          editData.cleanerIds = parseJSON(editData.cleanerIds);
        }
        if (editData.staffIds && typeof editData.staffIds === 'string') {
          editData.staffIds = parseJSON(editData.staffIds);
        }
        if (editData.facilityIds && typeof editData.facilityIds === 'string') {
          editData.facilityIds = parseJSON(editData.facilityIds);
        }
...
async function handleDelete(row) {
  ...
  try {
    if (isApiTab.value) {
      await deleteCommercialStreet(row.id);
    } else {
      // 模拟数据删除
...
async function handleDeleteBatch() {
  ...
  try {
    if (isApiTab.value) {
      await deleteCommercialStreetBatch(checkedIds.value);
    } else {
      // 模拟数据批量删除
...
async function handleExport() {
  if (isApiTab.value) {
    ...
    try {
      const response = await exportCommercialStreetExcel(params);
      const blob = response.data || response;
      const url = URL.createObjectURL(blob);
      const link = document.createElement('a');
      link.href = url;
      link.download = `商业街环境管理_${new Date().toLocaleDateString()}.xlsx`;
...
async function loadOptions() {
  try {
    const [areaRes, userRes, statusRes, facilityRes] = await Promise.all([
      getAreaOptions(),
      getUserOptions(),
      getOperationStatusOptions(),
      getFacilityOptions(), // 新增
    ]);
    ...
    await apiEditFormApi.updateSchema([
      ...
      { fieldName: 'facilityIds', componentProps: { options: loadedOptions.facility } }, // 新增
    ]);
