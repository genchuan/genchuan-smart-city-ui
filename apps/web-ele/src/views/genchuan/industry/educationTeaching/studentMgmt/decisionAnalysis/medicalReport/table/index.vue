...
import {
  getMedicalReportPage,
  createMedicalReport,
  archiveMedicalReport,
  exportMedicalReport,
  getMedicalReportDetail,
  getTreatMgmtPage,
  getUserInfo,
} from '#/api/genchuan/industry/educationTeaching/studentMgmt/decisionAnalysis/medicalReport/data.js';
...
import {
  ...
  useGridColumns,
  useSearchFormSchema,
} from '#/api/genchuan/industry/educationTeaching/studentMgmt/decisionAnalysis/medicalReport/form.js';
import TreatDetailDialog from '../components/TreatDetailDialog.vue';
...
const [FormDrawer, formDrawerApi] = useVbenDrawer({
  ...
  async onConfirm() {
    ...
    try {
      ...
      const response = await createMedicalReport(params);
...
const getTableData = async (pageObj) => {
  ...
  try {
    ...
    const response = await getMedicalReportPage(params);
    if (response?.code === 200) {
...
const handleExport = async () => {
  try {
    ...
    const blob = await exportMedicalReport(params);
    downloadFileFromBlobPart({ fileName: '就医报表数据.xlsx', source: blob });
...
const handleExportRow = async (row) => {
  try {
    ...
    const loadingInstance = ElLoading.service({ text: '正在导出...' });
    const blob = await exportMedicalReport({ id: row.id });
    downloadFileFromBlobPart({ fileName: `就医报表_${row.reportCycle}.xlsx`, source: blob });
    ElMessage.success('导出成功');
    loadingInstance.close();
  } catch (error) {
    if (error !== 'cancel') {
...
const handleOpenDetail = async (row) => {
  ...
  try {
    const response = await getMedicalReportDetail({ id: row.id });
...
const handleGenerate = async (row) => {
  try {
    ...
    const loadingInstance = ElLoading.service({ text: '正在生成报表...' });
    const response = await createMedicalReport({ id: row.id });
    if (response?.code === 200) {
...
const handleArchive = async (row) => {
  try {
    ...
    const response = await archiveMedicalReport({ ids: [row.id] });
    if (response?.code === 200) {
...
const handleStatsFilter = (type, value) => {
  if (type === 'pie') {
    // 饼图钻取：就诊类型分布 -> 筛选该类型就诊记录
    treatDetailRef.value?.open({ treatType: value });
  } else if (type === 'line') {
    // 折线图钻取
...
<template>
  <div class="park-lot-table-new">
    ...
    <DetailDrawer ref="detailDrawerRef" :title="`${detailData.reportCycle || '就医报表'}详情`"
    ...
    <!-- 明细弹窗 -->
    <TreatDetailDialog ref="treatDetailRef" />
    ...
    <Grid>
      ...
      <!-- 就诊总人数 -->
      <template #treatTotal="{ row }">
        <span style="color: #409eff; cursor: pointer" @click="handleFieldDrill('treatTotal', row)">
          {{ row.treatTotal }}
        </span>
      </template>
