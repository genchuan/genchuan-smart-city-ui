...
// 计算属性处理标题，优先使用泊位编号，兜底显示默认值
const drawerTitle = computed(() => {
  const targetBerthNo = detailObj.value?.targetBerthNo || '停车记录';
  return title.value || `${targetBerthNo}详情`;
});
...
.detail-card {
  min-height: 200px; // 适配少量字段，降低最小高度
...
.detail-row-right {
  ...
  line-height: 18px;
  color: #303133; // 主文本色
  word-break: break-all; // 处理长文本换行（如泊位编号）
}
