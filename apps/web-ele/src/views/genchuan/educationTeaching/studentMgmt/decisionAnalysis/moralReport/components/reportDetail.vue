...
const drawerTitle = computed(() => {
  const name = props.detailObj?.reportType || `${props.detailObj?.timeScale}德育评比报表`;
...
const detailColumns = [
  ...
  { prop: 'score', label: '德育得分', minWidth: 100 },
...
async function loadFullDetail() {
  ...
  try {
    const [chartRes, rankRes] = await Promise.allSettled([
      getMoralReportChart(params),
      getMoralReportScoreRank(params),
    ]);
