...
export function dataList() {
  return deviceCodes.map((deviceCode, index) => {
    ...
    return {
      ...
      alarmTime,
      alarmRemark: isAbnormal ? '充电设备离线，触发异常告警' : '',
      processStatus,
...
export function normalizeCarChargeMonitorRow(row) {
...
export function filterMockList(params = {}) {
  const list = dataList().map((item) => normalizeCarChargeMonitorRow(item));
...
export const textObj = {
  excelAllName: '汽车充电监测数据.xlsx',
  total: '汽车充电监测支持状态采集、状态更新、异常标记、状态同步闭环管理',
};
