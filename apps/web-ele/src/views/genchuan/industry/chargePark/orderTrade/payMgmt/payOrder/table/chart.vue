...
import { getPayOrderChart } from '#/api/genchuan/industry/chargePark/orderTrade/payMgmt/index.js';
import Card from '#/components/stats/card.vue';

const state = reactive({
  cardList: [
    { title: '今日订单数', value: 0, color: '#FF6B6B' },
    { title: '支付成功率', value: 0, color: '#4ECDC4', suffix: '%' },
    { title: '渠道数量', value: 0, color: '#13ce66' },
  ],
  trendData: [],
});
...
const fetchOrderChartData = async () => {
  try {
    ...
    console.error('获取支付订单图表数据失败:', error);
    // 接口调用失败时使用假数据
