  ...
  <DetailDrawer :title="drawerTitle">
    <div class="detail-card">
      ...
      <div class="detail-row"><span
        class="label">机构名称：</span>{{ detailObj.name || detailObj.toiletName || '-' }}
      </div>
      <div class="detail-row"><span class="label">机构类型：</span>{{
          detailObj.institutionTypeName || detailObj.institutionType || '-'
        }}
      </div>
      <div class="detail-row"><span
        class="label">机构地址：</span>{{ detailObj.address || detailObj.location || '-' }}
      </div>
      <div class="detail-row"><span
      ...
      <template v-if="detailObj.name !== undefined">
        <div class="detail-row"><span
          class="label">保洁达标率：</span>{{ detailObj.cleaningRate ?? '-' }}%
        </div>
        <div class="detail-row"><span
          class="label">问题办结率：</span>{{ detailObj.problemRate ?? '-' }}%
        </div>
        <div class="detail-row"><span
          class="label">垃圾清运量：</span>{{ detailObj.wasteVolume ?? '-' }}kg
        </div>
        <div class="detail-row"><span
          class="label">核查通过率：</span>{{ detailObj.inspectionPassRate ?? '-' }}%
        </div>
        <div class="detail-row"><span
        ...
        <div class="detail-row"><span
          class="label">责任区域：</span>{{ detailObj.responsibilityArea || '-' }}
        </div>
      </template>
      ...
      <template v-if="detailObj.status === '保洁待执行'">
        ...
        <div class="detail-row"><span
          class="label">保洁内容：</span>{{ detailObj.cleaningContent || '-' }}
        </div>
        <div class="detail-row"><span class="label">保洁人员：</span>{{ detailObj.cleaner || '-' }}
        </div>
        <div class="detail-row"><span
          class="label">责任区域：</span>{{ detailObj.responsibilityArea || '-' }}
        </div>
      ...
      <template v-else-if="detailObj.status === '问题待处置'">
        ...
        <div class="detail-row"><span class="label">问题编号：</span>{{
            detailObj.complaintId || '-'
          }}
        </div>
        <div class="detail-row"><span
        ...
        <div class="detail-row"><span
          class="label">派单状态：</span>{{ detailObj.dispatchStatus || '-' }}
        </div>
      ...
      <template v-else-if="detailObj.status === '核查待验收'">
        ...
        <div class="detail-row"><span class="label">核查编号：</span>{{ detailObj.repairId || '-' }}
        </div>
        <div class="detail-row"><span class="label">关联任务：</span>{{ detailObj.taskId || '-' }}
        </div>
        <div class="detail-row"><span class="label">任务类型：</span>{{
            detailObj.facilityType || '-'
          }}
        </div>
        <div class="detail-row"><span class="label">任务描述：</span>{{
        ...
        <div class="detail-row"><span class="label">上报结果：</span>{{
            detailObj.reportResult || '-'
          }}
        </div>
        ...
        <div class="detail-row"><span class="label">核查人员：</span>{{ detailObj.repairBy || '-' }}
        </div>
      ...
      <template v-else-if="detailObj.status === '已完成'">
        ...
        <div class="detail-row"><span
          class="label">保洁达标率：</span>{{ detailObj.cleaningQualifiedRate ?? '-' }}%
        </div>
        <div class="detail-row"><span
