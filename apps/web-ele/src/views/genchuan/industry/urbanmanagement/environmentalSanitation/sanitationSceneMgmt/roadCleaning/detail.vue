...
<template>
  <DetailDrawer :title="drawerTitle">
    <div class="detail-card">
      ...
      <div class="detail-row"><span class="label">责任区域：</span>{{ detailObj.area || '-' }}</div>
      ...
      <!-- 动态显示各状态特有信息 -->
      <template v-if="detailObj.status === '清扫待执行'">
        <div class="detail-section">🧹 清扫计划详情</div>
        <div class="detail-row"><span class="label">清扫工具：</span>{{ detailObj.tool || '-' }}</div>
      ...
      <template v-else-if="detailObj.status === '作业进行中'">
        ...
        <div class="detail-row"><span class="label">是否异常：</span>{{ detailObj.isAbnormal ? '是' : '否' }}</div>
      </template>

      <template v-else-if="detailObj.status === '问题待处置'">
        <div class="detail-section">⚠️ 问题信息</div>
        <div class="detail-row"><span class="label">问题编号：</span>{{ detailObj.problemId || '-' }}</div>
        <div class="detail-row"><span class="label">问题类型：</span>{{ detailObj.problemType || '-' }}</div>
        <div class="detail-row"><span class="label">问题位置：</span>{{ detailObj.location || '-' }}</div>
        ...
        <div class="detail-row"><span class="label">问题描述：</span>{{ detailObj.desc || '-' }}</div>
        ...
        <div class="detail-row"><span class="label">超时提醒：</span>{{ detailObj.isTimeout ? '超时' : '正常' }}</div>
      </template>

      <template v-else-if="detailObj.status === '质量待核查'">
        ...
        <div class="detail-row"><span class="label">上报照片：</span>
          <span v-if="detailObj.checkPhotoUrl">
            <a v-for="(url, index) in detailObj.checkPhotoUrl.split(',')" :key="index" :href="url" target="_blank">照片{{ index+1 }} </a>
          </span>
          <span v-else>-</span>
        </div>
        <div class="detail-row"><span class="label">清扫工具：</span>{{ detailObj.tool || '-' }}</div>
        <div class="detail-row"><span class="label">核查状态：</span>{{ detailObj.reviewStatus || '-' }}</div>
        <div class="detail-row"><span class="label">核查人员：</span>{{ detailObj.reviewBy || '-' }}</div>
      ...
      <template v-else-if="detailObj.status === '已完成'">
        ...
        <div class="detail-row"><span class="label">质量核查结果：</span>{{ detailObj.reviewResult || '-' }}</div>
