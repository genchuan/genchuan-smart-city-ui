...
const getTitle = computed(() => {
  return formData.value?.alarmId ? textObj.editText : textObj.addText;
});
...
async function handleDeleteBatch() {
  ...
  try {
    dataObj.apilist = dataObj.apilist.filter(
      (v) => !checkedIds.value.includes(v.alarmId),
    );
    checkedIds.value = [];
...
function handleRowCheckboxChange({ records }) {
  checkedIds.value = records.map((item) => item.alarmId);
}
...
<template>
  <div class="park-lot-table-new">
    ...
    <Grid>
      ...
      <template #dealStatusName="{ row }">
        <el-tag :type="getStatusType(row.dealStatusName)">
          {{ row.dealStatusName }}
        </el-tag>
      </template>
      ...
      <template #bottom>
        <div class="common-total" @click="changeTotalShow">
          ...
          <span> 本页统计：告警数: {{ dataObj.total }} </span>
        </div>
